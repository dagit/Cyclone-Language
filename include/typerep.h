...
typedef datatype Typestruct @`r typestruct_t<`r>;
...
EXTERN_TYPEREP datatype Typestruct {
  ...
  ThinPtr(unsigned int,typestruct_t);
  ...
  **/
  Struct(string_t<`H>*, unsigned int, $(unsigned int, string_t<`H>, typestruct_t)@?);
  /** [Tuple(szb,(ofs_1,typ_1)::...)] is a buffer of values, one per
      element in the array.  The value at index i has type [typ_i] and
      is at offset [ofs_i].  The total size is [szb] bytes. 
  **/
  Tuple(unsigned int, $(unsigned int,typestruct_t)@?);
  ...
  **/
  TUnion(string_t<`H>,$(unsigned int,string_t<`H>)@?,
			$(unsigned int, string_t<`H>,typestruct_t)@?);
  ...
  **/
  ...
  TUnionField(string_t<`H>,string_t<`H>,unsigned int, $(unsigned int,typestruct_t)@?);
  ...
  **/
  XTUnion(string_t<`H>,$(string_t<`H>, typestruct_t)@?);
  ...
  **/
  Union(string_t<`H>*,int,$(string_t<`H>,typestruct_t)@?);
