...
/* $Id: index_dir.c,v 1.1 2001-09-24 21:59:18 mharris Exp $*/
...
char *html_escape_string(char *inp, char *buf);
char *http_escape_string(char *inp, char *buf);
 ...
 */
char *html_escape_string(char *inp, char *dest)
{
    int max;
    char *buf;
    unsigned char c;
...
int index_directory(char *dir, char *title)
{
    ...
    for (i = 0; i < numdir; ++i) {
        ...
        if (html_escape_string(dirbuf->d_name, html_filename) == NULL) {
            send_error(4);
            return -1;
        }
        if (http_escape_string(dirbuf->d_name, http_filename) == NULL) {
            send_error(4);
            return -1;
        }
        printf("<tr>"
