...
extern pset_family primes_consensus(pset *T);
...
extern int verify();
static int primes_consensus_special_cases(pset *T, pset_family *Tnew);
static pset_family primes_consensus_merge(pset_family Tl, pset_family Tr, pset cl, pset cr);
static pset_family and_with_cofactor(pset_family A, register pset cof); pset_family primes_consensus(pset *T)
{
register pset cl, cr;
register int best;
pset_family Tnew, Tl, Tr;
if (primes_consensus_special_cases(T, &Tnew) == 2) {
...
static pset_family primes_consensus_merge(pset_family Tl, pset_family Tr, pset cl, pset cr)
{
register pset pl, pr, lastl, lastr, pt;
pset_family T, Tsave;
...
for( pl=Tl->data, lastl= pl+Tl->count*Tl->wsize; pl< lastl; pl+=Tl->wsize) {
for( pr=Tr->data, lastr= pr+Tr->count*Tr->wsize; pr< lastr; pr+=Tr->wsize) {
if (cdist01(pl, pr) == 1) {
consensus(pt, pl, pr);
...
for( p=A->data, last= p+A->count*A->wsize; p< last; p+=A->wsize) {
{register int i_=( p[0] & 0x03ff); (p[0] &= ~0x03ff, p[0] |= (i_)); do p[i_] = p[i_] & cof[i_]; while (--i_>0);};
