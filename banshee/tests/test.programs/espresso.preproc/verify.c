...
extern PLA_permute(pPLA PLA1, pPLA PLA2);
extern int PLA_verify(pPLA PLA1, pPLA PLA2);
extern int check_consistency(pPLA PLA);
extern int verify(pset_family F, pset_family Fold, pset_family Dold);
...
FD = cube2list(F, Dold);
for( p=Fold->data, last= p+Fold->count*Fold->wsize; p< last; p+=Fold->wsize)
if (! cube_is_covered(FD, p)) {
...
{
if (PLA1->label != 0 && PLA1->label[0] != 0 &&
PLA2->label != 0 && PLA2->label[0] != 0) {
PLA_permute(PLA1, PLA2);
} else {
fprintf((&_iob[2]), "Warning: cannot permute columns without names\n");
return 1;
}
if (PLA1->F->sf_size != PLA2->F->sf_size) {
fprintf((&_iob[2]), "PLA_verify: PLA's are not the same size\n");
return 1;
}
...
npermute = 0;
permute = ((int *) malloc(sizeof(int) * ( PLA2->F->sf_size)));
...
label = ((char * *) malloc(sizeof(char *) * ( cube.size)));
...
((PLA1->label) ? (free((char *) (PLA1->label)), (PLA1->label) = 0) : 0);
PLA1->label = label;
