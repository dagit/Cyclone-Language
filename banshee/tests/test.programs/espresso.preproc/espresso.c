...
extern pset_family espresso(pset_family F, pset_family D1, pset_family R);
...
begin:
Fsave = sf_save(F);	D = sf_save(D1);	if (recompute_onset) {
...
F = sf_append(F, E); if (trace) size_stamp(F, "ADJUST     ");
}
sf_free(D);
if (! skip_make_sparse) {
