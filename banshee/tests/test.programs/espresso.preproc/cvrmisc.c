...
extern char *fmt_cost(pcost cost);
extern char *print_cost(pset_family F);
extern char *util_strsav();
extern void copy_cost(pcost s, pcost d);
extern void cover_cost(pset_family F, pcost cost);
extern void fatal(char *s);
extern void print_trace(pset_family T, char *name, long int time);
extern void size_stamp(pset_family T, char *name);
extern void totals(long int time, int i, pset_family T, pcost cost);
...
name, util_print_time(time), print_cost(T));
(void) fflush((&_iob[1]));
}
...
if (trace) {
printf("# %s\tTime was %s, cost is %s\n",
total_name[i], util_print_time(time), fmt_cost(cost));
(void) fflush((&_iob[1]));
}
}
void fatal(char *s)
{
fprintf((&_iob[2]), "espresso: %s\n", s);
exit(1);
}
