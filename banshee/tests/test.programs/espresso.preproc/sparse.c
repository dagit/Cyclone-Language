...
extern pset_family make_sparse(pset_family F, pset_family D, pset_family R);
extern pset_family mv_reduce(pset_family F, pset_family D);
...
do {
{long t=util_cpu_time();F = mv_reduce(F, D);totals(t, 12, F,&( cost));};
if (cost.total == best_cost.total)
break;
copy_cost(&cost, &best_cost);
{long t=util_cpu_time();F = expand(F, R, 1);totals(t, 13, F,&( cost));};
if (cost.total == best_cost.total)
break;
copy_cost(&cost, &best_cost);
...
index = 0;
for( p1=F1->data, last= p1+F1->count*F1->wsize; p1< last; p1+=F1->wsize) {
if (! (p1[0] & ( 0x2000))) {
...
for(var = 0; var < cube.num_vars; var++) {
if (cube.sparse[var]) {
for( p=F->data, last= p+F->count*F->wsize; p< last; p+=F->wsize) if (( p[0] & ( 0x2000))) {
