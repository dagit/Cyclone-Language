...
extern pset_family minimize_exact(pset_family F, pset_family D, pset_family R, int exact_cover);
extern pset_family minimize_exact_literals(pset_family F, pset_family D, pset_family R, int exact_cover);
...
static pset_family do_minimize(pset_family F, pset_family D, pset_family R, int exact_cover, int weighted);
pset_family
...
if (weights != 0) {
((weights) ? (free((char *) (weights)), (weights) = 0) : 0);
}
if (debug & 0x0400) {
...
{
struct _iobuf *fp_pi_table, *fp_primes;
pPLA PLA;
pset last, p;
char *file;
if (filename == 0 || strcmp(filename, "(stdin)") == 0) {
fp_pi_table = fp_primes = (&_iob[1]);
} else {
file = ((char *) malloc(sizeof(char) * ( strlen(filename)+20)));
(void) sprintf(file, "%s.primes", filename);
if ((fp_primes = fopen(file, "w")) == 0) {
fprintf((&_iob[2]), "espresso: Unable to open %s\n", file);
fp_primes = (&_iob[1]);
}
(void) sprintf(file, "%s.pi", filename);
if ((fp_pi_table = fopen(file, "w")) == 0) {
fprintf((&_iob[2]), "espresso: Unable to open %s\n", file);
fp_pi_table = (&_iob[1]);
}
((file) ? (free((char *) (file)), (file) = 0) : 0);
}
PLA = new_PLA();
PLA_labels(PLA);
