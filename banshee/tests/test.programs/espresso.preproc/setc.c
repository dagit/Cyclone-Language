...
extern int ccommon(register pset a, register pset b, register pset cof);
extern int cdist0(register pset a, register pset b);
extern int full_row(register pset p, register pset cof);
extern int ascend(pset *a, pset *b);
extern int cactive(register pset a);
extern int cdist(register pset a, register pset b);
extern int cdist01(register pset a, register pset b);
extern int cvolume();
extern int d1_order(pset *a, pset *b);
extern int d1_order_size();
extern int desc1(register pset a, register pset b);
extern int descend(pset *a, pset *b);
extern int lex_order(pset *a, pset *b);
extern int lex_order1();
extern pset force_lower(pset xlower, register pset a, register pset b);
extern void consensus(pset r, register pset a, register pset b);
...
extern int verify();
int full_row(register pset p, register pset cof)
{
register int i = (p[0] & 0x03ff);
do if ((p[i] | cof[i]) != cube.fullset[i]) return 0; while (--i > 0);
return 1;
}

int cdist0(register pset a, register pset b)
{
{ register int w, last; register unsigned int x;
if ((last = cube.inword) != -1) {
x = a[last] & b[last];
if (~(x | x >> 1) & cube.inmask)
return 0; for(w = 1; w < last; w++) {
x = a[w] & b[w];
if (~(x | x >> 1) & 0x55555555)
return 0; }
}
}
{ register int w, var, last; register pset mask;
...
if ((dist = (bit_count[x & 255] + bit_count[(x >> 8) & 255] + bit_count[(x >> 16) & 255] + bit_count[(x >> 24) & 255])) > 1)
return 2;
for(w = 1; w < last; w++) {
x = a[w] & b[w];
if (x = ~ (x | x >> 1) & 0x55555555)
if (dist == 1 || (dist += (bit_count[x & 255] + bit_count[(x >> 8) & 255] + bit_count[(x >> 16) & 255] + bit_count[(x >> 24) & 255])) > 1)
return 2;
}
}
}
...
x = a[last] & b[last];
if (x = ~ (x | x >> 1) & cube.inmask)
dist = (bit_count[x & 255] + bit_count[(x >> 8) & 255] + bit_count[(x >> 16) & 255] + bit_count[(x >> 24) & 255]);
for(w = 1; w < last; w++) {
x = a[w] & b[w];
if (x = ~ (x | x >> 1) & 0x55555555)
dist += (bit_count[x & 255] + bit_count[(x >> 8) & 255] + bit_count[(x >> 16) & 255] + bit_count[(x >> 24) & 255]);
...
{
{register int i_=(( cube.size <= 32) ? 1 : (((( cube.size)-1) >> 5) + 1)); *r=i_; do r[i_] = 0; while (--i_ > 0);};
{ register int w, last; register unsigned int x;
if ((last = cube.inword) != -1) {
...
{ int empty; int var; unsigned int x;
register int w, last; register pset mask;
for(var = cube.num_binary_vars; var < cube.num_vars; var++) {
...
if (x = ~ (x & x >> 1) & cube.inmask) {
if ((dist = (bit_count[x & 255] + bit_count[(x >> 8) & 255] + bit_count[(x >> 16) & 255] + bit_count[(x >> 24) & 255])) > 1)
return -1;	active = (last-1)*(32/2) + bit_index(x) / 2;
}
for(w = 1; w < last; w++) {
x = a[w];
if (x = ~ (x & x >> 1) & 0x55555555) {
if ((dist += (bit_count[x & 255] + bit_count[(x >> 8) & 255] + bit_count[(x >> 16) & 255] + bit_count[(x >> 24) & 255])) > 1)
return -1;	active = (w-1)*(32/2) + bit_index(x) / 2;
}
}
}
}
...

int ccommon(register pset a, register pset b, register pset cof)
{
{ int last;
register int w;
...
if ((a1[0] >> 16) > (b1[0] >> 16)) return -1;
else if ((a1[0] >> 16) < (b1[0] >> 16)) return 1;
else {
register int i = (a1[0] & 0x03ff);
do
...
return -1;
if ((a[0] >> 16) > (b[0] >> 16)) return -1;
else if ((a[0] >> 16) < (b[0] >> 16)) return 1;
else {
