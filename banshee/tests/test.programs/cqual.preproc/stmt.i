...
void check_return(expression e)
{
  ...
  if (type_void(ret))
    ...
    {
      check_assignment(ret, default_conversion_for_assignment(e), e, "return", ((void *)0), ((void *)0), 0);
    }
}
void check_computed_goto(expression e)
{
...
label_declaration new_label_declaration(region r, const char *name, id_label firstuse)
{
  label_declaration ldecl = (sizeof(struct label_declaration) < (1 << (13 - 3)) ? (((void)0), __rc_ralloc_small0)((r), sizeof(struct label_declaration)) : (((void)0), __rc_typed_ralloc)((r), sizeof(struct label_declaration), 0));
  ldecl->name = name;
  ldecl->explicitly_declared = 0;
  ldecl->firstuse = firstuse;
  ldecl->definition = ((void *)0);
...
void lookup_label(id_label label)
{
  ...
  ldecl = env_lookup(current_function_decl->scoped_labels, label->cstring.data,
                     0);
...
{
  label_declaration ldecl =
    env_lookup(current_function_decl->scoped_labels, label->cstring.data, 1);
  if (ldecl)
    {
      label->ldecl = ldecl;
      duplicate_label_error(label);
...
static breakable_stmt containing_switch(label l)
{
  ...
  while (sw && !((sw)->kind >= kind_switch_stmt && (sw)->kind <= postkind_switch_stmt))
    sw = sw->parent_loop;
  if (sw)
    {
      label *last = &({ast_generic tEmPcast = (ast_generic)(sw); if (tEmPcast) ((void) ((((tEmPcast)->kind >= kind_switch_stmt && (tEmPcast)->kind <= postkind_switch_stmt)) ? 0 : (__assert_fail ("((tEmPcast)->kind >= kind_switch_stmt && (tEmPcast)->kind <= postkind_switch_stmt)", "stmt.c", 206, __PRETTY_FUNCTION__), 0))); (switch_stmt)(tEmPcast); })->next_label;
      bool lisdefault = ((l)->kind >= kind_default_label && (l)->kind <= postkind_default_label);
      while (*last)
        {
          if (((*last)->kind >= kind_default_label && (*last)->kind <= postkind_default_label) && lisdefault)
            {
...
void check_case(label label0)
{
  ...
  case_label label = ({ast_generic tEmPcast = (ast_generic)(label0); if (tEmPcast) ((void) ((((tEmPcast)->kind >= kind_case_label && (tEmPcast)->kind <= postkind_case_label)) ? 0 : (__assert_fail ("((tEmPcast)->kind >= kind_case_label && (tEmPcast)->kind <= postkind_case_label)", "stmt.c", 238, __PRETTY_FUNCTION__), 0))); (case_label)(tEmPcast); });
  if (!sw)
...
void check_break(statement break_statement)
{
  break_stmt bs = ({ast_generic tEmPcast = (ast_generic)(break_statement); if (tEmPcast) ((void) ((((tEmPcast)->kind >= kind_break_stmt && (tEmPcast)->kind <= postkind_break_stmt)) ? 0 : (__assert_fail ("((tEmPcast)->kind >= kind_break_stmt && (tEmPcast)->kind <= postkind_break_stmt)", "stmt.c", 261, __PRETTY_FUNCTION__), 0))); (break_stmt)(tEmPcast); });
...
void check_continue(statement continue_statement)
{
  ...
  continue_stmt cs = ({ast_generic tEmPcast = (ast_generic)(continue_statement); if (tEmPcast) ((void) ((((tEmPcast)->kind >= kind_continue_stmt && (tEmPcast)->kind <= postkind_continue_stmt)) ? 0 : (__assert_fail ("((tEmPcast)->kind >= kind_continue_stmt && (tEmPcast)->kind <= postkind_continue_stmt)", "stmt.c", 272, __PRETTY_FUNCTION__), 0))); (continue_stmt)(tEmPcast); });
  while (loop && ((loop)->kind >= kind_switch_stmt && (loop)->kind <= postkind_switch_stmt))
    loop = loop->parent_loop;
  if (!loop)
    error("continue statement not within a loop");
  cs->parent_loop = loop;
}
const char CANON_IDENT_c2d5884c254a2357979d0449b5c29ecc[] = "CANON_IDENT_/moa/sc1/jkodumal/work/banshee/experiments/cqual/src/stmt.c";
