...
void start_timer(timer *t)
{
  ((void) ((!((&t->start)->tv_sec || (&t->start)->tv_usec)) ? 0 : (__assert_fail ("!((&t->start)->tv_sec || (&t->start)->tv_usec)", "timer.c", 122, __PRETTY_FUNCTION__), 0)));
  ((void) ((!(gettimeofday(&t->start, ((void *)0)))) ? 0 : (__assert_fail ("!(gettimeofday(&t->start, ((void *)0)))", "timer.c", 123, __PRETTY_FUNCTION__), 0)));
}
void end_timer(timer *t)
{
  struct timeval end, diff;
  ((void) ((!(gettimeofday(&end, ((void *)0)))) ? 0 : (__assert_fail ("!(gettimeofday(&end, ((void *)0)))", "timer.c", 132, __PRETTY_FUNCTION__), 0)));
  ((void) ((((&t->start)->tv_sec || (&t->start)->tv_usec)) ? 0 : (__assert_fail ("((&t->start)->tv_sec || (&t->start)->tv_usec)", "timer.c", 133, __PRETTY_FUNCTION__), 0)));
  do { (&diff)->tv_sec = (&end)->tv_sec - (&t->start)->tv_sec; (&diff)->tv_usec = (&end)->tv_usec - (&t->start)->tv_usec; if ((&diff)->tv_usec < 0) { --(&diff)->tv_sec; (&diff)->tv_usec += 1000000; } } while (0);
  do { (&t->elapsed)->tv_sec = (&t->elapsed)->tv_sec + (&diff)->tv_sec; (&t->elapsed)->tv_usec = (&t->elapsed)->tv_usec + (&diff)->tv_usec; if ((&t->elapsed)->tv_usec >= 1000000) { ++(&t->elapsed)->tv_sec; (&t->elapsed)->tv_usec -= 1000000; } } while (0);
  ((&t->start)->tv_sec = (&t->start)->tv_usec = 0);
}
char *timer_to_ascii(timer *t)
{
  static char buf[80];
  ...
  snprintf(buf, 80, "%.3f", secs);
  buf[80 -1] = '\0';
  return buf;
}
const char CANON_IDENT_18795192e311288f9a9fcde9bb0b5126[] = "CANON_IDENT_/moa/sc1/jkodumal/work/banshee/experiments/cqual/src/timer.c";
