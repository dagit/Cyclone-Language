...
bool file_pointer_qtype(qtype qt);
hash_table globals = ((void *)0);
effect global_effect = ((void *)0);
effect global_env = ((void *)0);
inline void mk_effect_leq_global_env(effect e)
{
...
einfo do_assign(einfo arg1, einfo arg2, location loc, effect eff)
{
  ...
  if (mkleq_assign_convert(loc, &fi_qgate, arg2.qt, points_to_qtype(arg1.qt)))
    report_qerror(loc, sev_err, "incompatible types in assignment");
...
einfo do_cast(einfo arg1, type cast_t, location loc, context context)
{
  ...
  if (context == lpos || context == apos)
    {
      type pt;
      ((void) ((!type_function(cast_t)) ? 0 : (__assert_fail ("!type_function(cast_t)", "common-analyze.c", 82, __PRETTY_FUNCTION__), 0)));
      ((void) ((!type_array(cast_t)) ? 0 : (__assert_fail ("!type_array(cast_t)", "common-analyze.c", 83, __PRETTY_FUNCTION__), 0)));
  ...
  if (flag_casts_preserve &&
    ...
    if (mkeq_qtype_cast(loc, &fi_qgate, qt, arg1.qt))
...
{
  qtype qt;
  ((void) ((context == rpos) ? 0 : (__assert_fail ("context == rpos", "common-analyze.c", 112, __PRETTY_FUNCTION__), 0)));
  qt = mkqtype_size_t(make_qvar(tag, loc, 0,0));
  return mkeinfo(qt, effect_empty, 0);
}
                ...
                einfo arg1, einfo arg2, effect eff, operator op)
{
  qtype qt;
  einfo result;
  qt = mkqtype_bool(make_qvar("__boolop", loc, 0,0));
  if (is_eq_or_neq &&
  ...
  if (op && add_op_constraints(op, result, arg1, arg2, loc))
    report_qerror(loc, sev_err,
...
sinfo do_change_type(einfo exp, type t, qtype *qt, location loc)
{
  ...
  if (qt)
    {
      ((void) ((!*qt) ? 0 : (__assert_fail ("!*qt", "common-analyze.c", 166, __PRETTY_FUNCTION__), 0)));
      *qt = new_qt;
    }
  al = aloc_qtype(exp.qt);
  mark_aloc_interesting(al);
  ((void) ((qtype_pointer(exp.qt)) ? 0 : (__assert_fail ("qtype_pointer(exp.qt)", "common-analyze.c", 173, __PRETTY_FUNCTION__), 0)));
  if (mkleq_qtype(loc, &fi_qgate, points_to_qtype(exp.qt), new_qt))
    report_qerror(loc, sev_err,
...
sinfo do_assert_type(einfo exp, type t, qtype *qt, location loc, location tloc)
{
  ...
  if (qt)
    {
      ((void) ((!*qt) ? 0 : (__assert_fail ("!*qt", "common-analyze.c", 188, __PRETTY_FUNCTION__), 0)));
      *qt = assert_qt;
    }
  if (mkleq_qtype(loc, &fi_qgate, exp.qt, assert_qt))
    report_qerror(loc, sev_err,
...
einfo put_id_in_context(location loc, einfo ei, context c)
{
  ...
  if (qtype_error(ei.qt))
    ...
    if (c == rpos)
      ...
      result_qt = mkqtype_pointer(loc,
                                  make_fresh_qvar("q", loc),
                                  ei.qt,
                                  array_aloc_qtype(ei.qt));
    else
      __fail("common-analyze.c", 229, __FUNCTION__, "Unexpected context %d\n", c);
  }
  else if (qtype_function(ei.qt)) {
    ((void) ((c == rpos) ? 0 : (__assert_fail ("c == rpos", "common-analyze.c", 232, __PRETTY_FUNCTION__), 0)));
    if (flag_poly)
      {
        result_qt = copy_qtype_poly(ei.qt, loc);
        mkinst_qtype(loc, &open_qgate, ei.qt, result_qt, p_pos);
      ...
      if (qtype_pointer(ei.qt))
        {
          ...
          result_eff = effect_union(result_eff,
                                    effect_r(aloc_qtype(ei.qt)));
        }
      else
        __fail("common-analyze.c", 264, __FUNCTION__, "Unexpected qtype\n");
    }
  else __fail("common-analyze.c", 267, __FUNCTION__, "(at %s:%ld) " "Unexpected context %d\n", ((loc)->filename), ((loc)->lineno), c);
  return mkeinfo(result_qt, result_eff, 0);
}
bool init_string(location loc, qtype lhs_qtype, qtype rhs_qtype)
{
  bool result = 0;
...
void traverse_globals(void (*f)(data_declaration, void *), void *arg)
{
  ...
  while (hash_table_next(&hts, ((void *)0), (hash_data *) &ddecl))
    {
      ((void) ((!ddecl->shadowed) ? 0 : (__assert_fail ("!ddecl->shadowed", "common-analyze.c", 357, __PRETTY_FUNCTION__), 0)));
      f(ddecl, arg);
    }
}
                             ...
                             void *arg)
{
  ...
  while (hash_table_next_sorted(&htss, ((void *)0), (hash_data *) &ddecl))
    {
      ((void) ((!ddecl->shadowed) ? 0 : (__assert_fail ("!ddecl->shadowed", "common-analyze.c", 371, __PRETTY_FUNCTION__), 0)));
      f(ddecl, arg);
    }
}
...
bool mkNonConst_qual(location loc, qual q)
{
  if (!nonconst_qual)
    return 0;
  return mkleq_qual(loc, &fi_qgate, q, nonconst_qual);
}
bool mkNonConst_pointer(location loc, qtype ptr_qtype)
{
  ...
  if (!nonconst_qual)
    return 0;
  ((void) ((qtype_pointer(ptr_qtype)) ? 0 : (__assert_fail ("qtype_pointer(ptr_qtype)", "common-analyze.c", 409, __PRETTY_FUNCTION__), 0)));
...
bool mkConst_pointer(location loc, qtype ptr_qtype)
{
  ...
  if (!const_qual)
    return 0;
  ((void) ((qtype_pointer(ptr_qtype)) ? 0 : (__assert_fail ("qtype_pointer(ptr_qtype)", "common-analyze.c", 424, __PRETTY_FUNCTION__), 0)));
  result = mkleq_qual(loc, &fi_qgate, const_qual, qual_qtype(ptr_qtype));
  return result;
}
...
struct operator operators[] =
  {
    {"_op_uminus", sig_unop, ((void *)0)},
    {"_op_uplus", sig_unop, ((void *)0)},
    {"_op_bitnot", sig_unop, ((void *)0)},
    {"_op_not", sig_unop, ((void *)0)},
    {"_op_times", sig_binop, ((void *)0)},
    {"_op_div", sig_binop, ((void *)0)},
    {"_op_mod", sig_binop, ((void *)0)},
    {"_op_lshift", sig_binop, ((void *)0)},
    {"_op_rshift", sig_binop, ((void *)0)},
    {"_op_lt", sig_binop, ((void *)0)},
    {"_op_gt", sig_binop, ((void *)0)},
    {"_op_leq", sig_binop, ((void *)0)},
    {"_op_geq", sig_binop, ((void *)0)},
    {"_op_eq", sig_binop, ((void *)0)},
    {"_op_neq", sig_binop, ((void *)0)},
    {"_op_bitand", sig_binop, ((void *)0)},
    {"_op_bitor", sig_binop, ((void *)0)},
    {"_op_bitxor", sig_binop, ((void *)0)},
    {"_op_andand", sig_binop, ((void *)0)},
    {"_op_oror", sig_binop, ((void *)0)},
    {"_op_deref", sig_deref, ((void *)0)},
    {0, 0, ((void *)0)}};
inline operator find_op_name(const char *name)
{
  int i;
  for (i = 0; operators[i].name; i++)
    {
      if (!__extension__ ({ size_t __s1_len, __s2_len; (__builtin_constant_p (name) && __builtin_constant_p (operators[i].name) && (__s1_len = strlen (name), __s2_len = strlen (operators[i].name), (!((size_t)(const void *)((name) + 1) - (size_t)(const void *)(name) == 1) || __s1_len >= 4) && (!((size_t)(const void *)((operators[i].name) + 1) - (size_t)(const void *)(operators[i].name) == 1) || __s2_len >= 4)) ? memcmp ((__const char *) (name), (__const char *) (operators[i].name), (__s1_len < __s2_len ? __s1_len : __s2_len) + 1) : (__builtin_constant_p (name) && ((size_t)(const void *)((name) + 1) - (size_t)(const void *)(name) == 1) && (__s1_len = strlen (name), __s1_len < 4) ? (__builtin_constant_p (operators[i].name) && ((size_t)(const void *)((operators[i].name) + 1) - (size_t)(const void *)(operators[i].name) == 1) ? (__extension__ ({ register int __result = (((__const unsigned char *) (__const char *) (name))[0] - ((__const unsigned char *) (__const char *)(operators[i].name))[0]); if (__s1_len > 0 && __result == 0) { __result = (((__const unsigned char *) (__const char *) (name))[1] - ((__const unsigned char *) (__const char *) (operators[i].name))[1]); if (__s1_len > 1 && __result == 0) { __result = (((__const unsigned char *) (__const char *) (name))[2] - ((__const unsigned char *) (__const char *) (operators[i].name))[2]); if (__s1_len > 2 && __result == 0) __result = (((__const unsigned char *) (__const char *) (name))[3] - ((__const unsigned char *) (__const char *) (operators[i].name))[3]); } } __result; })) : (__extension__ ({ __const unsigned char *__s2 = (__const unsigned char *) (__const char *) (operators[i].name); register int __result = (((__const unsigned char *) (__const char *) (name))[0] - __s2[0]); if (__s1_len > 0 && __result == 0) { __result = (((__const unsigned char *) (__const char *) (name))[1] - __s2[1]); if (__s1_len > 1 && __result == 0) { __result = (((__const unsigned char *) (__const char *) (name))[2] - __s2[2]); if (__s1_len > 2 && __result == 0) __result = (((__const unsigned char *) (__const char *) (name))[3] - __s2[3]); } } __result; }))) : (__builtin_constant_p (operators[i].name) && ((size_t)(const void *)((operators[i].name) + 1) - (size_t)(const void *)(operators[i].name) == 1) && (__s2_len = strlen (operators[i].name), __s2_len < 4) ? (__builtin_constant_p (name) && ((size_t)(const void *)((name) + 1) - (size_t)(const void *)(name) == 1) ? (__extension__ ({ register int __result = (((__const unsigned char *) (__const char *) (name))[0] - ((__const unsigned char *) (__const char *)(operators[i].name))[0]); if (__s2_len > 0 && __result == 0) { __result = (((__const unsigned char *) (__const char *) (name))[1] - ((__const unsigned char *) (__const char *) (operators[i].name))[1]); if (__s2_len > 1 && __result == 0) { __result = (((__const unsigned char *) (__const char *) (name))[2] - ((__const unsigned char *) (__const char *) (operators[i].name))[2]); if (__s2_len > 2 && __result == 0) __result = (((__const unsigned char *) (__const char *) (name))[3] - ((__const unsigned char *) (__const char *) (operators[i].name))[3]); } } __result; })) : (__extension__ ({ __const unsigned char *__s1 = (__const unsigned char *) (__const char *) (name); register int __result = __s1[0] - ((__const unsigned char *) (__const char *) (operators[i].name))[0]; if (__s2_len > 0 && __result == 0) { __result = (__s1[1] - ((__const unsigned char *) (__const char *) (operators[i].name))[1]); if (__s2_len > 1 && __result == 0) { __result = (__s1[2] - ((__const unsigned char *) (__const char *) (operators[i].name))[2]); if (__s2_len > 2 && __result == 0) __result = (__s1[3] - ((__const unsigned char *) (__const char *) (operators[i].name))[3]); } } __result; }))) : strcmp (name, operators[i].name)))); }))
        return operators+i;
    }
  return ((void *)0);
}
      ...
      switch (k)
        ...
        case sig_deref:
          fmatch = (qtype_var(ret) && qtype_pointer(arg1) &&
                    !mkeq_qtype(loc, &fi_qgate, points_to_qtype(arg1), ret));
          break;
        case sig_addr:
        ...
        case sig_binop:
          ...
          break;
        default:
          __fail("common-analyze.c", 607, __FUNCTION__, "Unexpected signature kind %x\n", k);
        }
      return fmatch;
    }
  else
                        ...
                        location loc)
{
  ...
  bool result;
  result = 0;
  op_inst = instantiate_qtype(op->qt, ((void *)0));
  result = mkeq_qtype(loc, &fi_qgate, return_qtype(op_inst), ret.qt) || result;
  qtypelist_scan(arg_qtypes(op_inst), &s);
  result = mkeq_qtype(loc, &fi_qgate, arg1.qt, qtypelist_next(&s)) || result;
  if (arg2.qt)
    result = mkeq_qtype(loc, &fi_qgate, arg2.qt, qtypelist_next(&s)) || result;
  else
    ((void) ((!qtypelist_next(&s)) ? 0 : (__assert_fail ("!qtypelist_next(&s)", "common-analyze.c", 653, __PRETTY_FUNCTION__), 0)));
  return result;
}
const char CANON_IDENT_09fc4da14b9c45af65ddbee1db4ea8e6[] = "CANON_IDENT_/moa/sc1/jkodumal/work/banshee/experiments/cqual/src/common-analyze.c";
