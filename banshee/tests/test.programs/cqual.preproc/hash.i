...
{
  hash_table result;
  ((void) ((size > 0) ? 0 : (__assert_fail ("size > 0", "hash.c", 54, __PRETTY_FUNCTION__), 0)));
  result = (sizeof(struct Hash_table) < (1 << (13 - 3)) ? (((void)0), __rc_ralloc_small0)((r), sizeof(struct Hash_table)) : (((void)0), __rc_typed_ralloc)((r), sizeof(struct Hash_table), 0));
  result->r = r;
  result->hash = hash;
  result->cmp = cmp;
  result->size = 1;
  ...
  result->used = 0;
  result->table = (((void)0), __rc_typed_rarrayalloc)((result->r), (result->size), sizeof(bucket), 0);
  return result;
}
hash_table make_string_hash_table(region rhash, unsigned long size)
{
...
static inline bucket *find_bucket(hash_table ht, hash_key k)
{
  unsigned long hash;
  hash = ht->hash(k);
  hash = hash * ((unsigned long long)(2*0.6180339987 * (1ULL << (8 * sizeof(unsigned long) - 1))));
  hash = hash >> (8 * sizeof(unsigned long) - ht->log2size);
  ((void) ((hash < ht->size) ? 0 : (__assert_fail ("hash < ht->size", "hash.c", 109, __PRETTY_FUNCTION__), 0)));
...
bool hash_table_insert(hash_table ht, hash_key k, hash_data d)
{
  ...
  *cur = (sizeof(struct bucket) < (1 << (13 - 3)) ? (((void)0), __rc_ralloc_small0)((ht->r), sizeof(struct bucket)) : (((void)0), __rc_typed_ralloc)((ht->r), sizeof(struct bucket), 0));
  (*cur)->key = k;
  (*cur)->data = d;
  (*cur)->next = ((void *)0);
  ht->used++;
  ...
  for (i = 0; i < ht->size; i++)
    {
      ...
      for (cur = ht->table[i]; cur; cur = cur->next)
        {
          newbucket = (sizeof(struct bucket) < (1 << (13 - 3)) ? (((void)0), __rc_ralloc_small0)((result->r), sizeof(struct bucket)) : (((void)0), __rc_typed_ralloc)((result->r), sizeof(struct bucket), 0));
          newbucket->key = cur->key;
          newbucket->data = cur->data;
          newbucket->next = ((void *)0);
          ((void) ((!*prev) ? 0 : (__assert_fail ("!*prev", "hash.c", 210, __PRETTY_FUNCTION__), 0)));
          *prev = newbucket;
          prev = &newbucket->next;
        }
    }
...
static void rehash(hash_table ht)
{
  ...
  ht->used = 0;
  ht->table = (((void)0), __rc_typed_rarrayalloc)((ht->r), (ht->size), sizeof(bucket), 0);
  for (i = 0; i < old_table_size; i++)
    for (cur = old_table[i]; cur; cur = cur->next)
      ...
      for (cur = ht->table[i]; cur; cur = cur->next)
        {
          ...
          newbucket->next = ((void *)0);
          ((void) ((!*prev) ? 0 : (__assert_fail ("!*prev", "hash.c", 308, __PRETTY_FUNCTION__), 0)));
          *prev = newbucket;
          prev = &newbucket->next;
        }
    }
...
static keycmp_fn cur_cmp = ((void *)0);
                            ...
                            hash_table_scanner_sorted *htss)
{
  ...
  htss->entries = (((void)0), __rc_typed_rarrayalloc)((htss->r), (htss->size), sizeof(struct sorted_entry), 0);
  htss->i = 0;
  ...
  while (hash_table_next(&hts, &htss->entries[i].k,
                         &htss->entries[i].d))
    i++;
  ((void) ((i == htss->size) ? 0 : (__assert_fail ("i == htss->size", "hash.c", 342, __PRETTY_FUNCTION__), 0)));
  cur_cmp = f;
...
const char CANON_IDENT_31d340750f8ad15c29cb6c41e20f83a0[] = "CANON_IDENT_/moa/sc1/jkodumal/work/banshee/experiments/cqual/src/hash.c";
