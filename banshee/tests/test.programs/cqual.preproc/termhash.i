...
static void post_insert(term_hash tab) ;
static void rehash(term_hash tab) ;
static void reinsert(term_hash tab, term_bucket b);
static void insert(term_hash tab, term e, stamp * stamps, int len);
static void insert_entry(term_hash tab, struct hash_entry *entry);
static term walk(term_bucket b, stamp * stamps, int len);
...
term_hash make_term_hash(region rgn)
{
  ...
  int i;
  region r;
  term_hash tab = (sizeof(struct term_hash) < (1 << (13 - 3)) ? (((void)0), __rc_ralloc_small0)((rgn), sizeof(struct term_hash)) : (((void)0), __rc_typed_ralloc)((rgn), sizeof(struct term_hash), 0));
  r = newregion();
  ub = ((1<<initial_table_size)-1);
  n = (1<<initial_table_size);
  tab->term_buckets = (((void)0), __rc_typed_rarrayalloc)((r), (n), sizeof(term_bucket), 0);
  for (i = 0; i < n; i++)
    {
      tab->term_buckets[i] = ((void *)0);
    }
  tab->rgn = r;
  tab->ub = ub;
...
void term_hash_delete(term_hash tab)
{
  deleteregion(tab->rgn);
}
term term_hash_find(term_hash tab, stamp stamps[], int len)
{
  int hash_val;
  term_bucket b;
...
bool term_hash_insert(term_hash tab, term e, stamp * stamps, int len)
{
  if (term_hash_find(tab, stamps, len) != ((void *)0))
    {
      return 1;
    }
...
static void insert(term_hash tab, term e, stamp stamps[], int len)
{
  ...
  stamp * stamp_cpy;
  int i;
  entry = (sizeof(struct hash_entry) < (1 << (13 - 3)) ? (((void)0), __rc_ralloc_small0)((tab->rgn), sizeof(struct hash_entry)) : (((void)0), __rc_typed_ralloc)((tab->rgn), sizeof(struct hash_entry), 0));
  stamp_cpy = (((void)0), __rc_typed_rarrayalloc)((tab->rgn), (len), sizeof(stamp), 0);
  for (i = 0; i < len; i++)
    {
...
static void insert_entry(term_hash tab, hash_entry entry)
{
  ...
  b = tab->term_buckets[hash_val];
  new_term_bucket = (sizeof(struct term_bucket) < (1 << (13 - 3)) ? (((void)0), __rc_ralloc_small0)((tab->rgn), sizeof(struct term_bucket)) : (((void)0), __rc_typed_ralloc)((tab->rgn), sizeof(struct term_bucket), 0));
...
static void rehash(term_hash tab)
{
  ...
  old_rgn = tab->rgn;
  tab->rgn = newregion();
  tab->term_buckets = (((void)0), __rc_typed_rarrayalloc)((tab->rgn), (tab->capacity), sizeof(term_bucket), 0);
  for (i = 0; i < old_table_size; i++)
    {
      if (old_term_buckets[i] != ((void *)0) && old_term_buckets[i]->entry != ((void *)0))
...
const char CANON_IDENT_2872d93e0208e51af656266834ee9ce5[] = "CANON_IDENT_/moa/sc1/jkodumal/work/banshee/experiments/cqual/src/termhash.c";
