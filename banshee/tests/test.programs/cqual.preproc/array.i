...
struct array *new_array(region r, size_t initialsize,
                        size_t typesize, type_t typeinfo)
{
  struct array *a = (sizeof(struct array) < (1 << (13 - 3)) ? (((void)0), __rc_ralloc_small0)((r), sizeof(struct array)) : (((void)0), __rc_typed_ralloc)((r), sizeof(struct array), 0));
  a->r = r;
  a->data = (((void)0), __rc_typed_rarrayalloc)(r, initialsize, typesize, typeinfo);
  a->elemsize = typesize;
  a->elemtype = typeinfo;
  a->nelems = 0;
...
void *array_extend(struct array *a, int by)
{
  ...
  if (by < 0)
    ((void) ((-by <= a->nelems && by != (-2147483647 -1)) ? 0 : (__assert_fail ("-by <= a->nelems && by != (-2147483647-1)", "array.c", 52, __PRETTY_FUNCTION__), 0)));
  else if (a->nelems + by > a->nalloc)
    {
      size_t newsize = a->nalloc * 2 + by;
      void *newdata = (((void)0), __rc_typed_rarrayalloc)(a->r, newsize, a->elemsize, a->elemtype);
      ((void) ((newsize > a->nalloc) ? 0 : (__assert_fail ("newsize > a->nalloc", "array.c", 60, __PRETTY_FUNCTION__), 0)));
...
wchar_array new_wchar_array(region r, size_t initialsize) { return (wchar_array)new_array(r, initialsize, sizeof(wchar_t), 0); } wchar_t *wchar_array_extend(wchar_array a, int by) { return array_extend((struct array *)a, by); } void wchar_array_reset(wchar_array a) { return array_reset((struct array *)a); } size_t wchar_array_length(wchar_array a) { return array_length((struct array *)a); } wchar_t *wchar_array_data(wchar_array a) { return array_data((struct array *)a); }
const char CANON_IDENT_f87f79c79a17e8f37edf282bfd650956[] = "CANON_IDENT_/moa/sc1/jkodumal/work/banshee/experiments/cqual/src/array.c";
