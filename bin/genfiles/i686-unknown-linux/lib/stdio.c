...
_tagged_arr name= Cstring_to_string( tmpnam( 0)); return( unsigned int) s.curr?
Cyc_Std_strcpy( s,( struct _tagged_arr) name): name;} int Cyc_Std_fclose( struct
...
return( unsigned int) cf?({ struct Cyc_Std___sFILE* _temp1=( struct Cyc_Std___sFILE*)
_cycalloc( sizeof( struct Cyc_Std___sFILE)); _temp1->file= cf; _temp1;}): 0;}
