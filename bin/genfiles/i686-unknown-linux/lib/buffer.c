...
struct _dynforward_ptr Cyc_Buffer_contents(struct Cyc_Buffer_t*);unsigned int Cyc_Buffer_length(
...
b,1);({struct _dynforward_ptr _tmp1=_dynforward_ptr_plus(b->buffer,sizeof(char),
pos);char _tmp2=*((char*)_check_dynforward_subscript(_tmp1,sizeof(char),0));char
_tmp3=c;if(_get_dynforward_size(_tmp1,sizeof(char))== 1  && (_tmp2 == '\000'  && 
_tmp3 != '\000'))_throw_arraybounds();*((char*)_tmp1.curr)=_tmp3;});b->position=(
unsigned int)(pos + 1);return;}void Cyc_Buffer_add_substring(struct Cyc_Buffer_t*b,
struct _dynforward_ptr s,int offset,int len){if((offset < 0  || len < 0) || offset + len
> _get_dynforward_size(s,sizeof(char)))(int)_throw((void*)({struct Cyc_Core_Invalid_argument_struct*
...
_tag_dynforward(_tmp6,sizeof(char),_get_zero_arr_size_char(_tmp6,22));});_tmp5;});
_tmp4;}));{int new_position=(int)(b->position + len);if(new_position > b->length)
