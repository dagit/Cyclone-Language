...
extern int time( int* t); extern struct _tagged_arr Cyc_Time_asctime( const
struct Cyc_Time_tm* timeptr); extern struct _tagged_arr Cyc_Time_ctime( const
int* timep); extern struct Cyc_Time_tm* gmtime( const int* timep); extern struct
Cyc_Time_tm* localtime( const int* timep); extern unsigned int Cyc_Time_strftime(
struct _tagged_arr s, unsigned int maxsize, struct _tagged_arr fmt, const struct
Cyc_Time_tm* t); extern struct _tagged_arr Cyc_Time_asctime_r( const struct Cyc_Time_tm*,
struct _tagged_arr); extern struct _tagged_arr Cyc_Time_ctime_r( const int*,
struct _tagged_arr); extern int timezone; extern int daylight; extern
unsigned char* asctime( const struct Cyc_Time_tm* timeptr); extern unsigned char*
ctime( const int* timep); extern unsigned int strftime( unsigned char* s,
unsigned int maxsize, unsigned char* fmt, const struct Cyc_Time_tm* t); extern
unsigned char* asctime_r( const struct Cyc_Time_tm*, unsigned char*); extern
unsigned char* ctime_r( const int*, unsigned char*); struct _tagged_arr Cyc_Time_asctime(
const struct Cyc_Time_tm* timeptr){ return Cstring_to_string( asctime( timeptr));}
struct _tagged_arr Cyc_Time_ctime( const int* timep){ return Cstring_to_string(
ctime( timep));} unsigned int Cyc_Time_strftime( struct _tagged_arr s,
unsigned int maxsize, struct _tagged_arr fmt, const struct Cyc_Time_tm* t){
unsigned int m= _get_arr_size( s, sizeof( unsigned char)) <  maxsize?
_get_arr_size( s, sizeof( unsigned char)): maxsize; return strftime(
underlying_Cstring( s), m, underlying_Cstring( fmt), t);} struct _tagged_arr Cyc_Time_asctime_r(
const struct Cyc_Time_tm* t, struct _tagged_arr s){ if( _get_arr_size( s,
