...
Cyc_Hashtable_iter_c( void(* f)( void*, void*, void*), struct Cyc_Hashtable_Table*
t, void* env){ struct _tagged_arr odata= t->tab; int osize=( int) _get_arr_size(
odata, sizeof( struct Cyc_List_List*)); int i= 0; for( 0; i <  osize; i ++){
struct Cyc_List_List* iter=*(( struct Cyc_List_List**) _check_unknown_subscript(
odata, sizeof( struct Cyc_List_List*), i)); for( 0; iter !=  0; iter=(( struct
Cyc_List_List*) _check_null( iter))->tl){ f(((( struct _tuple0*)(( struct Cyc_List_List*)
_check_null( iter))->hd)[ _check_known_subscript_notnull( 1u, 0)]).f1,((( struct
...
_check_null( iter))->tl){({ void* _temp15[ 0u]={}; Cyc_Std_printf( _tag_arr("(",
sizeof( unsigned char), 2u), _tag_arr( _temp15, sizeof( void*), 0u));}); prn_key((((
struct _tuple0*)(( struct Cyc_List_List*) _check_null( iter))->hd)[
