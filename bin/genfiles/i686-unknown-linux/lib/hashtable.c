...
prn_key)( void*), void(* prn_val)( void*)); struct Cyc_Cstdio___sFILE; struct
...
tab.curr)[ bucket]=({ struct Cyc_List_List* _temp5=( struct Cyc_List_List*)
_cycalloc( sizeof( struct Cyc_List_List)); _temp5->hd=( void*)({ struct _tuple0*
_temp6=( struct _tuple0*) _cycalloc( sizeof( struct _tuple0)); _temp6->f1= key;
_temp6->f2= val; _temp6;}); _temp5->tl=(( struct Cyc_List_List**) tab.curr)[
bucket]; _temp5;}); if((( int(*)( struct Cyc_List_List* x)) Cyc_List_length)(((
struct Cyc_List_List**) tab.curr)[ bucket]) >  t->max_len){ Cyc_Hashtable_resize(
...
tab.curr)[ bucket]; if( l ==  0){ return;} if((* cmp)( key,((( struct _tuple0*)
l->hd)[ _check_known_subscript_notnull( 1u, 0)]).f1) ==  0){(( struct Cyc_List_List**)
tab.curr)[ bucket]= l->tl; return;}{ struct Cyc_List_List* prev= l; l= l->tl;
for( 0; l->tl !=  0;( prev= l, l= l->tl)){ if((* cmp)( key,((( struct _tuple0*)
l->hd)[ _check_known_subscript_notnull( 1u, 0)]).f1) ==  0){ prev->tl= l->tl;
return;}}}} int Cyc_Hashtable_hash_string( struct _tagged_arr s){ int ans= 0;
int sz=( int) _get_arr_size( s, sizeof( unsigned char)); int shift= 0;{ int i= 0;
for( 0; i <  sz; ++ i){ ans= ans ^ (( const unsigned char*) s.curr)[ i] << 
...
struct _tagged_arr tab, int(* hash)( void*), struct Cyc_List_List* elems){ if(
elems ==  0){ return;} Cyc_Hashtable_insert_bucket( tab, hash, elems->tl);{ void*
key=((( struct _tuple0*) elems->hd)[ _check_known_subscript_notnull( 1u, 0)]).f1;
void* val=((( struct _tuple0*) elems->hd)[ _check_known_subscript_notnull( 1u, 0)]).f2;
...
_temp7=( struct Cyc_List_List*) _cycalloc( sizeof( struct Cyc_List_List));
_temp7->hd=( void*)(( struct _tuple0*)({ struct _tuple0* _temp8=( struct _tuple0*)
_cycalloc( sizeof( struct _tuple0)); _temp8->f1= key; _temp8->f2= val; _temp8;}));
_temp7->tl=(( struct Cyc_List_List**) tab.curr)[ nidx]; _temp7;});}} void Cyc_Hashtable_resize(
struct Cyc_Hashtable_Table* t){ struct _tagged_arr odata= t->tab; int osize=(
int) _get_arr_size( odata, sizeof( struct Cyc_List_List*)); int nsize= 2 * 
osize +  1; struct Cyc_List_List* mt= 0; struct _tagged_arr ndata=({
unsigned int _temp9=( unsigned int) nsize; struct Cyc_List_List** _temp10=(
struct Cyc_List_List**) _cycalloc( _check_times( sizeof( struct Cyc_List_List*),
...
void(* f)( void*, void*), struct Cyc_Hashtable_Table* t){ struct _tagged_arr
...
odata.curr)[ i]; for( 0; iter !=  0; iter= iter->tl){ f(((( struct _tuple0*)
...
iter=(( struct Cyc_List_List**) odata.curr)[ i]; for( 0; iter !=  0; iter= iter->tl){
f(((( struct _tuple0*) iter->hd)[ _check_known_subscript_notnull( 1u, 0)]).f1,(((
struct _tuple0*) iter->hd)[ _check_known_subscript_notnull( 1u, 0)]).f2, env);}}}
void Cyc_Hashtable_print_table_map( struct Cyc_Hashtable_Table* t, void(*
prn_key)( void*), void(* prn_val)( void*)){ struct _tagged_arr odata= t->tab;
int osize=( int) _get_arr_size( odata, sizeof( struct Cyc_List_List*)); int i= 0;
for( 0; i <  osize; i ++){({ struct Cyc_Std_Int_pa_struct _temp14; _temp14.tag=
Cyc_Std_Int_pa; _temp14.f1=( int)(( unsigned int) i);{ void* _temp13[ 1u]={&
_temp14}; Cyc_Std_printf( _tag_arr("%d: ", sizeof( unsigned char), 5u), _tag_arr(
_temp13, sizeof( void*), 1u));}});{ struct Cyc_List_List* iter=(( struct Cyc_List_List**)
odata.curr)[ i]; for( 0; iter !=  0; iter= iter->tl){({ void* _temp15[ 0u]={};
