...
struct Cyc_Std___cycFILE;extern struct Cyc_Std___cycFILE*Cyc_Std_stdout;int Cyc_Std_putc(
...
struct _tagged_arr fmt,struct _tagged_arr);int Cyc_Std_sprintf(struct _tagged_arr s,
struct _tagged_arr fmt,struct _tagged_arr);int Cyc_Std_snprintf(struct _tagged_arr s,
unsigned int n,struct _tagged_arr fmt,struct _tagged_arr ap);struct _tagged_arr Cyc_Std_aprintf(
struct _tagged_arr fmt,struct _tagged_arr);struct _tagged_arr Cyc_Std_rprintf(struct
...
_tagged_arr fmt0,struct _tagged_arr ap){register struct _tagged_arr fmt;register int ch;
register int n;register struct _tagged_arr cp=_tag_arr(0,0,0);register struct
_tagged_arr cp2=(struct _tagged_arr)_tag_arr(0,0,0);int use_cp2;struct _tagged_arr
fmark;register int flags;int ret;int width;int prec;unsigned char sign;int softsign=0;
double _double;int fpprec;unsigned int _ulong;int dprec;int dpad;int fieldsz;int size=0;
...
sizeof(unsigned char));for(n=0;(n < fmt_sz?(ch=(int)((const unsigned char*)fmt.curr)[
n])!= '\000': 0)?ch != '%': 0;n ++){;}fmt=_tagged_arr_plus(fmt,sizeof(unsigned char),
n);if((n=(fmt.curr - fmark.curr)/ sizeof(unsigned char))!= 0){do{if(Cyc_Std__IO_sputn(
ioputc,ioputc_env,fmark,n)!= n)goto error;}while(0);ret +=n;}if(ch == '\000')goto
done;_tagged_arr_inplace_plus_post(& fmt,sizeof(unsigned char),1);flags=0;dprec=0;
...
1),sizeof(unsigned char),0));reswitch: use_cp2=0;switch(ch){case ' ': _LL33: if(!((
int)sign))sign=' ';goto rflag;case '#': _LL34: flags |=8;goto rflag;case '*': _LL35:
width=Cyc_Std_va_arg_int(ap);_tagged_arr_inplace_plus_post(& ap,sizeof(void*),1);
if(width >= 0)goto rflag;width=- width;goto _LL36;case '-': _LL36: flags |=16;flags &=~ 32;
goto rflag;case '+': _LL37: sign='+';goto rflag;case '.': _LL38: if((ch=(int)*((const
...
register struct _tagged_arr p;register struct _tagged_arr t;register double fract;int
...
number,& integer);t=_tagged_arr_inplace_plus(& startp,sizeof(unsigned char),1);
for(p=_tagged_arr_plus(endp,sizeof(unsigned char),- 1);p.curr >= startp.curr?
integer != 0.0: 0;++ expcnt){tmp=modf(integer / 10,& integer);*((unsigned char*)
_check_unknown_subscript(_tagged_arr_inplace_plus_post(& p,sizeof(unsigned char),
-1),sizeof(unsigned char),0))=(unsigned char)((int)((tmp + .01)* 10)+ '0');}
switch(fmtch){case 'f': _LL5F: goto _LL60;case 'F': _LL60: if(expcnt)for(0;(
...
_check_unknown_subscript(p,sizeof(unsigned char),0))){;}else{*((unsigned char*)
...
_check_unknown_subscript(_tagged_arr_inplace_plus_post(& t,sizeof(unsigned char),
1),sizeof(unsigned char),0))='.';if(fract != 0.0){if(prec)do{fract=modf(fract * 10,&
tmp);*((unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& t,
sizeof(unsigned char),1),sizeof(unsigned char),0))=(unsigned char)((int)tmp + '0');}while(
-- prec?fract != 0.0: 0);if(fract != 0.0)startp=Cyc_Std_round(fract,0,startp,
_tagged_arr_plus(t,sizeof(unsigned char),- 1),(unsigned char)0,signp);}for(0;prec
--;*((unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& t,
sizeof(unsigned char),1),sizeof(unsigned char),0))='0'){;}break;case 'e': _LL61:
goto _LL62;case 'E': _LL62: eformat: if(expcnt){*((unsigned char*)
_check_unknown_subscript(_tagged_arr_inplace_plus_post(& t,sizeof(unsigned char),
1),sizeof(unsigned char),0))=*((unsigned char*)_check_unknown_subscript(
_tagged_arr_inplace_plus(& p,sizeof(unsigned char),1),sizeof(unsigned char),0));
...
0))='.';for(0;prec?(_tagged_arr_inplace_plus(& p,sizeof(unsigned char),1)).curr < 
endp.curr: 0;-- prec){*((unsigned char*)_check_unknown_subscript(
_tagged_arr_inplace_plus_post(& t,sizeof(unsigned char),1),sizeof(unsigned char),
0))=*((unsigned char*)_check_unknown_subscript(p,sizeof(unsigned char),0));}if(!
prec?(_tagged_arr_inplace_plus(& p,sizeof(unsigned char),1)).curr < endp.curr: 0){
fract=(double)0;startp=Cyc_Std_round((double)0,(int*)& expcnt,startp,
_tagged_arr_plus(t,sizeof(unsigned char),- 1),*((unsigned char*)
_check_unknown_subscript(p,sizeof(unsigned char),0)),signp);}-- expcnt;}else{if(
fract != 0.0){for(expcnt=- 1;1;-- expcnt){fract=modf(fract * 10,& tmp);if(tmp != 0.0)
break;}*((unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(&
t,sizeof(unsigned char),1),sizeof(unsigned char),0))=(unsigned char)((int)tmp + '0');
if(prec?1: flags & 8)*((unsigned char*)_check_unknown_subscript(
_tagged_arr_inplace_plus_post(& t,sizeof(unsigned char),1),sizeof(unsigned char),
0))='.';}else{*((unsigned char*)_check_unknown_subscript(
_tagged_arr_inplace_plus_post(& t,sizeof(unsigned char),1),sizeof(unsigned char),
0))='0';if(prec?1: flags & 8)*((unsigned char*)_check_unknown_subscript(
_tagged_arr_inplace_plus_post(& t,sizeof(unsigned char),1),sizeof(unsigned char),
0))='.';}}if(fract != 0.0){if(prec)do{fract=modf(fract * 10,& tmp);*((unsigned char*)
_check_unknown_subscript(_tagged_arr_inplace_plus_post(& t,sizeof(unsigned char),
1),sizeof(unsigned char),0))=(unsigned char)((int)tmp + '0');}while(-- prec?fract
!= 0.0: 0);if(fract != 0.0)startp=Cyc_Std_round(fract,(int*)& expcnt,startp,
_tagged_arr_plus(t,sizeof(unsigned char),- 1),(unsigned char)0,signp);}for(0;prec
--;*((unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& t,
sizeof(unsigned char),1),sizeof(unsigned char),0))='0'){;}if(gformat?!(flags & 8):
0){while(t.curr > startp.curr?*((unsigned char*)_check_unknown_subscript(
...
0))== '.')_tagged_arr_inplace_plus(& t,sizeof(unsigned char),-1);
_tagged_arr_inplace_plus(& t,sizeof(unsigned char),1);}t=Cyc_Std_exponent(t,
...
1;goto eformat;}if(expcnt)for(0;(_tagged_arr_inplace_plus(& p,sizeof(unsigned char),
1)).curr < endp.curr;(*((unsigned char*)_check_unknown_subscript(
_tagged_arr_inplace_plus_post(& t,sizeof(unsigned char),1),sizeof(unsigned char),
0))=*((unsigned char*)_check_unknown_subscript(p,sizeof(unsigned char),0)),--
prec)){;}else{*((unsigned char*)_check_unknown_subscript(
_tagged_arr_inplace_plus_post(& t,sizeof(unsigned char),1),sizeof(unsigned char),
0))='0';}if(prec?1: flags & 8){dotrim=1;*((unsigned char*)_check_unknown_subscript(
_tagged_arr_inplace_plus_post(& t,sizeof(unsigned char),1),sizeof(unsigned char),
0))='.';}else{dotrim=0;}if(fract != 0.0){if(prec){do{fract=modf(fract * 10,& tmp);*((
unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& t,sizeof(
unsigned char),1),sizeof(unsigned char),0))=(unsigned char)((int)tmp + '0');}while(
tmp == 0.0?!expcnt: 0);while(-- prec?fract != 0.0: 0){fract=modf(fract * 10,& tmp);*((
unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& t,sizeof(
...
Cyc_Std_putc_string,& _tmp1D,fmt,ap);if(0 <= ans)*((unsigned char*)
_check_unknown_subscript(s,sizeof(unsigned char),ans))='\000';return ans;}int Cyc_Std_snprintf(
