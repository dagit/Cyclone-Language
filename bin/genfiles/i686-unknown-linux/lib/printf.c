...
struct _tagged_arr fmt,struct _tagged_arr);extern int Cyc_Std_sprintf(struct
...
static const int Cyc_Std_ShortPtr_sa=0;struct Cyc_Std_ShortPtr_sa_struct{int tag;
...
extern unsigned int Cyc_Std_strlen(struct _tagged_arr s);extern int isalnum(int);
extern int isalpha(int);extern int isascii(int);extern int iscntrl(int);extern int
isdigit(int);extern int isgraph(int);extern int islower(int);extern int isprint(int);
extern int ispunct(int);extern int isspace(int);extern int isupper(int);extern int
isxdigit(int);extern int toascii(int);extern int tolower(int);extern int toupper(int);
extern int _tolower(int);extern int _toupper(int);extern unsigned char Cyc_Array_Array_mismatch[
...
grantpt(int __fd);extern int unlockpt(int __fd);extern double acos(double);extern float
acosf(float);extern double acosh(double);extern float acoshf(float);extern double asin(
double);extern float asinf(float);extern double asinh(double);extern float asinhf(
float);extern double atan(double);extern double atan2(double,double);extern float
atan2f(float,float);extern float atanf(float);extern double atanh(double);extern
float atanhf(float);extern double cbrt(double);extern float cbrtf(float);extern double
ceil(double);extern float ceilf(float);extern double copysign(double,double);extern
float copysignf(float,float);extern double cos(double);extern float cosf(float);
extern double cosh(double);extern float coshf(float);extern double drem(double,double);
extern float dremf(float,float);extern double erf(double);extern double erfc(double);
extern float erfcf(float);extern float erff(float);extern double exp(double);extern
float expf(float);extern double expm1(double);extern float expm1f(float);extern double
fabs(double);extern float fabsf(float);extern int finite(double);extern int finitef(
float);extern double floor(double);extern float floorf(float);extern double fmod(
double,double);extern float fmodf(float,float);extern double frexp(double,int*);
extern float frexpf(float,int*);extern double gamma(double);extern float gammaf(float);
extern double hypot(double,double);extern float hypotf(float,float);extern int ilogb(
double);extern int ilogbf(float);extern int isinf(double);extern int isinff(float);
extern int isnan(double);extern int isnanf(float);extern double j0(double);extern float
j0f(float);extern double j1(double);extern float j1f(float);extern double jn(int,
double);extern float jnf(int,float);extern double ldexp(double,int);extern float
ldexpf(float,int);extern double lgamma(double);extern float lgammaf(float);extern
double log(double);extern float logf(float);extern double log10(double);extern float
log10f(float);extern double log1p(double);extern float log1pf(float);extern double
modf(double,double*);extern float modff(float,float*);extern double nextafter(double,
double);extern float nextafterf(float,float);extern double pow(double,double);extern
float powf(float,float);extern double remainder(double,double);extern float
remainderf(float,float);extern double rint(double);extern float rintf(float);extern
double scalbn(double,int);extern float scalbnf(float,int);extern double sin(double);
extern float sinf(float);extern double sinh(double);extern float sinhf(float);extern
double sqrt(double);extern float sqrtf(float);extern double tan(double);extern float
tanf(float);extern double tanh(double);extern float tanhf(float);extern double y0(
double);extern float y0f(float);extern double y1(double);extern float y1f(float);
extern double yn(int,double);extern float ynf(int,float);static struct _tagged_arr Cyc_Std_parg2string(
void*x){void*_tmp0=x;_LL2: if(*((int*)_tmp0)== Cyc_Std_String_pa){goto _LL3;}else{
goto _LL4;}_LL4: if(*((int*)_tmp0)== Cyc_Std_Int_pa){goto _LL5;}else{goto _LL6;}_LL6:
if(*((int*)_tmp0)== Cyc_Std_Double_pa){goto _LL7;}else{goto _LL8;}_LL8: if(*((int*)
_tmp0)== Cyc_Std_ShortPtr_pa){goto _LL9;}else{goto _LL10;}_LL10: if(*((int*)_tmp0)
== Cyc_Std_IntPtr_pa){goto _LL11;}else{goto _LL1;}_LL3: return _tag_arr("string",
sizeof(unsigned char),7);_LL5: return _tag_arr("int",sizeof(unsigned char),4);_LL7:
return _tag_arr("double",sizeof(unsigned char),7);_LL9: return _tag_arr("short *",
sizeof(unsigned char),8);_LL11: return _tag_arr("unsigned long *",sizeof(
unsigned char),16);_LL1:;}static void*Cyc_Std_badarg(struct _tagged_arr s){return(
void*)_throw((void*)({struct Cyc_Core_Invalid_argument_struct*_tmpC=_cycalloc(
sizeof(struct Cyc_Core_Invalid_argument_struct));_tmpC[0]=({struct Cyc_Core_Invalid_argument_struct
_tmpD;_tmpD.tag=Cyc_Core_Invalid_argument;_tmpD.f1=s;_tmpD;});_tmpC;}));}static
int Cyc_Std_va_arg_int(struct _tagged_arr ap){void*_tmpE=*((void**)
_check_unknown_subscript(ap,sizeof(void*),0));unsigned int _tmp14;_LL16: if(*((int*)
_tmpE)== Cyc_Std_Int_pa){_LL21: _tmp14=((struct Cyc_Std_Int_pa_struct*)_tmpE)->f1;
goto _LL17;}else{goto _LL18;}_LL18: goto _LL19;_LL17: return(int)_tmp14;_LL19: return((
int(*)(struct _tagged_arr s))Cyc_Std_badarg)(_tag_arr("printf expected int",
sizeof(unsigned char),20));_LL15:;}static int Cyc_Std_va_arg_long(struct
_tagged_arr ap){void*_tmp16=*((void**)_check_unknown_subscript(ap,sizeof(void*),0));
unsigned int _tmp1C;_LL24: if(*((int*)_tmp16)== Cyc_Std_Int_pa){_LL29: _tmp1C=((
struct Cyc_Std_Int_pa_struct*)_tmp16)->f1;goto _LL25;}else{goto _LL26;}_LL26: goto
_LL27;_LL25: return(int)_tmp1C;_LL27: return((int(*)(struct _tagged_arr s))Cyc_Std_badarg)(
_tag_arr("printf expected int",sizeof(unsigned char),20));_LL23:;}static
unsigned int Cyc_Std_va_arg_ulong(struct _tagged_arr ap){void*_tmp1E=*((void**)
_check_unknown_subscript(ap,sizeof(void*),0));unsigned int _tmp24;_LL32: if(*((int*)
_tmp1E)== Cyc_Std_Int_pa){_LL37: _tmp24=((struct Cyc_Std_Int_pa_struct*)_tmp1E)->f1;
goto _LL33;}else{goto _LL34;}_LL34: goto _LL35;_LL33: return(unsigned int)_tmp24;
_LL35: return((unsigned int(*)(struct _tagged_arr s))Cyc_Std_badarg)(_tag_arr("printf expected int",
sizeof(unsigned char),20));_LL31:;}static unsigned int Cyc_Std_va_arg_uint(struct
_tagged_arr ap){void*_tmp26=*((void**)_check_unknown_subscript(ap,sizeof(void*),0));
unsigned int _tmp2C;_LL40: if(*((int*)_tmp26)== Cyc_Std_Int_pa){_LL45: _tmp2C=((
struct Cyc_Std_Int_pa_struct*)_tmp26)->f1;goto _LL41;}else{goto _LL42;}_LL42: goto
_LL43;_LL41: return(unsigned int)_tmp2C;_LL43: return((unsigned int(*)(struct
_tagged_arr s))Cyc_Std_badarg)(_tag_arr("printf expected int",sizeof(
unsigned char),20));_LL39:;}static double Cyc_Std_va_arg_double(struct _tagged_arr
ap){void*_tmp2E=*((void**)_check_unknown_subscript(ap,sizeof(void*),0));double
_tmp34;_LL48: if(*((int*)_tmp2E)== Cyc_Std_Double_pa){_LL53: _tmp34=((struct Cyc_Std_Double_pa_struct*)
_tmp2E)->f1;goto _LL49;}else{goto _LL50;}_LL50: goto _LL51;_LL49: return _tmp34;_LL51:(
int)_throw(Cyc_Std_badarg((struct _tagged_arr)({struct Cyc_Std_String_pa_struct
_tmp37;_tmp37.tag=Cyc_Std_String_pa;_tmp37.f1=(struct _tagged_arr)Cyc_Std_parg2string(*((
void**)_check_unknown_subscript(ap,sizeof(void*),0)));{void*_tmp36[1]={& _tmp37};
Cyc_Std_aprintf(_tag_arr("printf expected double but found %s",sizeof(
unsigned char),36),_tag_arr(_tmp36,sizeof(void*),1));}})));_LL47:;}static short*
Cyc_Std_va_arg_short_ptr(struct _tagged_arr ap){void*_tmp38=*((void**)
_check_unknown_subscript(ap,sizeof(void*),0));short*_tmp3E;_LL58: if(*((int*)
_tmp38)== Cyc_Std_ShortPtr_pa){_LL63: _tmp3E=((struct Cyc_Std_ShortPtr_pa_struct*)
_tmp38)->f1;goto _LL59;}else{goto _LL60;}_LL60: goto _LL61;_LL59: return _tmp3E;_LL61:(
int)_throw(Cyc_Std_badarg(_tag_arr("printf expected short pointer",sizeof(
unsigned char),30)));_LL57:;}static unsigned int*Cyc_Std_va_arg_int_ptr(struct
_tagged_arr ap){void*_tmp40=*((void**)_check_unknown_subscript(ap,sizeof(void*),0));
unsigned int*_tmp46;_LL66: if(*((int*)_tmp40)== Cyc_Std_IntPtr_pa){_LL71: _tmp46=((
struct Cyc_Std_IntPtr_pa_struct*)_tmp40)->f1;goto _LL67;}else{goto _LL68;}_LL68:
goto _LL69;_LL67: return _tmp46;_LL69:(int)_throw(Cyc_Std_badarg(_tag_arr("printf expected int pointer",
sizeof(unsigned char),28)));_LL65:;}static struct _tagged_arr Cyc_Std_va_arg_string(
struct _tagged_arr ap){void*_tmp48=*((void**)_check_unknown_subscript(ap,sizeof(
void*),0));struct _tagged_arr _tmp4E;_LL74: if(*((int*)_tmp48)== Cyc_Std_String_pa){
_LL79: _tmp4E=((struct Cyc_Std_String_pa_struct*)_tmp48)->f1;goto _LL75;}else{goto
_LL76;}_LL76: goto _LL77;_LL75: return _tmp4E;_LL77:(int)_throw(Cyc_Std_badarg(
_tag_arr("printf expected string",sizeof(unsigned char),23)));_LL73:;}int Cyc_Std___cvt_double(
...
_tagged_arr s,int howmany){int i=0;while(i < howmany){if(_get_arr_size(s,sizeof(
unsigned char))== 0? 1: ioputc((int)*((const unsigned char*)
_check_unknown_subscript(s,sizeof(unsigned char),0)),ioputc_env)== - 1){return i;}
_tagged_arr_inplace_plus_post(& s,sizeof(unsigned char),1);i ++;}return i;}static
int Cyc_Std__IO_padn(int(*ioputc)(int,void*),void*ioputc_env,unsigned char c,int
howmany){int i=0;while(i < howmany){if(ioputc((int)c,ioputc_env)== - 1){return i;}i
++;}return i;}static struct _tagged_arr Cyc_Std_my_memchr(struct _tagged_arr s,
unsigned char c,int n){int sz=(int)_get_arr_size(s,sizeof(unsigned char));n=n < sz? n:
sz;for(0;n != 0;(n --,_tagged_arr_inplace_plus_post(& s,sizeof(unsigned char),1))){
if(*((const unsigned char*)_check_unknown_subscript(s,sizeof(unsigned char),0))== 
c){return s;}}return(struct _tagged_arr)_tag_arr(0,0,0);}int Cyc_Std__IO_vfprintf(
int(*ioputc)(int,void*),void*ioputc_env,struct _tagged_arr fmt0,struct _tagged_arr
ap){struct _tagged_arr fmt;int ch;int n;struct _tagged_arr cp=_tag_arr(0,0,0);struct
_tagged_arr cp2=(struct _tagged_arr)_tag_arr(0,0,0);int use_cp2;struct _tagged_arr
fmark;int flags;int ret;int width;int prec;unsigned char sign;int softsign=0;double
_double;int fpprec;unsigned int _ulong;int dprec;int dpad;int fieldsz;int size=0;
unsigned char buf[348];unsigned char ox[2]={'\000','\000'};enum Cyc_Std_BASE base;
fmt=fmt0;ret=0;for(0;1;0){for(fmark=fmt;(ch=(int)*((const unsigned char*)
_check_unknown_subscript(fmt,sizeof(unsigned char),0)))!= '\000'? ch != '%': 0;
_tagged_arr_inplace_plus_post(& fmt,sizeof(unsigned char),1)){;}if((n=(fmt.curr - 
fmark.curr)/ sizeof(unsigned char))!= 0){do {if(Cyc_Std__IO_sputn(ioputc,
ioputc_env,fmark,n)!= n){goto error;}} while (0);ret +=n;}if(ch == '\000'){goto done;}
_tagged_arr_inplace_plus_post(& fmt,sizeof(unsigned char),1);flags=0;dprec=0;
fpprec=0;width=0;prec=- 1;sign='\000';rflag: ch=(int)*((const unsigned char*)
_check_unknown_subscript(_tagged_arr_inplace_plus_post(& fmt,sizeof(unsigned char),
1),sizeof(unsigned char),0));reswitch: use_cp2=0;switch(ch){case ' ': _LL80: if(!((
int)sign)){sign=' ';}goto rflag;case '#': _LL81: flags |=8;goto rflag;case '*': _LL82:
width=Cyc_Std_va_arg_int(ap);_tagged_arr_inplace_plus_post(& ap,sizeof(void*),1);
if(width >= 0){goto rflag;}width=- width;goto _LL83;case '-': _LL83: flags |=16;flags &=~
32;goto rflag;case '+': _LL84: sign='+';goto rflag;case '.': _LL85: if((ch=(int)*((const
unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& fmt,
sizeof(unsigned char),1),sizeof(unsigned char),0)))== '*'){n=Cyc_Std_va_arg_int(
ap);_tagged_arr_inplace_plus_post(& ap,sizeof(void*),1);prec=n < 0? - 1: n;goto rflag;}
n=0;while((unsigned int)(ch - '0')<= 9){n=10 * n + (ch - '0');ch=(int)*((const
unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& fmt,
sizeof(unsigned char),1),sizeof(unsigned char),0));}prec=n < 0? - 1: n;goto reswitch;
case '0': _LL86: if(!(flags & 16)){flags |=32;}goto rflag;case '1': _LL87: goto _LL88;case
'2': _LL88: goto _LL89;case '3': _LL89: goto _LL90;case '4': _LL90: goto _LL91;case '5':
_LL91: goto _LL92;case '6': _LL92: goto _LL93;case '7': _LL93: goto _LL94;case '8': _LL94:
goto _LL95;case '9': _LL95: n=0;do {n=10 * n + (ch - '0');ch=(int)*((const unsigned char*)
_check_unknown_subscript(_tagged_arr_inplace_plus_post(& fmt,sizeof(unsigned char),
1),sizeof(unsigned char),0));} while ((unsigned int)(ch - '0')<= 9);width=n;goto
reswitch;case 'L': _LL96: flags |=2;goto rflag;case 'h': _LL97: flags |=4;goto rflag;case 'l':
_LL98: flags |=1;goto rflag;case 'c': _LL99: cp=_tag_arr(buf,sizeof(unsigned char),348);*((
...
void*),1);size=1;sign='\000';break;case 'D': _LL100: flags |=1;goto _LL101;case 'd':
_LL101: goto _LL102;case 'i': _LL102: _ulong=(unsigned int)(flags & 1? Cyc_Std_va_arg_long(
...
_ulong;sign='-';}base=Cyc_Std_DEC;goto number;case 'e': _LL103: goto _LL104;case 'E':
_LL104: goto _LL105;case 'f': _LL105: goto _LL106;case 'F': _LL106: goto _LL107;case 'g':
_LL107: goto _LL108;case 'G': _LL108: _double=Cyc_Std_va_arg_double(ap);
...
_tagged_arr_inplace_plus_post(& cp,sizeof(unsigned char),1);}break;case 'n': _LL109:
...
case 'O': _LL110: flags |=1;goto _LL111;case 'o': _LL111: _ulong=flags & 1? Cyc_Std_va_arg_ulong(
...
void*),1);base=Cyc_Std_OCT;goto nosign;case 'p': _LL112: _ulong=(unsigned int)Cyc_Std_va_arg_long(
ap);_tagged_arr_inplace_plus_post(& ap,sizeof(void*),1);base=Cyc_Std_HEX;flags |=
64;ch=(int)'x';goto nosign;case 's': _LL113: use_cp2=1;cp2=Cyc_Std_va_arg_string(ap);
...
cp2);}sign='\000';break;case 'U': _LL114: flags |=1;goto _LL115;case 'u': _LL115: _ulong=
...
sizeof(void*),1);base=Cyc_Std_DEC;goto nosign;case 'X': _LL116: goto _LL117;case 'x':
_LL117: _ulong=flags & 1? Cyc_Std_va_arg_ulong(ap):(flags & 4?(unsigned int)((
...
switch(base){case Cyc_Std_OCT: _LL119: do {*((unsigned char*)
...
break;case Cyc_Std_DEC: _LL120: while(_ulong >= 10){*((unsigned char*)
_check_unknown_subscript(_tagged_arr_inplace_plus(& cp,sizeof(unsigned char),-1),
...
case Cyc_Std_HEX: _LL121: if(ch == 'X'){xdigs=_tag_arr("0123456789ABCDEF",sizeof(
...
unsigned char);skipsize: break;default: _LL118: if(ch == '\000'){goto done;}cp=
_tag_arr(buf,sizeof(unsigned char),348);*((unsigned char*)
...
fieldsz? width: fieldsz;}done: return ret;error: return - 1;}static struct _tagged_arr Cyc_Std_exponent(
struct _tagged_arr p,int exp,int fmtch){struct _tagged_arr t;unsigned char expbuf[308];*((
unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& p,sizeof(
unsigned char),1),sizeof(unsigned char),0))=(unsigned char)fmtch;if(exp < 0){exp=
- exp;*((unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& p,
...
_tagged_arr_inplace_plus_post(& p,sizeof(unsigned char),-1),sizeof(unsigned char),
0))=(unsigned char)((int)((tmp + .01)* 10)+ '0');}switch(fmtch){case 'f': _LL124:
goto _LL125;case 'F': _LL125: if(expcnt){for(0;(_tagged_arr_inplace_plus(& p,sizeof(
...
1),sizeof(unsigned char),0))='0'){;}break;case 'e': _LL126: goto _LL127;case 'E':
...
_LL128: goto _LL129;case 'G': _LL129: if(! prec){++ prec;}if(expcnt > prec? 1:((! expcnt?
...
default: _LL130:(int)_throw((void*)({struct Cyc_Core_Impossible_struct*_tmp84=
_cycalloc(sizeof(struct Cyc_Core_Impossible_struct));_tmp84[0]=({struct Cyc_Core_Impossible_struct
_tmp85;_tmp85.tag=Cyc_Core_Impossible;_tmp85.f1=_tag_arr("Std::__cvt_double",
sizeof(unsigned char),18);_tmp85;});_tmp84;}));}return(t.curr - startp.curr)/ 
sizeof(unsigned char);}int Cyc_Std_vfprintf(struct Cyc_Std___sFILE*f,struct
_tagged_arr fmt,struct _tagged_arr ap){int ans;ans=((int(*)(int(*ioputc)(int,struct
Cyc_Std___sFILE*),struct Cyc_Std___sFILE*ioputc_env,struct _tagged_arr fmt0,struct
_tagged_arr ap))Cyc_Std__IO_vfprintf)(Cyc_Std_putc,f,fmt,ap);return ans;}int Cyc_Std_fprintf(
struct Cyc_Std___sFILE*f,struct _tagged_arr fmt,struct _tagged_arr ap){return Cyc_Std_vfprintf(
...
_tuple0*sptr_n){struct _tuple0 _tmp88;unsigned int*_tmp89;struct _tagged_arr*_tmp8B;
struct _tuple0*_tmp86=sptr_n;_tmp88=*_tmp86;_LL140: _tmp8B=_tmp88.f1;goto _LL138;
_LL138: _tmp89=_tmp88.f2;goto _LL135;_LL135: {struct _tagged_arr s=*_tmp8B;
unsigned int n=*_tmp89;if(_get_arr_size(s,sizeof(unsigned char))== 0? 1: n == 0){
return - 1;}*((unsigned char*)_check_unknown_subscript(s,sizeof(unsigned char),0))=(
unsigned char)c;*_tmp8B=_tagged_arr_plus(s,sizeof(unsigned char),1);*_tmp89=n - 1;
return 1;}}int Cyc_Std_vsnprintf(struct _tagged_arr s,unsigned int n,struct
_tagged_arr fmt,struct _tagged_arr ap){int ans;struct _tagged_arr _tmp8D=s;
unsigned int _tmp8E=n;struct _tuple0 _tmp8F=({struct _tuple0 _tmp90;_tmp90.f1=& _tmp8D;
_tmp90.f2=& _tmp8E;_tmp90;});ans=((int(*)(int(*ioputc)(int,struct _tuple0*),struct
_tuple0*ioputc_env,struct _tagged_arr fmt0,struct _tagged_arr ap))Cyc_Std__IO_vfprintf)(
Cyc_Std_putc_string,& _tmp8F,fmt,ap);if(0 <= ans){((unsigned char*)s.curr)[ans]='\000';}
...
Cyc_Std_putc_void,0,fmt,ap)+ 1;struct _tagged_arr s=({unsigned int _tmp91=(
unsigned int)size;unsigned char*_tmp92=(unsigned char*)_region_malloc(r1,
_check_times(sizeof(unsigned char),_tmp91));struct _tagged_arr _tmp94=_tag_arr(
_tmp92,sizeof(unsigned char),(unsigned int)size);{unsigned int _tmp93=_tmp91;
unsigned int i;for(i=0;i < _tmp93;i ++){_tmp92[i]='\000';}};_tmp94;});Cyc_Std_vsprintf(
