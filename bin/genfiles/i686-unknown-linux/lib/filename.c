...
while(i >= 0?*((const unsigned char*)_check_unknown_subscript(filename,sizeof(
unsigned char),i))!= '.': 0){-- i;}if(i < 0)(int)_throw((void*)({struct Cyc_Core_Invalid_argument_struct*
...
int i=(int)(_get_arr_size(filename,sizeof(unsigned char))- 1);while(i >= 0?*((
const unsigned char*)_check_unknown_subscript(filename,sizeof(unsigned char),i))
!= '/': 0){-- i;}if(i < 0)return Cyc_Core_new_string(0);return Cyc_Std_substring(
filename,0,(unsigned int)i);}struct _tagged_arr Cyc_Filename_basename(struct
_tagged_arr filename){int i=(int)(_get_arr_size(filename,sizeof(unsigned char))- 1);
while(i >= 0?*((const unsigned char*)_check_unknown_subscript(filename,sizeof(
unsigned char),i))!= '/': 0){-- i;}return Cyc_Std_substring(filename,i + 1,
_get_arr_size(filename,sizeof(unsigned char))- (i + 1));}int Cyc_Filename_check_suffix(
