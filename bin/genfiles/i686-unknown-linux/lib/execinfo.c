...
pid, int* status, int options); void(* Cyc_Signal_signal( int sig, void(* func)(
int)))( int); extern int raise( int sig); extern int kill( int, int); struct Cyc_timeval{
int tv_sec; int tv_usec; } ; struct Cyc_Getopt_option{ struct _tagged_arr name;
int has_arg; int* flag; int val; } ; extern unsigned int alarm( unsigned int
seconds); extern int close( int); extern int getpid(); extern int getppid();
extern int fork(); extern int fchdir( int); extern int dup( int); extern int
dup2( int, int); extern unsigned int getuid(); extern int setuid( unsigned int
...
unsigned int getegid(); extern int setegid( unsigned int egid); extern int pipe(
int* filedes); extern int lseek( int filedes, int offset, int whence); int Cyc_Unistd_chdir(
struct _tagged_arr); struct _tagged_arr Cyc_Unistd_getcwd( struct _tagged_arr
buf, unsigned int size); int Cyc_Unistd_execl( struct _tagged_arr path, struct
_tagged_arr arg0, struct _tagged_arr argv); int Cyc_Unistd_execlp( struct
...
int Cyc_Unistd_read( int fd, struct _tagged_arr buf, unsigned int count); int
Cyc_Unistd_write( int fd, struct _tagged_arr buf, unsigned int count); int Cyc_Unistd_unlink(
struct _tagged_arr pathname); extern int backtrace( int*, int); int Cyc_Execinfo_backtrace(
...
_tag_arr( 0u, 0u, 0u); _temp2[ 2u]=( struct _tagged_arr)({ struct Cyc_Stdio_Int_pa_struct
_temp4; _temp4.tag= Cyc_Stdio_Int_pa; _temp4.f1=( int)(( unsigned int) self_pid);{
void* _temp3[ 1u]={& _temp4}; Cyc_Stdio_aprintf( _tag_arr("/proc/%d/exe",
sizeof( unsigned char), 13u), _tag_arr( _temp3, sizeof( void*), 1u));}}); _temp2[
1u]= _tag_arr("-e", sizeof( unsigned char), 3u); _temp2[ 0u]= _tag_arr("--functions",
