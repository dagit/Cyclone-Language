...
unsigned char** h_aliases; int h_addrtype; int h_length; struct Cyc_Std_in_addr**
...
ntCsl_to_ntsl( src->h_aliases); _temp1->h_addrtype= src->h_addrtype; _temp1->h_length=
src->h_length; _temp1->h_addr_list= pntlp_toCyc( src->h_addr_list); _temp1;}): 0;}
struct Cyc_Std_protoent* Cyc_Std_getprotobyname( struct _tagged_arr name){
struct Cyc_Cnetdb_Cprotoent* src= getprotobyname( string_to_Cstring( name));
return( unsigned int) src?({ struct Cyc_Std_protoent* _temp2=( struct Cyc_Std_protoent*)
_cycalloc( sizeof( struct Cyc_Std_protoent)); _temp2->p_name= Cstring_to_string(
src->p_name); _temp2->p_aliases= ntCsl_to_ntsl( src->p_aliases); _temp2->p_proto=
src->p_proto; _temp2;}): 0;} void Cyc_Std_herror( struct _tagged_arr s){ herror(
string_to_Cstring( s));}
