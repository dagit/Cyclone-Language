...
int listen( int fd, int n); extern int shutdown( int fd, int how); struct Cyc_std_in_addr{
unsigned int s_addr; } ; struct Cyc_std_sockaddr_in{ unsigned short sin_family;
...
GC_malloc( sizeof( struct Cyc_std_servent)); _temp0->s_name= Cstring_to_string(((
...
_temp1=( struct Cyc_std_hostent*) GC_malloc( sizeof( struct Cyc_std_hostent));
