...
i;for(i=howmuch - 1;i >= 0?!isspace((int)*((const unsigned char*)
_check_unknown_subscript(s,sizeof(unsigned char),i))): 0;i --){;}if(i < 0)for(i=
howmuch?howmuch - 1: 0;(i < _tmp2?(int)((const unsigned char*)s.curr)[i]: 0)?!isspace((
int)((const unsigned char*)s.curr)[i]): 0;i ++){;}Cyc_Buffer_add_substring(b,s,0,i);{
...
indent - _tmp3;unsigned char*_tmp5=(unsigned char*)_cycalloc_atomic(_check_times(
sizeof(unsigned char),_tmp4));struct _tagged_arr _tmp7=_tag_arr(_tmp5,sizeof(
unsigned char),indent - _tmp3);{unsigned int _tmp6=_tmp4;unsigned int i;for(i=0;i < 
_tmp6;i ++){_tmp5[i]=' ';}}_tmp7;});}}else{return;}Cyc_Buffer_add_string(b,
itemsep);while(desc.curr != ((struct _tagged_arr)_tag_arr(0,0,0)).curr){desc=Cyc_Arg_Justify_break_line(
