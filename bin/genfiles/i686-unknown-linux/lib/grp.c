...
unsigned int group); extern int Cyc_Std_setgroups( struct _tagged_arr groups);
struct Cyc_Cgrp_Cgroup{ unsigned char* gr_name; unsigned char* gr_passwd;
unsigned int gr_gid; unsigned char** gr_mem; } ; extern struct Cyc_Cgrp_Cgroup*
getgrnam( unsigned char* name); extern struct Cyc_Cgrp_Cgroup* getgrgid(
unsigned int gid); extern int initgroups( unsigned char* user, unsigned int
group); extern int setgroups( unsigned int n, const unsigned int* groups);
...
Cyc_Std_group* _temp0=( struct Cyc_Std_group*) _cycalloc( sizeof( struct Cyc_Std_group));
_temp0->gr_name=( struct _tagged_arr) Cstring_to_string( src->gr_name); _temp0->gr_passwd=(
struct _tagged_arr) Cstring_to_string( src->gr_passwd); _temp0->gr_gid= src->gr_gid;
_temp0->gr_mem= ntCsl_to_ntsl( src->gr_mem); _temp0;}): 0;} struct Cyc_Std_group*
Cyc_Std_getgrgid( unsigned int gid){ struct Cyc_Cgrp_Cgroup* src= getgrgid( gid);
return( unsigned int) src?({ struct Cyc_Std_group* _temp1=( struct Cyc_Std_group*)
...
user), group);} int Cyc_Std_setgroups( struct _tagged_arr groups){ return
setgroups( _get_arr_size( groups, sizeof( unsigned int)),( const unsigned int*)
_check_null( _untag_arr( groups, sizeof( unsigned int), 1u)));}
