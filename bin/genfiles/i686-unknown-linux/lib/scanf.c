...
_IO_peekc(fp);while(!isspace((int)((char)c))){n ++;_IO_getc(fp);if(-- width == 0)
break;if((c=_IO_peekc(fp))== - 1){seen_eof ++;break;}}nread +=n;}}else{struct
_tagged_arr _tmp19=Cyc_Std_va_arg_string_ptr(*((void**)_check_unknown_subscript(
ap,sizeof(void*),0)));_tagged_arr_inplace_plus_post(& ap,sizeof(void*),1);{struct
_tagged_arr _tmp1A=_tmp19;int c=_IO_peekc(fp);while(!isspace((int)((char)c))){c=
_IO_getc(fp);if(_get_arr_size(_tmp1A,sizeof(char))== 0)goto eof_failure;*((char*)
_check_unknown_subscript(_tmp1A,sizeof(char),0))=(char)c;
_tagged_arr_inplace_plus_post(& _tmp1A,sizeof(char),1);if(-- width == 0)break;if((c=
_IO_peekc(fp))== - 1){seen_eof ++;break;}}if(_get_arr_size(_tmp1A,sizeof(char))== 
0)goto eof_failure;*((char*)_check_unknown_subscript(_tmp1A,sizeof(char),0))='\000';
nread +=(_tmp1A.curr - _tmp19.curr)/ sizeof(char);nassigned ++;}}continue;case 3:
_LL44: if(width == 0?1: width > sizeof(buf)- 1)width=(long long)(sizeof(buf)- 1);
flags |=(64 | 128)| 512;for(p=_tag_arr(buf,sizeof(char),350);width != 0;width --){c=(
int)((char)_IO_peekc(fp));switch(c){case '0': _LL46: if(base == 0){base=8;flags |=256;}
...
unsigned int res;*((char*)_check_unknown_subscript(p,sizeof(char),0))='\000';if(
use_strtoul)res=Cyc_Std_strtoul(_tag_arr(buf,sizeof(char),350),0,base);else{res=(
unsigned int)Cyc_Std_strtol(_tag_arr(buf,sizeof(char),350),0,base);}if(flags & 16)*
Cyc_Std_va_arg_int_ptr(*((void**)_check_unknown_subscript(ap,sizeof(void*),0)))=(
int)res;else{if(flags & 4)*Cyc_Std_va_arg_short_ptr(*((void**)
_check_unknown_subscript(ap,sizeof(void*),0)))=(short)res;else{if(flags & 1)*Cyc_Std_va_arg_int_ptr(*((
void**)_check_unknown_subscript(ap,sizeof(void*),0)))=(int)res;else{*Cyc_Std_va_arg_int_ptr(*((
...
_tag_arr(buf,sizeof(char),350)).curr)/ sizeof(char);break;case 4: _LL45: if(width == 
0?1: width > sizeof(buf)- 1)width=(long long)(sizeof(buf)- 1);flags |=((64 | 128)| 
256)| 512;for(p=_tag_arr(buf,sizeof(char),350);width != 0;width --){c=_IO_peekc(fp);
switch(c){case '0': _LL63: goto _LL64;case '1': _LL64: goto _LL65;case '2': _LL65: goto
_LL66;case '3': _LL66: goto _LL67;case '4': _LL67: goto _LL68;case '5': _LL68: goto _LL69;
case '6': _LL69: goto _LL6A;case '7': _LL6A: goto _LL6B;case '8': _LL6B: goto _LL6C;case '9':
_LL6C: flags &=~(64 | 128);goto fok;case '+': _LL6D: goto _LL6E;case '-': _LL6E: if(flags & 
64){flags &=~ 64;goto fok;}break;case '.': _LL6F: if(flags & 256){flags &=~(64 | 256);
goto fok;}break;case 'e': _LL70: goto _LL71;case 'E': _LL71: if((flags & (128 | 512))== 
512){flags=(flags & ~(512 | 256)| 64)| 128;goto fok;}break;default: _LL72: break;}
break;fok:*((char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& p,
sizeof(char),1),sizeof(char),0))=(char)c;_IO_getc(fp);if(_IO_peekc(fp)== - 1){
seen_eof ++;break;}}if(flags & 128){if(flags & 512){while(p.curr > (_tag_arr(buf,
sizeof(char),350)).curr){_tagged_arr_inplace_plus(& p,sizeof(char),-1);_IO_ungetc((
int)*((char*)_check_unknown_subscript(p,sizeof(char),0)),fp);}goto match_failure;}
...
for(n=0;n < 256;n ++){((char*)tab.curr)[n]=(char)v;}if(c == 0)return
_tagged_arr_plus(fmt,sizeof(char),- 1);v=1 - v;for(0;1;0){((char*)tab.curr)[c]=(
...
int)'-';break;}_tagged_arr_inplace_plus_post(& fmt,sizeof(char),1);do{*((char*)
