...
_LL4D: base=(int)basefix[base];flags &=~((64 | 256)| 128);goto ok;case '8': _LL4E: goto
_LL4F;case '9': _LL4F: base=(int)basefix[base];if(base <= 8)break;flags &=~((64 | 256)
...
v;}if(c == 0)return _tagged_arr_plus(fmt,sizeof(unsigned char),- 1);v=1 - v;for(0;1;
0){((unsigned char*)tab.curr)[c]=(unsigned char)v;doswitch: n=(int)*((const
unsigned char*)_check_unknown_subscript(_tagged_arr_inplace_plus_post(& fmt,
sizeof(unsigned char),1),sizeof(unsigned char),0));switch(n){case 0: _LL75: return
_tagged_arr_plus(fmt,sizeof(unsigned char),- 1);case '-': _LL76: n=(int)*((const
...
_LL77: return fmt;default: _LL78: c=n;break;}}}static int Cyc_Std_string_getc(struct
...
== '\000')return - 1;*sptr=_tagged_arr_plus(s,sizeof(unsigned char),1);return(int)
c;}static int Cyc_Std_string_ungetc(int ignore,struct _tagged_arr*sptr){*sptr=
