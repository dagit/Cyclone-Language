...
_tagged_arr pw_passwd; unsigned int pw_uid; unsigned int pw_gid; struct
...
extern struct Cyc_Pwd_passwd* Cyc_Pwd_getpwuid( unsigned int uid); struct Cyc_Cpwd_Cpasswd{
unsigned char* pw_name; unsigned char* pw_passwd; unsigned int pw_uid;
unsigned int pw_gid; unsigned char* pw_gecos; unsigned char* pw_dir;
unsigned char* pw_shell; } ; extern struct Cyc_Cpwd_Cpasswd* getpwnam(
unsigned char* name); extern struct Cyc_Cpwd_Cpasswd* getpwuid( unsigned int uid);
struct Cyc_Pwd_passwd* Cyc_Pwd_getpwnam( struct _tagged_arr name){ struct Cyc_Cpwd_Cpasswd*
src= getpwnam( string_to_Cstring( name)); return( unsigned int) src?({ struct
Cyc_Pwd_passwd* _temp0=( struct Cyc_Pwd_passwd*) GC_malloc( sizeof( struct Cyc_Pwd_passwd));
...
_temp0;}): 0;} struct Cyc_Pwd_passwd* Cyc_Pwd_getpwuid( unsigned int uid){
