...
_temp1;}));} void Cyc_Id_id_prn( struct Cyc_std___sFILE* f, struct Cyc_Id_id* x){({
struct Cyc_std_String_pa_struct _temp7; _temp7.tag= Cyc_std_String_pa; _temp7.f1=(
struct _tagged_arr) Cyc_Id_id_to_string( x);{ void* _temp6[ 1u]={& _temp7}; Cyc_std_fprintf(
f, _tag_arr("%s", sizeof( unsigned char), 3u), _tag_arr( _temp6, sizeof( void*),
1u));}});} struct _tagged_arr Cyc_Id_id_to_source( struct Cyc_Id_id* x){ return
Cyc_std_strdup( x->s);} int Cyc_Id_id_compare( struct Cyc_Id_id* x1, struct Cyc_Id_id*
x2){ if( x1->i ==  x2->i){ return Cyc_std_zstrcmp( x1->s, x2->s);} else{ if( x1->i
<  x2->i){ return - 1;} else{ return 1;}}}
