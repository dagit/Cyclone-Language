...
extern int daylight; struct Cyc_Stat_stat_t{ unsigned long long st_dev;
unsigned int st_ino; unsigned int st_mode; unsigned int st_nlink; unsigned int
st_uid; unsigned int st_gid; unsigned long long st_rdev; int st_size;
unsigned int st_blksize; unsigned int st_blocks; int st_atime; int st_mtime; int
st_ctime; } ; extern int Cyc_Stat_open( struct _tagged_arr pathname, int flags,
struct _tagged_arr mode); extern int Cyc_Stat_stat( struct _tagged_arr filename,
struct Cyc_Stat_stat_t* buf); extern int fstat( int fd, struct Cyc_Stat_stat_t*
buf); extern int Cyc_Stat_lstat( struct _tagged_arr filename, struct Cyc_Stat_stat_t*
buf); extern unsigned int umask( unsigned int mask); extern int open(
unsigned char* pathname, int flags, unsigned int mode); extern int stat(
...
Cyc_Stat_stat( struct _tagged_arr filename, struct Cyc_Stat_stat_t* buf){ return
stat( string_to_Cstring( filename), buf);} int Cyc_Stat_lstat( struct
_tagged_arr filename, struct Cyc_Stat_stat_t* buf){ return lstat(
