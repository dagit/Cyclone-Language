...
filename, struct Cyc_Std_stat_t* buf); extern int mkdir( unsigned char* pathname,
unsigned int mode); int Cyc_Std_stat( struct _tagged_arr filename, struct Cyc_Std_stat_t*
buf){ return stat( string_to_Cstring( filename), buf);} int Cyc_Std_lstat(
struct _tagged_arr filename, struct Cyc_Std_stat_t* buf){ return lstat(
string_to_Cstring( filename), buf);} int Cyc_Std_mkdir( struct _tagged_arr
pathname, unsigned int mode){ return mkdir( string_to_Cstring( pathname), mode);}
