...
sizeof(unsigned char),lbuf->lex_start_pos),(unsigned int)len);*((unsigned char*)
_check_unknown_subscript(s,sizeof(unsigned char),len))='\000';return s;}
unsigned char Cyc_Lexing_lexeme_char(struct Cyc_Lexing_lexbuf*lbuf,int i){return*((
unsigned char*)_check_unknown_subscript(lbuf->lex_buffer,sizeof(unsigned char),
lbuf->lex_start_pos + i));}int Cyc_Lexing_lexeme_start(struct Cyc_Lexing_lexbuf*
