...
int dest_offset,int max_count);static const int Cyc_Std_String_pa=0;struct Cyc_Std_String_pa_struct{
...
void*),void*);extern struct Cyc_Lexing_lexbuf*Cyc_Lexing_from_file(struct Cyc_Std___sFILE*);
...
lexbuf->lex_abs_pos - oldlen;lexbuf->lex_curr_pos=lexbuf->lex_curr_pos + oldlen;
lexbuf->lex_start_pos=lexbuf->lex_start_pos + oldlen;lexbuf->lex_last_pos=lexbuf->lex_last_pos
...
lexbuf->lex_buffer_len - n));Cyc_Std_zstrncpy(_tagged_arr_plus(_tagged_arr_plus(
lexbuf->lex_buffer,sizeof(unsigned char),lexbuf->lex_buffer_len),sizeof(
unsigned char),- n),(struct _tagged_arr)Cyc_Lexing_aux_buffer,(unsigned int)n);
...
sizeof(unsigned char),lbuf->lex_start_pos),(unsigned int)len);((unsigned char*)s.curr)[
...
struct Cyc_Lexing_lexbuf*lbuf){return lbuf->lex_abs_pos + lbuf->lex_start_pos;}int
