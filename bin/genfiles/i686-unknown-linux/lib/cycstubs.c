...
struct Cyc_Std_dirent{int d_ino;int d_off;unsigned short d_reclen;unsigned char
d_type;char d_name[256];};void*Cyc_Std_opendir(struct _tagged_arr name);void*
opendir(char*name);void*Cyc_Std_opendir(struct _tagged_arr name){return opendir(
...
open_with_mode(char*,int,unsigned int);int Cyc_Std_fcntl(int fd,int cmd,struct
...
int Cyc_Std_creat(struct _tagged_arr s,unsigned int m){return creat(string_to_Cstring(
s),m);}int Cyc_Std_open(struct _tagged_arr s,int i,struct _tagged_arr ms){if(
_get_arr_size(ms,sizeof(unsigned int))>= 1)return open_with_mode(
string_to_Cstring(s),i,*((unsigned int*)_check_unknown_subscript(ms,sizeof(
...
mode);}struct Cyc_Cstdio___abstractFILE;struct Cyc_Std___cycFILE;struct Cyc_Std___cycFILE*
Cyc_Std_fromCfile(struct Cyc_Cstdio___abstractFILE*cf);int Cyc_Std_remove(struct
_tagged_arr);int Cyc_Std_rename(struct _tagged_arr,struct _tagged_arr);int Cyc_Std_fclose(
...
unsigned int,unsigned int);char*getcwd(char*buf,unsigned int size);int execv(char*
path,char**argv);int execvp(char*file,char**argv);int execve(char*path,char**argv,
char**envp);int link(char*path1,char*path2);int read(int fd,char*buf,unsigned int
count);int rmdir(char*);int symlink(char*path1,char*path2);int truncate(char*,int);
int write(int fd,char*buf,unsigned int count);int unlink(char*pathname);int
gethostname(char*,unsigned int);int chroot(char*);char*getpass(char*);int Cyc_Std_access(
...
newenvp=({unsigned int _tmpC1=_get_arr_size(envp,sizeof(struct _tagged_arr));char**
_tmpC2=(char**)_cycalloc(_check_times(sizeof(char*),_tmpC1));struct _tagged_arr
_tmpC4=_tag_arr(_tmpC2,sizeof(char*),_get_arr_size(envp,sizeof(struct _tagged_arr)));{
unsigned int _tmpC3=_tmpC1;unsigned int i;for(i=0;i < _tmpC3;i ++){_tmpC2[i]=0;}}
_tmpC4;});{int i=0;for(0;i < _get_arr_size(envp,sizeof(struct _tagged_arr));i ++){*((
...
1)),(char**)_check_null(_untag_arr(newenvp,sizeof(char*),1)));}}}int Cyc_Std_link(
struct _tagged_arr path1,struct _tagged_arr path2){return link(string_to_Cstring(
path1),string_to_Cstring(path2));}int Cyc_Std_read(int fd,struct _tagged_arr buf,
unsigned int count){if(count > _get_arr_size(buf,sizeof(char)))(int)_throw((void*)({
struct Cyc_Core_Failure_struct*_tmpC9=_cycalloc(sizeof(*_tmpC9));_tmpC9[0]=({
struct Cyc_Core_Failure_struct _tmpCA;_tmpCA.tag=Cyc_Core_Failure;_tmpCA.f1=
_tag_arr("read: called with count > buf.size",sizeof(char),35);_tmpCA;});_tmpC9;}));
return read(fd,underlying_Cstring((struct _tagged_arr)buf),count);}int Cyc_Std_rmdir(
struct _tagged_arr path){return rmdir(string_to_Cstring(path));}int Cyc_Std_symlink(
struct _tagged_arr path1,struct _tagged_arr path2){return symlink(string_to_Cstring(
path1),string_to_Cstring(path2));}int Cyc_Std_truncate(struct _tagged_arr path,int
length){return truncate(string_to_Cstring(path),length);}int Cyc_Std_write(int fd,
struct _tagged_arr buf,unsigned int count){if(count > _get_arr_size(buf,sizeof(char)))(
int)_throw((void*)({struct Cyc_Core_Failure_struct*_tmpCB=_cycalloc(sizeof(*
_tmpCB));_tmpCB[0]=({struct Cyc_Core_Failure_struct _tmpCC;_tmpCC.tag=Cyc_Core_Failure;
_tmpCC.f1=_tag_arr("write: called with count > buf.size",sizeof(char),36);_tmpCC;});
_tmpCB;}));return write(fd,underlying_Cstring(buf),count);}int Cyc_Std_unlink(
struct _tagged_arr pathname){return unlink(string_to_Cstring(pathname));}int Cyc_Std_gethostname(
struct _tagged_arr buf,unsigned int count){if(count > _get_arr_size(buf,sizeof(char)))(
int)_throw((void*)({struct Cyc_Core_Failure_struct*_tmpCD=_cycalloc(sizeof(*
_tmpCD));_tmpCD[0]=({struct Cyc_Core_Failure_struct _tmpCE;_tmpCE.tag=Cyc_Core_Failure;
_tmpCE.f1=_tag_arr("gethostname: called with count > buf.size",sizeof(char),42);
_tmpCE;});_tmpCD;}));return gethostname(underlying_Cstring((struct _tagged_arr)buf),
count);}int Cyc_Std_chroot(struct _tagged_arr pathname){return chroot(
string_to_Cstring(pathname));}struct _tagged_arr Cyc_Std_getpass(struct _tagged_arr
prompt){return wrap_Cstring_as_string(getpass(string_to_Cstring(prompt)),- 1);}
struct Cyc_Std_utimbuf{int actime;int modtime;};int Cyc_Std_utime(struct _tagged_arr
file,struct Cyc_Std_utimbuf*file_times);int utime(char*filename,struct Cyc_Std_utimbuf*
buf);int Cyc_Std_utime(struct _tagged_arr filename,struct Cyc_Std_utimbuf*buf){
return utime(string_to_Cstring(filename),buf);}
