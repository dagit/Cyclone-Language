...
l_whence;int l_len;int l_pid;};static const int Cyc_Std_Flock=0;struct Cyc_Std_Flock_struct{
int tag;struct Cyc_Std_flock*f1;};static const int Cyc_Std_Long=1;struct Cyc_Std_Long_struct{
int tag;int f1;};extern int Cyc_Std_fcntl(int fd,int cmd,struct _tagged_arr);extern int
Cyc_Std_open(struct _tagged_arr,int,struct _tagged_arr);extern int Cyc_Std_creat(
struct _tagged_arr,unsigned int);extern int fcntl(int fd,int cmd);extern int
fcntl_with_arg(int fd,int cmd,int arg);extern int fcntl_with_lock(int fd,int cmd,struct
Cyc_Std_flock*lock);extern int creat(unsigned char*,unsigned int);extern int
open_without_mode(unsigned char*,int);extern int open_with_mode(unsigned char*,int,
unsigned int);int Cyc_Std_fcntl(int fd,int cmd,struct _tagged_arr argv){if(
...
struct Cyc_Std_flock*)_tmpA);_LL3:;}}}int Cyc_Std_creat(struct _tagged_arr s,
...
unsigned short sa_family;unsigned char sa_data[14];};static const int Cyc_Std_SA_sockaddr_in=
0;struct Cyc_Std_SA_sockaddr_in_struct{int tag;struct Cyc_Std_sockaddr_in*f1;};
static const int Cyc_Std_SA_sockaddr=1;struct Cyc_Std_SA_sockaddr_struct{int tag;
struct Cyc_Std_sockaddr*f1;};static const int Cyc_Std_SA_socklenptr=2;struct Cyc_Std_SA_socklenptr_struct{
int tag;unsigned int*f1;};static const int Cyc_Std_SA_socklen=3;struct Cyc_Std_SA_socklen_struct{
int tag;unsigned int f1;};extern int Cyc_Std_accept(int fd,struct _tagged_arr);extern
int Cyc_Std_bind(int fd,struct _tagged_arr);extern int Cyc_Std_connect(int fd,struct
_tagged_arr);extern int Cyc_Std_getpeername(int fd,struct _tagged_arr);extern int Cyc_Std_getsockname(
int fd,struct _tagged_arr);extern int Cyc_Std_send(int fd,struct _tagged_arr buf,
unsigned int n,int flags);extern int Cyc_Std_recv(int fd,struct _tagged_arr buf,
unsigned int n,int flags);extern int Cyc_Std_sendto(int fd,struct _tagged_arr buf,
unsigned int n,int flags,struct _tagged_arr);extern int Cyc_Std_recvfrom(int fd,struct
_tagged_arr buf,unsigned int n,int flags,struct _tagged_arr);static const int Cyc_Std_SO_int=
0;struct Cyc_Std_SO_int_struct{int tag;int*f1;};static const int Cyc_Std_SO_timeval=1;
struct Cyc_Std_SO_timeval_struct{int tag;struct Cyc_Std_timeval*f1;};static const int
Cyc_Std_SO_socklenptr=2;struct Cyc_Std_SO_socklenptr_struct{int tag;unsigned int*
f1;};static const int Cyc_Std_SO_socklen=3;struct Cyc_Std_SO_socklen_struct{int tag;
unsigned int f1;};extern int Cyc_Std_getsockopt(int fd,int level,int optname,struct
_tagged_arr);extern int Cyc_Std_setsockopt(int fd,int level,int optname,struct
...
unsigned char*h_name;unsigned char**h_aliases;int h_addrtype;int h_length;struct
Cyc_Std_in_addr**h_addr_list;};struct Cyc_Cnetdb_Cprotoent{unsigned char*p_name;
...
src->h_addrtype;_tmpD->h_length=src->h_length;_tmpD->h_addr_list=pntlp_toCyc(src->h_addr_list);
_tmpD;}): 0;}struct Cyc_Std_protoent*Cyc_Std_getprotobyname(struct _tagged_arr name){
struct Cyc_Cnetdb_Cprotoent*src=getprotobyname(string_to_Cstring(name));return(
unsigned int)src?({struct Cyc_Std_protoent*_tmpE=_cycalloc(sizeof(struct Cyc_Std_protoent));
_tmpE->p_name=Cstring_to_string(src->p_name);_tmpE->p_aliases=ntCsl_to_ntsl(src->p_aliases);
_tmpE->p_proto=src->p_proto;_tmpE;}): 0;}void Cyc_Std_herror(struct _tagged_arr s){
...
struct Cyc_Std_stat_t*buf);extern int fstat(int fd,struct Cyc_Std_stat_t*buf);extern
...
fork();extern int fchdir(int);extern int fchown(int,unsigned int,unsigned int);
extern int fsync(int);extern int ftruncate(int,int);extern int dup(int);extern int dup2(
int,int);extern int setsid();extern int getsid(int pid);extern unsigned int getuid();
extern int setuid(unsigned int uid);extern unsigned int geteuid();extern int seteuid(
unsigned int euid);extern unsigned int getgid();extern int setgid(unsigned int gid);
extern unsigned int getegid();extern int setegid(unsigned int egid);extern int nice(int);
...
int)_throw((void*)({struct Cyc_Core_Failure_struct*_tmp154=_cycalloc(sizeof(
struct Cyc_Core_Failure_struct));_tmp154[0]=({struct Cyc_Core_Failure_struct
