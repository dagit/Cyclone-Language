...
_tmpC3=_tmpC1;unsigned int i;for(i=0;i < _tmpC3;i ++){_tmpC2[i]=0;}}_tmpC4;});{int i=
0;for(0;i < _get_arr_size(envp,sizeof(struct _tagged_arr));i ++){((unsigned char**)
...
sizeof(unsigned char),36);_tmpCC;});_tmpCB;}));return write(fd,underlying_Cstring(
buf),count);}int Cyc_Std_unlink(struct _tagged_arr pathname){return unlink(
string_to_Cstring(pathname));}int Cyc_Std_gethostname(struct _tagged_arr buf,
unsigned int count){if(count > _get_arr_size(buf,sizeof(unsigned char)))(int)
_throw((void*)({struct Cyc_Core_Failure_struct*_tmpCD=_cycalloc(sizeof(*_tmpCD));
_tmpCD[0]=({struct Cyc_Core_Failure_struct _tmpCE;_tmpCE.tag=Cyc_Core_Failure;
_tmpCE.f1=_tag_arr("gethostname: called with count > buf.size",sizeof(
unsigned char),42);_tmpCE;});_tmpCD;}));return gethostname(underlying_Cstring((
struct _tagged_arr)buf),count);}int Cyc_Std_chroot(struct _tagged_arr pathname){
return chroot(string_to_Cstring(pathname));}struct _tagged_arr Cyc_Std_getpass(
struct _tagged_arr prompt){return wrap_Cstring_as_string(getpass(string_to_Cstring(
prompt)),- 1);}struct Cyc_utimbuf{int actime;int modtime;};int utime(unsigned char*
filename,struct Cyc_utimbuf*buf);int Cyc_Std_utime(struct _tagged_arr filename,
struct Cyc_utimbuf*buf){return utime(string_to_Cstring(filename),buf);}
