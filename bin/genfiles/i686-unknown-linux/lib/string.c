...
_tagged_arr, struct _tagged_arr, unsigned int); extern double acos( double);
extern float acosf( float); extern double acosh( double); extern float acoshf(
float); extern double asin( double); extern float asinf( float); extern double
asinh( double); extern float asinhf( float); extern double atan( double); extern
double atan2( double, double); extern float atan2f( float, float); extern float
atanf( float); extern double atanh( double); extern float atanhf( float); extern
double cbrt( double); extern float cbrtf( float); extern double ceil( double);
extern float ceilf( float); extern double copysign( double, double); extern
float copysignf( float, float); extern double cos( double); extern float cosf(
float); extern double cosh( double); extern float coshf( float); extern double
drem( double, double); extern float dremf( float, float); extern double erf(
double); extern double erfc( double); extern float erfcf( float); extern float
erff( float); extern double exp( double); extern float expf( float); extern
double expm1( double); extern float expm1f( float); extern double fabs( double);
extern float fabsf( float); extern int finite( double); extern int finitef(
float); extern double floor( double); extern float floorf( float); extern double
fmod( double, double); extern float fmodf( float, float); extern double frexp(
double, int*); extern float frexpf( float, int*); extern double gamma( double);
extern float gammaf( float); extern double hypot( double, double); extern float
hypotf( float, float); extern int ilogb( double); extern int ilogbf( float);
extern int isinf( double); extern int isinff( float); extern int isnan( double);
extern int isnanf( float); extern double j0( double); extern float j0f( float);
extern double j1( double); extern float j1f( float); extern double jn( int,
double); extern float jnf( int, float); extern double ldexp( double, int);
...
unsigned int Cyc_std_int_strleno( struct _tagged_arr s, int ofs, struct
...
return( unsigned int)( i -  ofs);}} static int Cyc_std_case_cmp( unsigned char
c1, unsigned char c2){ return c1 -  c2;} static int Cyc_std_cmp( struct
_tagged_arr s1, int ofs1, unsigned int len1, struct _tagged_arr s2, int ofs2,
unsigned int len2, int(* f)( unsigned char, unsigned char)){ unsigned int
...
int) len1 - ( int) len2;} int Cyc_std_strcmp( struct _tagged_arr s1, struct
_tagged_arr s2){ if( s1.curr ==  s2.curr){ return 0;}{ unsigned int len1= Cyc_std_int_strleno(
s1, 0, _tag_arr("std::strcmp", sizeof( unsigned char), 12u)); unsigned int len2=
Cyc_std_int_strleno( s2, 0, _tag_arr("std::strcmp", sizeof( unsigned char), 12u));
return Cyc_std_cmp( s1, 0, len1, s2, 0, len2, Cyc_std_case_cmp);}} int Cyc_std_strptrcmp(
struct _tagged_arr* s1, struct _tagged_arr* s2){ return Cyc_std_strcmp(* s1,* s2);}
static int Cyc_std_ncmp( struct _tagged_arr s1, int ofs1, unsigned int len1,
struct _tagged_arr s2, int ofs2, unsigned int len2, unsigned int n, int(* f)(
unsigned char, unsigned char)){ if( n <=  0){ return 0;}{ unsigned int min_len=
len1 >  len2? len2: len1; unsigned int bound= min_len >  n? n: min_len;{ int i=
...
s1, 0, _tag_arr("std::strncmp", sizeof( unsigned char), 13u)); unsigned int len2=
Cyc_std_int_strleno( s2, 0, _tag_arr("std::strncmp", sizeof( unsigned char), 13u));
return Cyc_std_ncmp( s1, 0, len1, s2, 0, len2, n, Cyc_std_case_cmp);} int Cyc_std_zstrcmp(
...
_tagged_arr src, int srcOfs, struct _tagged_arr error){ int i; unsigned int
dsize; unsigned int slen; unsigned int dlen; dsize= _get_arr_size( dest, sizeof(
...
_temp3.f1= error; _temp3;}); _temp2;}));} return dest;} struct _tagged_arr Cyc_std_strcat(
struct _tagged_arr dest, struct _tagged_arr src){ return Cyc_std_int_strcato(
dest, src, 0, _tag_arr("std::strcat", sizeof( unsigned char), 12u));} struct
_tagged_arr Cyc_std_rstrconcat( struct _RegionHandle* r, struct _tagged_arr a,
struct _tagged_arr b){ unsigned int _temp4= Cyc_std_strlen( a); unsigned int
_temp5= Cyc_std_strlen( b); struct _tagged_arr ans= Cyc_Core_rnew_string( r,(
...
unsigned int total_len= 0; struct _tagged_arr ans;{ struct _RegionHandle _temp6=
...
_region_malloc( temp, sizeof( struct Cyc_List_List)); _temp9->hd=( void*)((
unsigned int) 0); _temp9->tl= 0; _temp9;}); struct Cyc_List_List* end= lens;{
...
end))->tl=({ struct Cyc_List_List* _temp7=( struct Cyc_List_List*)
...
Cyc_std_strncpy( _tagged_arr_plus( ans, sizeof( unsigned char),( int) i), _temp8,
...
_temp10=(( struct Cyc_List_List*) _check_null( _temp10))->tl){ len= Cyc_std_strlen(*((
...
unsigned char),( int) i), _temp13, len); i += len; Cyc_std_strncpy(
...
_temp14;}}}}; _pop_region( temp);}} struct _tagged_arr Cyc_std_str_sepstr(
...
_temp19.f1= _tag_arr("std::strncpy", sizeof( unsigned char), 13u); _temp19;});
...
src, unsigned int n){ unsigned int _temp21= _get_arr_size( dest, sizeof(
unsigned char)); unsigned int _temp22= _get_arr_size( src, sizeof( unsigned char));
if( n <=  0){ return dest;} if( n >  _temp21? 1: n >  _temp22){( int) _throw((
void*)({ struct Cyc_Core_InvalidArg_struct* _temp23=( struct Cyc_Core_InvalidArg_struct*)
...
_tag_arr("std::zstrncpy", sizeof( unsigned char), 14u); _temp24;}); _temp23;}));}{
...
Cyc_Core_InvalidArg_struct* _temp25=( struct Cyc_Core_InvalidArg_struct*)
...
_tag_arr("std::substring", sizeof( unsigned char), 15u); _temp26;}); _temp25;}));}{
...
struct _tagged_arr) s, 0, _tag_arr("std::strpbrk", sizeof( unsigned char), 13u));
...
struct _tagged_arr s, unsigned char c){ int len=( int) Cyc_std_int_strleno( s, 0,
...
_tagged_arr) s, 0, _tag_arr("std::strrchr", sizeof( unsigned char), 13u)); int i=
...
_tag_arr("std::strstr", sizeof( unsigned char), 12u); _temp32;}); _temp31;}));}
...
needle, 0, _tag_arr("std::strstr", sizeof( unsigned char), 12u));{ struct
...
s, 0, _tag_arr("std::strspn", sizeof( unsigned char), 12u));{ unsigned int i= 0;
...
_temp35.f1= _tag_arr("std::to_int", sizeof( unsigned char), 12u); _temp35;});
...
_temp39.f1= _tag_arr("std::to_double", sizeof( unsigned char), 15u); _temp39;});
...
==  s2.curr){ return 0;}{ unsigned int len1= Cyc_std_int_strleno( s1, 0,
_tag_arr("std::strcasecmp", sizeof( unsigned char), 16u)); unsigned int len2=
Cyc_std_int_strleno( s2, 0, _tag_arr("std::strcasecmp", sizeof( unsigned char),
16u)); return Cyc_std_cmp( s1, 0, len1, s2, 0, len2, Cyc_std_case_cmp);}} static
int Cyc_std_nocase_cmp( unsigned char c1, unsigned char c2){ return Cyc_std_case_cmp((
...
Cyc_std_int_strleno( s1, 0, _tag_arr("std::strncasecmp", sizeof( unsigned char),
17u)); unsigned int len2= Cyc_std_int_strleno( s2, 0, _tag_arr("std::strncasecmp",
sizeof( unsigned char), 17u)); return Cyc_std_ncmp( s1, 0, len1, s2, 0, len2, n,
Cyc_std_nocase_cmp);}
