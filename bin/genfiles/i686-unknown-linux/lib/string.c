...
while(i < minsz){unsigned char c1=((const unsigned char*)s1.curr)[i];unsigned char
c2=((const unsigned char*)s2.curr)[i];if(c1 == '\000'){if(c2 == '\000')return 0;
else{return - 1;}}else{if(c2 == '\000')return 1;else{int diff=c1 - c2;if(diff != 0)
return diff;}}++ i;}if(sz1 == sz2)return 0;if(minsz < sz2){if(((const unsigned char*)
s2.curr)[i]== '\000')return 0;else{return - 1;}}else{if(((const unsigned char*)s1.curr)[
i]== '\000')return 0;else{return 1;}}}}int Cyc_Std_strptrcmp(struct _tagged_arr*s1,
struct _tagged_arr*s2){return Cyc_Std_strcmp(*s1,*s2);}inline static int Cyc_Std_ncmp(
...
int)len2;}}int Cyc_Std_strcasecmp(struct _tagged_arr s1,struct _tagged_arr s2){if(s1.curr
...
int)len1 - (int)len2;return 0;}}int Cyc_Std_strncasecmp(struct _tagged_arr s1,struct
_tagged_arr s2,unsigned int n){unsigned int len1=Cyc_Std_int_strleno(s1,_tag_arr("Std::strncasecmp",
sizeof(unsigned char),17));unsigned int len2=Cyc_Std_int_strleno(s2,_tag_arr("Std::strncasecmp",
sizeof(unsigned char),17));return Cyc_Std_caseless_ncmp(s1,len1,s2,len2,n);}void*
memcpy(void*,const void*,unsigned int n);void*memmove(void*,const void*,
unsigned int n);int memcmp(const void*,const void*,unsigned int n);unsigned char*
memchr(const unsigned char*,unsigned char c,unsigned int n);void*memset(void*,int c,
unsigned int n);void bcopy(const void*src,void*dest,unsigned int n);void bzero(void*s,
unsigned int n);unsigned char*GC_realloc(unsigned char*,unsigned int n);struct
_tagged_arr Cyc_Std_realloc(struct _tagged_arr s,unsigned int n){unsigned char*
_tmp19=GC_realloc((unsigned char*)_check_null(_untag_arr(s,sizeof(unsigned char),
1)),n);return wrap_Cstring_as_string(_tmp19,n);}struct _tagged_arr Cyc_Std__memcpy(
...
_tmp1D.f1=_tag_arr("Std::memove",sizeof(unsigned char),12);_tmp1D;});_tmp1C;}));
