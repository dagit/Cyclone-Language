...
struct _tagged_arr wrap_Cstring_as_string(unsigned char*,unsigned int);struct Cyc_List_List{
...
int toupper(int);struct _tagged_arr Cyc_Std_strerror(int);unsigned int Cyc_Std_strlen(
...
struct _tagged_arr s,unsigned char c,unsigned int n);struct _tagged_arr Cyc_Std_mmemchr(
struct _tagged_arr s,unsigned char c,unsigned int n);struct _tagged_arr Cyc_Std_memset(
struct _tagged_arr s,unsigned char c,unsigned int n);void Cyc_Std_bzero(struct
_tagged_arr s,unsigned int n);void Cyc_Std__bcopy(struct _tagged_arr src,struct
_tagged_arr dst,unsigned int n,unsigned int sz);struct _tagged_arr Cyc_Std_expand(
struct _tagged_arr s,unsigned int sz);struct _tagged_arr Cyc_Std_rexpand(struct
_RegionHandle*,struct _tagged_arr s,unsigned int sz);struct _tagged_arr Cyc_Std_realloc_str(
struct _tagged_arr str,unsigned int sz);struct _tagged_arr Cyc_Std_rrealloc_str(
struct _RegionHandle*r,struct _tagged_arr str,unsigned int sz);struct _tagged_arr Cyc_Std_strdup(
...
_tagged_arr new_suffix);struct _tagged_arr Cyc_Std_strchr(struct _tagged_arr s,
unsigned char c);struct _tagged_arr Cyc_Std_mstrchr(struct _tagged_arr s,
unsigned char c);struct _tagged_arr Cyc_Std_mstrrchr(struct _tagged_arr s,
unsigned char c);struct _tagged_arr Cyc_Std_strrchr(struct _tagged_arr s,
unsigned char c);struct _tagged_arr Cyc_Std_mstrstr(struct _tagged_arr haystack,
struct _tagged_arr needle);struct _tagged_arr Cyc_Std_strstr(struct _tagged_arr
haystack,struct _tagged_arr needle);struct _tagged_arr Cyc_Std_strpbrk(struct
_tagged_arr s,struct _tagged_arr accept);struct _tagged_arr Cyc_Std_mstrpbrk(struct
_tagged_arr s,struct _tagged_arr accept);unsigned int Cyc_Std_strspn(struct
_tagged_arr s,struct _tagged_arr accept);unsigned int Cyc_Std_strcspn(struct
_tagged_arr s,struct _tagged_arr accept);struct _tagged_arr Cyc_Std_strtok(struct
_tagged_arr s,struct _tagged_arr delim);struct Cyc_List_List*Cyc_Std_explode(struct
_tagged_arr s);struct Cyc_List_List*Cyc_Std_rexplode(struct _RegionHandle*,struct
_tagged_arr s);struct _tagged_arr Cyc_Std_implode(struct Cyc_List_List*c);int Cyc_Std_strcasecmp(
struct _tagged_arr,struct _tagged_arr);int Cyc_Std_strncasecmp(struct _tagged_arr s1,
struct _tagged_arr s2,unsigned int len);void*Cyc_Std___assert_fail(struct
_tagged_arr assertion,struct _tagged_arr file,unsigned int line);unsigned char*
strerror(int errnum);struct _tagged_arr Cyc_Std_strerror(int errnum){return(struct
_tagged_arr)wrap_Cstring_as_string(strerror(errnum),- 1);}unsigned int Cyc_Std_strlen(
struct _tagged_arr s){unsigned int i;unsigned int sz=_get_arr_size(s,sizeof(
unsigned char));for(i=0;i < sz;i ++){if(((const unsigned char*)s.curr)[(int)i]== '\000')
return i;}return i;}inline static unsigned int Cyc_Std_int_strleno(struct _tagged_arr s,
struct _tagged_arr error){int i;unsigned int sz=_get_arr_size(s,sizeof(unsigned char));
...
== s2.curr)return 0;{int i=0;unsigned int sz1=_get_arr_size(s1,sizeof(unsigned char));
unsigned int sz2=_get_arr_size(s2,sizeof(unsigned char));unsigned int minsz=sz1 < 
sz2?sz1: sz2;(minsz <= sz1?minsz <= sz2: 0)?0:((int(*)(struct _tagged_arr assertion,
struct _tagged_arr file,unsigned int line))Cyc_Std___assert_fail)(_tag_arr("minsz <= sz1 && minsz <= sz2",
sizeof(unsigned char),29),_tag_arr("string.cyc",sizeof(unsigned char),11),84);
while(i < minsz){unsigned char c1=*((const unsigned char*)_check_unknown_subscript(
s1,sizeof(unsigned char),i));unsigned char c2=*((const unsigned char*)
_check_unknown_subscript(s2,sizeof(unsigned char),i));if(c1 == '\000'){if(c2 == '\000')
return 0;else{return - 1;}}else{if(c2 == '\000')return 1;else{int diff=c1 - c2;if(diff
!= 0)return diff;}}++ i;}if(sz1 == sz2)return 0;if(minsz < sz2){if(*((const
unsigned char*)_check_unknown_subscript(s2,sizeof(unsigned char),i))== '\000')
return 0;else{return - 1;}}else{if(*((const unsigned char*)_check_unknown_subscript(
s1,sizeof(unsigned char),i))== '\000')return 0;else{return 1;}}}}int Cyc_Std_strptrcmp(
struct _tagged_arr*s1,struct _tagged_arr*s2){return Cyc_Std_strcmp(*s1,*s2);}inline
static int Cyc_Std_ncmp(struct _tagged_arr s1,unsigned int len1,struct _tagged_arr s2,
...
as=_get_arr_size(a,sizeof(unsigned char));unsigned int bs=_get_arr_size(b,sizeof(
unsigned char));unsigned int min_length=as < bs?as: bs;int i=- 1;(min_length <= 
_get_arr_size(a,sizeof(unsigned char))?min_length <= _get_arr_size(b,sizeof(
...
const unsigned char*)s1.curr)[i])return 1;}}}if(min_size <= bound)return 0;if(s1size
< s2size)return - 1;else{return 1;}}}int Cyc_Std_zstrptrcmp(struct _tagged_arr*a,
struct _tagged_arr*b){if((int)a == (int)b)return 0;return Cyc_Std_zstrcmp(*a,*b);}
inline static struct _tagged_arr Cyc_Std_int_strcato(struct _tagged_arr dest,struct
_tagged_arr src,struct _tagged_arr error){int i;unsigned int dsize;unsigned int slen;
unsigned int dlen;dsize=_get_arr_size(dest,sizeof(unsigned char));dlen=Cyc_Std_strlen((
struct _tagged_arr)dest);slen=Cyc_Std_int_strleno(src,error);if(slen + dlen <= 
dsize){slen <= _get_arr_size(src,sizeof(unsigned char))?0:((int(*)(struct
...
sizeof(unsigned char),11),204);for(i=0;i < slen;i ++){*((unsigned char*)
...
sizeof(unsigned char),11),228);for(i=0;i < _tmp2;++ i){((unsigned char*)ans.curr)[
...
unsigned char),15),_tag_arr("string.cyc",sizeof(unsigned char),11),230);for(j=0;
...
Cyc_Std_strncpy(dest,src,len);if(len < _get_arr_size(dest,sizeof(unsigned char)))((
unsigned char*)dest.curr)[(int)len]='\000';}return dest;}struct _tagged_arr Cyc_Std_rstrdup(
struct _RegionHandle*r,struct _tagged_arr src){unsigned int len;struct _tagged_arr
temp;len=Cyc_Std_strlen(src);temp=Cyc_Core_rnew_string(r,len + 1);Cyc_Std_strncpy(
temp,src,len);return temp;}struct _tagged_arr Cyc_Std_strdup(struct _tagged_arr src){
return Cyc_Std_rstrdup(Cyc_Core_heap_region,src);}struct _tagged_arr Cyc_Std_rexpand(
struct _RegionHandle*r,struct _tagged_arr s,unsigned int sz){struct _tagged_arr temp;
unsigned int slen;slen=Cyc_Std_strlen(s);sz=sz > slen?sz: slen;temp=Cyc_Core_rnew_string(
r,sz);Cyc_Std_strncpy(temp,s,slen);if(slen != _get_arr_size(s,sizeof(
unsigned char)))*((unsigned char*)_check_unknown_subscript(temp,sizeof(
unsigned char),(int)slen))='\000';return temp;}struct _tagged_arr Cyc_Std_expand(
struct _tagged_arr s,unsigned int sz){return Cyc_Std_rexpand(Cyc_Core_heap_region,s,
sz);}struct _tagged_arr Cyc_Std_rrealloc_str(struct _RegionHandle*r,struct
_tagged_arr str,unsigned int sz){unsigned int maxsizeP=_get_arr_size(str,sizeof(
unsigned char));if(maxsizeP == 0){maxsizeP=30 > sz?30: sz;str=Cyc_Core_rnew_string(
r,maxsizeP);*((unsigned char*)_check_unknown_subscript(str,sizeof(unsigned char),
0))='\000';}else{if(sz > maxsizeP){maxsizeP=maxsizeP * 2 > (sz * 5)/ 4?maxsizeP * 2:(
sz * 5)/ 4;str=Cyc_Std_rexpand(r,(struct _tagged_arr)str,maxsizeP);}}return str;}
struct _tagged_arr Cyc_Std_realloc_str(struct _tagged_arr str,unsigned int sz){return
Cyc_Std_rrealloc_str(Cyc_Core_heap_region,str,sz);}struct _tagged_arr Cyc_Std_rsubstring(
struct _RegionHandle*r,struct _tagged_arr s,int start,unsigned int amt){struct
_tagged_arr ans=Cyc_Core_rnew_string(r,amt + 1);s=_tagged_arr_plus(s,sizeof(
unsigned char),start);(amt < _get_arr_size(ans,sizeof(unsigned char))?amt <= 
_get_arr_size(s,sizeof(unsigned char)): 0)?0:((int(*)(struct _tagged_arr assertion,
struct _tagged_arr file,unsigned int line))Cyc_Std___assert_fail)(_tag_arr("amt < ans.size && amt <= s.size",
sizeof(unsigned char),32),_tag_arr("string.cyc",sizeof(unsigned char),11),431);{
unsigned int i=0;for(0;i < amt;++ i){((unsigned char*)ans.curr)[(int)i]=((const
unsigned char*)s.curr)[(int)i];}}((unsigned char*)ans.curr)[(int)amt]='\000';
return ans;}struct _tagged_arr Cyc_Std_substring(struct _tagged_arr s,int start,
unsigned int amt){return Cyc_Std_rsubstring(Cyc_Core_heap_region,s,start,amt);}
struct _tagged_arr Cyc_Std_rreplace_suffix(struct _RegionHandle*r,struct _tagged_arr
src,struct _tagged_arr curr_suffix,struct _tagged_arr new_suffix){unsigned int m=
_get_arr_size(src,sizeof(unsigned char));unsigned int n=_get_arr_size(curr_suffix,
sizeof(unsigned char));struct _tagged_arr err=_tag_arr("Std::replace_suffix",
...
_check_unknown_subscript(s,sizeof(unsigned char),0))== c)return s;}return _tag_arr(
0,0,0);}struct _tagged_arr Cyc_Std_strstr(struct _tagged_arr haystack,struct
...
Cyc_Core_Invalid_argument;_tmp15.f1=_tag_arr("Std::strstr",sizeof(unsigned char),
12);_tmp15;});_tmp14;}));if(*((const unsigned char*)_check_unknown_subscript(
needle,sizeof(unsigned char),0))== '\000')return haystack;{int len=(int)Cyc_Std_int_strleno(
needle,_tag_arr("Std::strstr",sizeof(unsigned char),12));{struct _tagged_arr start=
haystack;for(0;(start=Cyc_Std_strchr(start,*((const unsigned char*)
_check_unknown_subscript(needle,sizeof(unsigned char),0)))).curr != ((struct
_tagged_arr)_tag_arr(0,0,0)).curr;start=Cyc_Std_strchr(_tagged_arr_plus(start,
sizeof(unsigned char),1),*((const unsigned char*)_check_unknown_subscript(needle,
sizeof(unsigned char),0)))){if(Cyc_Std_strncmp(start,needle,(unsigned int)len)== 
0)return start;}}return(struct _tagged_arr)_tag_arr(0,0,0);}}struct _tagged_arr Cyc_Std_mstrstr(
...
haystack;{int len=(int)Cyc_Std_int_strleno(needle,_tag_arr("Std::mstrstr",sizeof(
unsigned char),13));{struct _tagged_arr start=haystack;for(0;(start=Cyc_Std_mstrchr(
...
_tag_arr("Std::strspn",sizeof(unsigned char),12));unsigned int asize=
...
unsigned char*)_check_unknown_subscript(s,sizeof(unsigned char),i)));_tmp18->tl=
...
unsigned int i=0;while(chars != 0){*((unsigned char*)_check_unknown_subscript(s,
sizeof(unsigned char),(int)i ++))=(unsigned char)((int)chars->hd);chars=chars->tl;}
return s;}inline static int Cyc_Std_casecmp(struct _tagged_arr s1,unsigned int len1,
struct _tagged_arr s2,unsigned int len2){unsigned int min_length=len1 < len2?len1:
len2;(min_length <= _get_arr_size(s1,sizeof(unsigned char))?min_length <= 
_get_arr_size(s2,sizeof(unsigned char)): 0)?0:((int(*)(struct _tagged_arr assertion,
struct _tagged_arr file,unsigned int line))Cyc_Std___assert_fail)(_tag_arr("min_length <= s1.size && min_length <= s2.size",
sizeof(unsigned char),47),_tag_arr("string.cyc",sizeof(unsigned char),11),680);{
int i=- 1;while((++ i,i < min_length)){int diff=toupper((int)((const unsigned char*)s1.curr)[
i])- toupper((int)((const unsigned char*)s2.curr)[i]);if(diff != 0)return diff;}
return(int)len1 - (int)len2;}}int Cyc_Std_strcasecmp(struct _tagged_arr s1,struct
_tagged_arr s2){if(s1.curr == s2.curr)return 0;{unsigned int len1=Cyc_Std_int_strleno(
s1,_tag_arr("Std::strcasecmp",sizeof(unsigned char),16));unsigned int len2=Cyc_Std_int_strleno(
s2,_tag_arr("Std::strcasecmp",sizeof(unsigned char),16));return Cyc_Std_casecmp(
...
min_len;(bound <= _get_arr_size(s1,sizeof(unsigned char))?bound <= _get_arr_size(
...
sizeof(unsigned char),37),_tag_arr("string.cyc",sizeof(unsigned char),11),707);{
int i=0;for(0;i < bound;i ++){int retc;if((retc=toupper((int)((const unsigned char*)
s1.curr)[i])- toupper((int)((const unsigned char*)s2.curr)[i]))!= 0)return retc;}}
if(len1 < n?1: len2 < n)return(int)len1 - (int)len2;return 0;}}int Cyc_Std_strncasecmp(
struct _tagged_arr s1,struct _tagged_arr s2,unsigned int n){unsigned int len1=Cyc_Std_int_strleno(
s1,_tag_arr("Std::strncasecmp",sizeof(unsigned char),17));unsigned int len2=Cyc_Std_int_strleno(
...
unsigned char*memchr(const unsigned char*,unsigned char c,unsigned int n);void*
memset(void*,int c,unsigned int n);void bcopy(const void*src,void*dest,unsigned int n);
void bzero(void*s,unsigned int n);unsigned char*GC_realloc(unsigned char*,
unsigned int n);struct _tagged_arr Cyc_Std_realloc(struct _tagged_arr s,unsigned int n){
unsigned char*_tmp19=GC_realloc((unsigned char*)_check_null(_untag_arr(s,sizeof(
unsigned char),1)),n);return wrap_Cstring_as_string(_tmp19,n);}struct _tagged_arr
...
int)_throw((void*)({struct Cyc_Core_Invalid_argument_struct*_tmp1A=_cycalloc(
sizeof(*_tmp1A));_tmp1A[0]=({struct Cyc_Core_Invalid_argument_struct _tmp1B;_tmp1B.tag=
Cyc_Core_Invalid_argument;_tmp1B.f1=_tag_arr("Std::memcpy",sizeof(unsigned char),
12);_tmp1B;});_tmp1A;}));memcpy((void*)_check_null(_untag_arr(d,sizeof(void),1)),(
const void*)_check_null(_untag_arr(s,sizeof(void),1)),n * sz);return d;}struct
_tagged_arr Cyc_Std__memmove(struct _tagged_arr d,struct _tagged_arr s,unsigned int n,
unsigned int sz){if(((d.curr == (_tag_arr(0,0,0)).curr?1: _get_arr_size(d,sizeof(
void))< n)?1: s.curr == ((struct _tagged_arr)_tag_arr(0,0,0)).curr)?1: _get_arr_size(
s,sizeof(void))< n)(int)_throw((void*)({struct Cyc_Core_Invalid_argument_struct*
_tmp1C=_cycalloc(sizeof(*_tmp1C));_tmp1C[0]=({struct Cyc_Core_Invalid_argument_struct
_tmp1D;_tmp1D.tag=Cyc_Core_Invalid_argument;_tmp1D.f1=_tag_arr("Std::memove",
sizeof(unsigned char),12);_tmp1D;});_tmp1C;}));memmove((void*)_check_null(
_untag_arr(d,sizeof(void),1)),(const void*)_check_null(_untag_arr(s,sizeof(void),
1)),n * sz);return d;}int Cyc_Std_memcmp(struct _tagged_arr s1,struct _tagged_arr s2,
unsigned int n){if(((s1.curr == ((struct _tagged_arr)_tag_arr(0,0,0)).curr?1: s2.curr
== ((struct _tagged_arr)_tag_arr(0,0,0)).curr)?1: _get_arr_size(s1,sizeof(
unsigned char))>= n)?1: _get_arr_size(s2,sizeof(unsigned char))>= n)(int)_throw((
void*)({struct Cyc_Core_Invalid_argument_struct*_tmp1E=_cycalloc(sizeof(*_tmp1E));
_tmp1E[0]=({struct Cyc_Core_Invalid_argument_struct _tmp1F;_tmp1F.tag=Cyc_Core_Invalid_argument;
_tmp1F.f1=_tag_arr("Std::memcmp",sizeof(unsigned char),12);_tmp1F;});_tmp1E;}));
...
_tmp20=_cycalloc(sizeof(*_tmp20));_tmp20[0]=({struct Cyc_Core_Invalid_argument_struct
