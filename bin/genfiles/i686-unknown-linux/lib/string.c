...
_dyneither_ptr)*s2);}inline static int Cyc_ncmp(struct _dyneither_ptr s1,
unsigned long len1,struct _dyneither_ptr s2,unsigned long len2,unsigned long n){if(n
<= 0)return 0;{unsigned long min_len=len1 > len2?len2: len1;unsigned long bound=
min_len > n?n: min_len;bound <= _get_dyneither_size(s1,sizeof(char)) && bound <= 
_get_dyneither_size(s2,sizeof(char))?0:((int(*)(struct _dyneither_ptr assertion,
struct _dyneither_ptr file,unsigned int line))Cyc___assert_fail)(({const char*_tmp2="bound <= numelts(s1) && bound <= numelts(s2)";
_tag_dyneither(_tmp2,sizeof(char),45);}),({const char*_tmp3="string.cyc";
_tag_dyneither(_tmp3,sizeof(char),11);}),95);{int i=0;for(0;i < bound;++ i){int retc;
...
Cyc_strlen(s1);unsigned long len2=Cyc_strlen(s2);return Cyc_ncmp(s1,len1,s2,len2,n);}
...
_tag_dyneither(_tmp5,sizeof(char),11);}),127);while((++ i,i < min_length)){int diff=(
...
_dyneither_ptr s2,unsigned long n){if(n <= 0)return 0;{unsigned long s1size=
_get_dyneither_size(s1,sizeof(char));unsigned long s2size=_get_dyneither_size(s2,
sizeof(char));unsigned long min_size=s1size > s2size?s2size: s1size;unsigned long
...
char),11);}),145);{int i=0;for(0;i < bound;++ i){if(((const char*)s1.curr)[i]< ((
...
_dyneither_ptr dest,struct _dyneither_ptr src){int i;unsigned long dsize;
...
dsize){slen <= _get_dyneither_size(src,sizeof(char))?0:((int(*)(struct
_dyneither_ptr assertion,struct _dyneither_ptr file,unsigned int line))Cyc___assert_fail)(({
const char*_tmp8="slen <= numelts(src)";_tag_dyneither(_tmp8,sizeof(char),21);}),({
const char*_tmp9="string.cyc";_tag_dyneither(_tmp9,sizeof(char),11);}),181);for(i=
0;i < slen;++ i){({struct _dyneither_ptr _tmpA=_dyneither_ptr_plus(dest,sizeof(char),(
int)(i + dlen));char _tmpB=*((char*)_check_dyneither_subscript(_tmpA,sizeof(char),
0));char _tmpC=((const char*)src.curr)[i];if(_get_dyneither_size(_tmpA,sizeof(char))
== 1  && (_tmpB == '\000'  && _tmpC != '\000'))_throw_arraybounds();*((char*)_tmpA.curr)=
_tmpC;});}if(i != dsize)({struct _dyneither_ptr _tmpD=_dyneither_ptr_plus(dest,
sizeof(char),(int)(i + dlen));char _tmpE=*((char*)_check_dyneither_subscript(_tmpD,
sizeof(char),0));char _tmpF='\000';if(_get_dyneither_size(_tmpD,sizeof(char))== 1
 && (_tmpE == '\000'  && _tmpF != '\000'))_throw_arraybounds();*((char*)_tmpD.curr)=
_tmpF;});}else{(int)_throw((void*)({struct Cyc_Core_Invalid_argument_struct*
...
_tag_dyneither(_tmp12,sizeof(char),7);});_tmp11;});_tmp10;}));}return dest;}
...
r,(_tmp13 + _tmp14)+ 1);int i;int j;_tmp13 <= _get_dyneither_size(ans,sizeof(char))
...
sizeof(char),11);}),205);for(i=0;i < _tmp13;++ i){({struct _dyneither_ptr _tmp17=
...
_tag_dyneither(_tmp1B,sizeof(char),11);}),207);for(j=0;j < _tmp14;++ j){({struct
...
_dyneither_ptr _tmp21=*((struct _dyneither_ptr*)strs->hd);len=(unsigned long)((
...
_region_malloc(temp,sizeof(*_tmp29));_tmp29->hd=(void*)((unsigned long)0);_tmp29->tl=
0;_tmp29;});struct Cyc_List_List*end=lens;unsigned long len;unsigned long total_len=
0;unsigned long list_len=0;for(0;_tmp24 != 0;_tmp24=_tmp24->tl){len=Cyc_strlen((
...
struct _dyneither_ptr ans=Cyc_Core_rnew_string(r,total_len + 1);unsigned long i=0;
...
unsigned long n){int i;n <= _get_dyneither_size(src,sizeof(char)) && n <= 
...
_tag_dyneither(_tmp2B,sizeof(char),11);}),298);for(i=0;i < n;++ i){char _tmp2C=((
...
Cyc_zstrncpy(struct _dyneither_ptr dest,struct _dyneither_ptr src,unsigned long n){n
...
_tag_dyneither(_tmp2E,sizeof(char),11);}),312);{int i;for(i=0;i < n;++ i){((char*)
...
_tmp32;});if(_tmp2F == '\000')break;}}else{unsigned long len=Cyc_strlen(src);Cyc_strncpy(
...
sizeof(char),11);}),408);{unsigned long i=0;for(0;i < amt;++ i){({struct
...
char));unsigned long n=_get_dyneither_size(curr_suffix,sizeof(char));struct
...
line))Cyc___assert_fail)(({const char*_tmp4F="len <= numelts(s)";_tag_dyneither(
_tmp4F,sizeof(char),18);}),({const char*_tmp50="string.cyc";_tag_dyneither(_tmp50,
sizeof(char),11);}),558);{unsigned long i=0;for(0;i < len;++ i){int j;for(j=0;j < 
asize;++ j){if(((const char*)s.curr)[(int)i]== ((const char*)accept.curr)[j])break;}
if(j == asize)return i;}}return len;}unsigned long Cyc_strcspn(struct _dyneither_ptr s,
struct _dyneither_ptr accept){unsigned long len=Cyc_strlen((struct _dyneither_ptr)s);
unsigned int asize=_get_dyneither_size(accept,sizeof(char));len <= 
_get_dyneither_size(s,sizeof(char))?0:((int(*)(struct _dyneither_ptr assertion,
struct _dyneither_ptr file,unsigned int line))Cyc___assert_fail)(({const char*_tmp51="len <= numelts(s)";
_tag_dyneither(_tmp51,sizeof(char),18);}),({const char*_tmp52="string.cyc";
_tag_dyneither(_tmp52,sizeof(char),11);}),578);{unsigned long i=0;for(0;i < len;++
i){int j;for(j=0;j < asize;++ j){if(((const char*)s.curr)[(int)i]!= ((const char*)
accept.curr)[j])break;}if(j == asize)return i;}}return len;}struct _dyneither_ptr Cyc_strtok(
struct _dyneither_ptr s,struct _dyneither_ptr delim){static struct _dyneither_ptr olds={(
void*)0,(void*)0,(void*)(0 + 0)};struct _dyneither_ptr token;if(s.curr == (
_tag_dyneither(0,0,0)).curr){if(olds.curr == (_tag_dyneither(0,0,0)).curr)return
_tag_dyneither(0,0,0);s=olds;}{unsigned long inc=Cyc_strspn((struct _dyneither_ptr)
s,delim);if(inc >= _get_dyneither_size(s,sizeof(char)) || *((char*)
_check_dyneither_subscript(_dyneither_ptr_plus(s,sizeof(char),(int)inc),sizeof(
char),0))== '\000'){olds=_tag_dyneither(0,0,0);return _tag_dyneither(0,0,0);}
else{_dyneither_ptr_inplace_plus(& s,sizeof(char),(int)inc);}token=s;s=Cyc_mstrpbrk(
...
unsigned long len1,struct _dyneither_ptr s2,unsigned long len2){unsigned long
min_length=len1 < len2?len1: len2;min_length <= _get_dyneither_size(s1,sizeof(char))
...
_tag_dyneither(_tmp5B,sizeof(char),11);}),659);{int i=- 1;while((++ i,i < min_length)){
...
_tag_dyneither(_tmp5D,sizeof(char),11);}),686);{int i=0;for(0;i < bound;++ i){int
...
unsigned long n){unsigned long len1=Cyc_strlen(s1);unsigned long len2=Cyc_strlen(s2);
return Cyc_caseless_ncmp(s1,len1,s2,len2,n);}void*memcpy(void*,const void*,
unsigned long n);void*memmove(void*,const void*,unsigned long n);int memcmp(const
void*,const void*,unsigned long n);char*memchr(const char*,char c,unsigned long n);
void*memset(void*,int c,unsigned long n);void bcopy(const void*src,void*dest,
unsigned long n);void bzero(void*s,unsigned long n);char*GC_realloc(char*,
unsigned long n);struct _dyneither_ptr Cyc_realloc(struct _dyneither_ptr s,
unsigned long n){char*_tmp5E=GC_realloc((char*)_check_null(_untag_dyneither_ptr(s,
sizeof(char),1)),n);return wrap_Cbuffer_as_buffer(_tmp5E,n);}struct _dyneither_ptr
Cyc__memcpy(struct _dyneither_ptr d,struct _dyneither_ptr s,unsigned long n,
unsigned int sz){if(((d.curr == (_tag_dyneither(0,0,0)).curr  || 
_get_dyneither_size(d,sizeof(void))< n) || s.curr == (_tag_dyneither(0,0,0)).curr)
 || _get_dyneither_size(s,sizeof(void))< n)(int)_throw((void*)({struct Cyc_Core_Invalid_argument_struct*
_tmp5F=_cycalloc(sizeof(*_tmp5F));_tmp5F[0]=({struct Cyc_Core_Invalid_argument_struct
_tmp60;_tmp60.tag=Cyc_Core_Invalid_argument;_tmp60.f1=({const char*_tmp61="memcpy";
_tag_dyneither(_tmp61,sizeof(char),7);});_tmp60;});_tmp5F;}));memcpy((void*)
...
d.curr == (_tag_dyneither(0,0,0)).curr  || _get_dyneither_size(d,sizeof(void))< n)
 || s.curr == (_tag_dyneither(0,0,0)).curr) || _get_dyneither_size(s,sizeof(void))
< n)(int)_throw((void*)({struct Cyc_Core_Invalid_argument_struct*_tmp62=_cycalloc(
sizeof(*_tmp62));_tmp62[0]=({struct Cyc_Core_Invalid_argument_struct _tmp63;_tmp63.tag=
Cyc_Core_Invalid_argument;_tmp63.f1=({const char*_tmp64="memove";_tag_dyneither(
_tmp64,sizeof(char),7);});_tmp63;});_tmp62;}));memmove((void*)_check_null(
...
_dyneither_ptr s1,struct _dyneither_ptr s2,unsigned long n){if(((s1.curr == (
...
>= n)(int)_throw((void*)({struct Cyc_Core_Invalid_argument_struct*_tmp65=
_cycalloc(sizeof(*_tmp65));_tmp65[0]=({struct Cyc_Core_Invalid_argument_struct
...
unsigned long n){if(s.curr == (_tag_dyneither(0,0,0)).curr  || n > 
...
0,0,0)).curr  || _get_dyneither_size(s,sizeof(char))< n)(int)_throw((void*)({
...
_tmp7A;});_tmp79;}));((void(*)(char*s,unsigned long n))bzero)((char*)_check_null(
_untag_dyneither_ptr(s,sizeof(char),1)),n);}void Cyc__bcopy(struct _dyneither_ptr
src,struct _dyneither_ptr dst,unsigned long n,unsigned int sz){if(((src.curr == (
