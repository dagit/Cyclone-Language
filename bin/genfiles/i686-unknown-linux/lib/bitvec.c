...
_dynforward_ptr bvec,int pos){int word=pos >> 5;int offset=pos & 31;return(*((int*)
_check_dynforward_subscript(bvec,sizeof(int),word))>> offset & 1)== 1;}void Cyc_Bitvec_set(
...
_check_dynforward_subscript(bvec,sizeof(int),word))| 1 << offset;}void Cyc_Bitvec_clear(
struct _dynforward_ptr bvec,int pos){int word=pos >> 5;int offset=pos & 31;*((int*)
_check_dynforward_subscript(bvec,sizeof(int),word))=*((int*)
_check_dynforward_subscript(bvec,sizeof(int),word))& ~(1 << offset);}int Cyc_Bitvec_get_and_set(
struct _dynforward_ptr bvec,int pos){int word=pos >> 5;int offset=pos & 31;int slot=*((
int*)_check_dynforward_subscript(bvec,sizeof(int),word));int ans=(slot >> offset & 
1)== 1;if(!ans)((int*)bvec.curr)[word]=slot | 1 << offset;return ans;}void Cyc_Bitvec_union_two(
struct _dynforward_ptr dest,struct _dynforward_ptr src1,struct _dynforward_ptr src2){
...
char*_tmp1="bitvec.cyc";_tag_dynforward(_tmp1,sizeof(char),
_get_zero_arr_size_char(_tmp1,11));}),71);{int i=0;for(0;i < len;++ i){((int*)dest.curr)[
i]=*((int*)_check_dynforward_subscript(src1,sizeof(int),i))| *((int*)
...
char*_tmp3="bitvec.cyc";_tag_dynforward(_tmp3,sizeof(char),
_get_zero_arr_size_char(_tmp3,11));}),78);{int i=0;for(0;i < len;++ i){((int*)dest.curr)[
i]=*((int*)_check_dynforward_subscript(src1,sizeof(int),i))& *((int*)
...
_tag_dynforward(_tmp4,sizeof(char),_get_zero_arr_size_char(_tmp4,37));}),({const
char*_tmp5="bitvec.cyc";_tag_dynforward(_tmp5,sizeof(char),
_get_zero_arr_size_char(_tmp5,11));}),85);{int i=0;for(0;i < len;++ i){((int*)dest.curr)[
i]=*((int*)_check_dynforward_subscript(src1,sizeof(int),i))& ~(*((int*)
...
_tag_dynforward(_tmp6,sizeof(char),_get_zero_arr_size_char(_tmp6,17));}),({const
char*_tmp7="bitvec.cyc";_tag_dynforward(_tmp7,sizeof(char),
_get_zero_arr_size_char(_tmp7,11));}),92);{int i=0;for(0;i < len;++ i){if(*((int*)
...
_tmpC;unsigned int i;for(i=0;i < _tmpE;i ++){_tmpD[i]=-1;}}_tmpF;});return ans;}
...
unsigned int i;for(i=0;i < _tmp12;i ++){_tmp11[i]=0;}}_tmp13;});for(0;l != 0;l=l->tl){
Cyc_Bitvec_set(ans,f(Cyc_Dict_lookup(d,(void*)l->hd)));}return ans;}struct Cyc_List_List*
Cyc_Bitvec_to_sorted_list(struct _dynforward_ptr bvec,int sz){struct Cyc_List_List*
ans=0;{int pos=sz - 1;for(0;pos >= 0;0){int word=pos >> 5;int bits=*((int*)
_check_dynforward_subscript(bvec,sizeof(int),word));int offset=pos & 31;for(0;
...
Cyc_Bitvec_clear_all(struct _dynforward_ptr bvec){unsigned int len=
_get_dynforward_size(bvec,sizeof(int));int i=0;for(0;i < len;++ i){((int*)bvec.curr)[
i]=0;}}void Cyc_Bitvec_set_all(struct _dynforward_ptr bvec){unsigned int len=
_get_dynforward_size(bvec,sizeof(int));int i=0;for(0;i < len;++ i){((int*)bvec.curr)[
i]=-1;}}int Cyc_Bitvec_all_set(struct _dynforward_ptr bvec,int sz){int words=sz >> 5;
words < _get_dynforward_size(bvec,sizeof(int))?0:((int(*)(struct _dynforward_ptr
...
_tmp18[1]={& _tmp1A};Cyc_printf(({const char*_tmp19="%d";_tag_dynforward(_tmp19,
sizeof(char),_get_zero_arr_size_char(_tmp19,3));}),_tag_dynforward(_tmp18,
sizeof(void*),1));}});}}({void*_tmp1B=0;Cyc_printf(({const char*_tmp1C="\n";
_tag_dynforward(_tmp1C,sizeof(char),_get_zero_arr_size_char(_tmp1C,2));}),
_tag_dynforward(_tmp1B,sizeof(void*),0));});}
