...
src2); struct Cyc_Std___sFILE; extern unsigned char Cyc_Std_FileCloseError[ 19u];
...
void Cyc_Bitvec_set( struct _tagged_arr bvec, int pos){ int word= pos >>  5; int
offset= pos &  31;*(( int*) _check_unknown_subscript( bvec, sizeof( int), word))=*((
int*) _check_unknown_subscript( bvec, sizeof( int), word)) |  1 <<  offset;}
void Cyc_Bitvec_clear( struct _tagged_arr bvec, int pos){ int word= pos >>  5;
int offset= pos &  31;*(( int*) _check_unknown_subscript( bvec, sizeof( int),
word))=*(( int*) _check_unknown_subscript( bvec, sizeof( int), word)) &  ~( 1 << 
offset);} int Cyc_Bitvec_get_and_set( struct _tagged_arr bvec, int pos){ int
word= pos >>  5; int offset= pos &  31; int slot=*(( int*)
_check_unknown_subscript( bvec, sizeof( int), word)); int ans=( slot >>  offset
&  1) ==  1; if( ! ans){*(( int*) _check_unknown_subscript( bvec, sizeof( int),
word))= slot |  1 <<  offset;} return ans;} void Cyc_Bitvec_union_two( struct
_tagged_arr dest, struct _tagged_arr src1, struct _tagged_arr src2){ int len=(
int) _get_arr_size( dest, sizeof( int)); int i= 0; for( 0; i <  len; ++ i){*((
...
_temp9[ i]= 0;}}; _temp11;}); for( 0; l !=  0; l=(( struct Cyc_List_List*)
...
int*) _check_unknown_subscript( bvec, sizeof( int), word)); int offset= pos & 
31; for( 0; offset >=  0;( -- offset, -- pos)){ if(( bits >>  offset &  1) ==  1){
ans=({ struct Cyc_List_List* _temp12=( struct Cyc_List_List*) _cycalloc( sizeof(
struct Cyc_List_List) *  1); _temp12[ 0]=({ struct Cyc_List_List _temp13;
_temp13.hd=( void*) pos; _temp13.tl= ans; _temp13;}); _temp12;});}}}} return ans;}
void Cyc_Bitvec_clear_all( struct _tagged_arr bvec){ int len=( int)
...
int), i))= -1;}} int Cyc_Bitvec_all_set( struct _tagged_arr bvec, int sz){ int
words= sz >>  5;{ int i= 0; for( 0; i <  words; i ++){ if(*(( int*)
