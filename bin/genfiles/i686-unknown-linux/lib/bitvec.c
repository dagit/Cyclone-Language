...
_get_zero_arr_size(_tmp16,18));}),({const char*_tmp17="bitvec.cyc";
_tag_dynforward(_tmp17,sizeof(char),_get_zero_arr_size(_tmp17,11));}),149);{int i=
0;for(0;i < words;i ++){if(*((int*)_check_dynforward_subscript(bvec,sizeof(int),i))
!= -1)return 0;}}{int i=words * 32;for(0;i < sz;i ++){if(!Cyc_Bitvec_get(bvec,i))
return 0;}}return 1;}void Cyc_Bitvec_print_bvec(struct _dynforward_ptr bvec){{int i=0;
for(0;i < 32 * _get_dynforward_size(bvec,sizeof(int));++ i){({struct Cyc_Int_pa_struct
...
sizeof(char),_get_zero_arr_size(_tmp19,3));}),_tag_dynforward(_tmp18,sizeof(void*),
1));}});}}({void*_tmp1B[0]={};Cyc_printf(({const char*_tmp1C="\n";_tag_dynforward(
_tmp1C,sizeof(char),_get_zero_arr_size(_tmp1C,2));}),_tag_dynforward(_tmp1B,
sizeof(void*),0));});}
