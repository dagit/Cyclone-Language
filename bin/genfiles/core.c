...
void*v;};typedef struct Cyc_Core_Opt*Cyc_Core_opt_t;struct Cyc_Core_Opt*Cyc_Core_opt_map(
...
extern struct _RegionHandle*Cyc_Core_unique_region;struct Cyc_Core_NewRegion Cyc_Core__rnew_dynregion(
...
int f1;};char Cyc_Core_Failure[8]="Failure";char Cyc_Core_Impossible[11]="Impossible";
char Cyc_Core_Not_found[10]="Not_found";struct Cyc_Core_Not_found_exn_struct Cyc_Core_Not_found_val={
...
== 0)return 0;{struct Cyc_Core_Opt*_tmp6;return(_tmp6=_cycalloc(sizeof(*_tmp6)),((
_tmp6->v=(void*)f((void*)o->v),_tmp6)));};}struct _dyneither_ptr Cyc_Core_new_string(
...
_tmp9),_tag_dyneither(_tmpA,sizeof(char),_tmp9))));}struct Cyc_Core_NewRegion Cyc_Core__new_dynregion(
const char*file,int lineno){return Cyc_Core__rnew_dynregion(Cyc_Core_heap_region,
file,lineno);}int Cyc_Core_true_f(void*x){return 1;}int Cyc_Core_false_f(void*x){
return 0;}int Cyc_Core_intcmp(int a,int b){return a - b;}int Cyc_Core_charcmp(char a,
char b){return(int)a - (int)b;}int Cyc_Core_nptrcmp(void*a,void*b){if(a == b)return 0;
if(a > b)return 1;return - 1;}int Cyc_Core_ptrcmp(void*a,void*b){if(a == b)return 0;if(
a > b)return 1;return - 1;}void*Cyc_Core_fst(struct _tuple0*pair){return(*pair).f1;}
