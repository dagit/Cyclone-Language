...
if(Cyc_AP_cmp(q->d,Cyc_AP_zero)< 0){
...
if(Cyc_AP_cmp(q->d,Cyc_AP_one)==0){
...
struct Cyc_AP_T*_Tmp0=Cyc_AP_neg(q->n);return Cyc_APQ_fromAP(_Tmp0,q->d);}
# 68
struct Cyc_APQ_T*Cyc_APQ_abs(struct Cyc_APQ_T*q){
struct Cyc_AP_T*_Tmp0=Cyc_AP_abs(q->n);return Cyc_APQ_fromAP(_Tmp0,q->d);}
# 72
...
struct Cyc_AP_T*_Tmp0=Cyc_AP_mul(p->n,q->n);return Cyc_APQ_fromAP(_Tmp0,Cyc_AP_mul(p->d,q->d));}
# 90
struct Cyc_APQ_T*Cyc_APQ_div(struct Cyc_APQ_T*p,struct Cyc_APQ_T*q){
if(Cyc_AP_cmp(q->n,Cyc_AP_zero)==0)_throw((void*)({struct Cyc_Invalid_argument_exn_struct*_Tmp0=_cycalloc(sizeof(struct Cyc_Invalid_argument_exn_struct));_Tmp0->tag=Cyc_Invalid_argument,_Tmp0->f1=_tag_fat("APQ_div: divide by zero",sizeof(char),24U);_Tmp0;}));{
struct Cyc_AP_T*_Tmp0=Cyc_AP_mul(p->n,q->d);return Cyc_APQ_fromAP(_Tmp0,Cyc_AP_mul(p->d,q->n));}}
# 95
