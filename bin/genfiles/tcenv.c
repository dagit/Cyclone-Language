...
({struct Cyc_Tcenv_SharedFenv*_tmp13A=Cyc_Tcenv_new_shared_fenv((fd->i).ret_type);_tmpAF->shared=_tmp13A;}),_tmpAF->type_vars=_tmpA2,_tmpAF->region_order=_tmpA3,_tmpAF->ctrl_env=0,({
# 533
...
struct Cyc_List_List*_tmp137=({struct Cyc_List_List*_tmpAD=_cycalloc(sizeof(*_tmpAD));_tmpAD->hd=(void*)_check_null((fd->i).effect),_tmpAD->tl=0;_tmpAD;});_tmpAE->tl=_tmp137;});_tmpAE;}));
# 533
...
# 552
({void*_tmpBB=0U;({struct _dyneither_ptr _tmp13F=({const char*_tmpBC="non-intuitionistic tvar in nested_fenv";_tag_dyneither(_tmpBC,sizeof(char),39U);});((int(*)(struct _dyneither_ptr,struct _dyneither_ptr))Cyc_Tcutil_impos)(_tmp13F,_tag_dyneither(_tmpBB,sizeof(void*),0U));});});}}}}
# 554
...
struct Cyc_List_List*_tmpC0=({struct Cyc_List_List*_tmpCE=_cycalloc(sizeof(*_tmpCE));_tmpCE->hd=rgn0,({struct Cyc_List_List*_tmp140=((struct Cyc_List_List*(*)(struct Cyc_List_List*x,struct Cyc_List_List*y))Cyc_List_append)((fd->i).tvars,_tmpD5);_tmpCE->tl=_tmp140;});_tmpCE;});
...
({struct Cyc_Tcenv_SharedFenv*_tmp145=Cyc_Tcenv_new_shared_fenv((fd->i).ret_type);_tmpCC->shared=_tmp145;}),_tmpCC->type_vars=_tmpC0,_tmpCC->region_order=_tmpD6,_tmpCC->ctrl_env=0,({
# 578
...
struct Cyc_List_List*_tmp142=({struct Cyc_List_List*_tmpCA=_cycalloc(sizeof(*_tmpCA));_tmpCA->hd=(void*)_check_null((fd->i).effect),_tmpCA->tl=0;_tmpCA;});_tmpCB->tl=_tmp142;});_tmpCB;}));
# 578
