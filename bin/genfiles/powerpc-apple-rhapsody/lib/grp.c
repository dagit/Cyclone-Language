...
struct _dynforward_ptr name);struct Cyc_group*Cyc_getgrnam(struct _dynforward_ptr
name){struct Cyc_Cgrp_Cgroup*src=getgrnam(string_to_Cstring(name));struct Cyc_group*
_tmp2;return(unsigned int)src?(_tmp2=_cycalloc(sizeof(*_tmp2)),((_tmp2->gr_name=(
struct _dynforward_ptr)Cstring_to_string(src->gr_name),((_tmp2->gr_passwd=(struct
_dynforward_ptr)Cstring_to_string(src->gr_passwd),((_tmp2->gr_gid=src->gr_gid,((
...
sizeof(*_tmp3)),((_tmp3->gr_name=(struct _dynforward_ptr)Cstring_to_string(src->gr_name),((
_tmp3->gr_passwd=(struct _dynforward_ptr)Cstring_to_string(src->gr_passwd),((
_tmp3->gr_gid=src->gr_gid,((_tmp3->gr_mem=ntCsl_to_ntsl(src->gr_mem),_tmp3))))))))):
0;}int Cyc_initgroups(struct _dynforward_ptr user,unsigned int group);int Cyc_initgroups(
struct _dynforward_ptr user,unsigned int group){return initgroups(string_to_Cstring(
user),group);}int Cyc_setgroups(struct _dynforward_ptr groups);int Cyc_setgroups(
