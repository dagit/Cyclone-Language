...
size);int Cyc_Execinfo_backtrace(struct _dynforward_ptr array,int size){{struct Cyc_Core_Failure_struct
...
_tmp7="backtrace: unsupported except under Linux",_tag_dynforward(_tmp7,sizeof(
char),_get_zero_arr_size_char(_tmp7,42)))),_tmp8)))),_tmp6)))));}return 0;}int Cyc_Execinfo_bt();
