...
return topt == 0?Cyc_Absyn_new_evar(& Cyc_Tcutil_mko,({struct Cyc_Core_Opt*_tmp3C=_cycalloc(sizeof(*_tmp3C));_tmp3C->v=s;_tmp3C;})):*topt;}
# 127
...
void*_tmp45A=t;Cyc_Tcutil_promote_array(_tmp45A,rgn_opt == 0?Cyc_Absyn_new_evar(& Cyc_Tcutil_rko,({struct Cyc_Core_Opt*_tmp4A=_cycalloc(sizeof(*_tmp4A));({struct Cyc_List_List*_tmp459=Cyc_Tcenv_lookup_type_vars(te);_tmp4A->v=_tmp459;});_tmp4A;})): rgn_opt,0);}): t;}struct _tuple18{struct Cyc_Absyn_Tvar*f1;int f2;};
# 173
...
# 217
({unsigned _tmp470=p->loc;struct Cyc_Tcenv_Tenv*_tmp46F=te2;struct Cyc_List_List*_tmp46E=Cyc_Tcenv_lookup_type_vars(te2);Cyc_Tctyp_check_type(_tmp470,_tmp46F,_tmp46E,& Cyc_Tcutil_tmk,1,0,vd->type);});
# 220
