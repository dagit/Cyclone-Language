...
i;} if( i <  0){ return Cyc_Core_new_string( 0);} return Cyc_String_substring(
...
unsigned char)) -  1); while( i >=  0?*(( const unsigned char*)
_check_unknown_subscript( filename, sizeof( unsigned char), i)) != '/': 0) { --
i;} return Cyc_String_substring( filename, i +  1, _get_arr_size( filename,
sizeof( unsigned char)) - ( i +  1));} int Cyc_Filename_check_suffix( struct
_tagged_arr filename, struct _tagged_arr suffix){ int i=( int)( _get_arr_size(
filename, sizeof( unsigned char)) -  1); int j=( int)( _get_arr_size( suffix,
sizeof( unsigned char)) -  1); while( i >=  0? j >=  0: 0) { if(*(( const
unsigned char*) _check_unknown_subscript( filename, sizeof( unsigned char), i --))
!= *(( const unsigned char*) _check_unknown_subscript( suffix, sizeof(
unsigned char), j --))){ return 0;}} if( j >=  0){ return 0;} else{ return 1;}}
struct _tagged_arr Cyc_Filename_gnuify( struct _tagged_arr filename){ int
has_drive_name= _get_arr_size( filename, sizeof( unsigned char)) >  1?*(( const
unsigned char*) _check_unknown_subscript( filename, sizeof( unsigned char), 1))
== ':': 0; int i; int j; struct _tagged_arr ans; int ans_sz; if( has_drive_name){
ans_sz=( int)( _get_arr_size( filename, sizeof( unsigned char)) +  1); ans=({
unsigned int _temp2=( unsigned int) ans_sz; unsigned char* _temp3=(
unsigned char*) GC_malloc_atomic( sizeof( unsigned char) *  _temp2); struct
_tagged_arr _temp5= _tag_arr( _temp3, sizeof( unsigned char),( unsigned int)
ans_sz);{ unsigned int _temp4= _temp2; unsigned int k; for( k= 0; k <  _temp4; k
++){ _temp3[ k]='\000';}}; _temp5;});*(( unsigned char*)
