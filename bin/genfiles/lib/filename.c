...
_temp9.curr + i; if( _temp9.base == 0? 1:( _temp11 < _temp9.base? 1: _temp11 >=
_temp9.last_plus_one)){ _throw( Null_Exception);}* _temp11;}) !=( int)'.': 0) {
-- i;} if( i < 0){( void) _throw(( struct _xtunion_struct*)({ struct Cyc_Core_InvalidArg_struct*
...
_tagged_string _temp17= filename; char* _temp19= _temp17.curr + i; if( _temp17.base
== 0? 1:( _temp19 < _temp17.base? 1: _temp19 >= _temp17.last_plus_one)){ _throw(
Null_Exception);}* _temp19;}) !=( int)'/': 0) { -- i;} if( i < 0){ return(
struct _tagged_string)({ char* _temp20=( char*)""; struct _tagged_string _temp21;
_temp21.curr= _temp20; _temp21.base= _temp20; _temp21.last_plus_one= _temp20 + 1;
_temp21;});} return Cyc_String_substring( filename, 0,( unsigned int) i);}
struct _tagged_string Cyc_Filename_basename( struct _tagged_string filename){
int i=( int)(({ struct _tagged_string _temp22= filename;( unsigned int)( _temp22.last_plus_one
- _temp22.curr);}) -( unsigned int) 1); while( i >= 0?( int)({ struct
_tagged_string _temp23= filename; char* _temp25= _temp23.curr + i; if( _temp23.base
== 0? 1:( _temp25 < _temp23.base? 1: _temp25 >= _temp23.last_plus_one)){ _throw(
Null_Exception);}* _temp25;}) !=( int)'/': 0) { -- i;} return Cyc_String_substring(
filename, i + 1,({ struct _tagged_string _temp26= filename;( unsigned int)(
_temp26.last_plus_one - _temp26.curr);}) -( unsigned int)( i + 1));} int Cyc_Filename_check_suffix(
...
_temp27.curr);}) -( unsigned int) 1); int j=( int)(({ struct _tagged_string
_temp28= suffix;( unsigned int)( _temp28.last_plus_one - _temp28.curr);}) -(
unsigned int) 1); while( i >= 0? j >= 0: 0) { if(( int)({ struct _tagged_string
_temp29= filename; char* _temp31= _temp29.curr +( i --); if( _temp29.base == 0?
1:( _temp31 < _temp29.base? 1: _temp31 >= _temp29.last_plus_one)){ _throw(
Null_Exception);}* _temp31;}) !=( int)({ struct _tagged_string _temp32= suffix;
char* _temp34= _temp32.curr +( j --); if( _temp32.base == 0? 1:( _temp34 <
_temp32.base? 1: _temp34 >= _temp32.last_plus_one)){ _throw( Null_Exception);}*
_temp34;})){ return 0;}} if( j >= 0){ return 0;} else{ return 1;}} struct
_tagged_string Cyc_Filename_gnuify( struct _tagged_string filename){ int
has_drive_name=({ struct _tagged_string _temp35= filename;( unsigned int)(
...
Null_Exception);}* _temp38;}) ==( int)':': 0; int i; int j; struct
_tagged_string ans; int ans_sz; if( has_drive_name){ ans_sz=( int)(({ struct
_tagged_string _temp39= filename;( unsigned int)( _temp39.last_plus_one -
_temp39.curr);}) +( unsigned int) 1); ans=({ unsigned int _temp40=( unsigned int)
ans_sz; char* _temp41=( char*) GC_malloc_atomic( sizeof( char) * _temp40);
unsigned int k; struct _tagged_string _temp42={ _temp41, _temp41, _temp41 +
_temp40}; for( k= 0; k < _temp40; k ++){ _temp41[ k]='\000';} _temp42;});({
struct _tagged_string _temp43= ans; char* _temp45= _temp43.curr + 0; if( _temp43.base
== 0? 1:( _temp45 < _temp43.base? 1: _temp45 >= _temp43.last_plus_one)){ _throw(
Null_Exception);}* _temp45=({ struct _tagged_string _temp46= ans; char* _temp48=
_temp46.curr + 1; if( _temp46.base == 0? 1:( _temp48 < _temp46.base? 1: _temp48
>= _temp46.last_plus_one)){ _throw( Null_Exception);}* _temp48='/';});});({
struct _tagged_string _temp49= ans; char* _temp51= _temp49.curr + 2; if( _temp49.base
...
_temp54= _temp52.curr + 0; if( _temp52.base == 0? 1:( _temp54 < _temp52.base? 1:
_temp54 >= _temp52.last_plus_one)){ _throw( Null_Exception);}* _temp54;});}); i=
3; j= 2;} else{ ans_sz=( int)({ struct _tagged_string _temp55= filename;(
unsigned int)( _temp55.last_plus_one - _temp55.curr);}); ans=({ unsigned int
_temp56=( unsigned int) ans_sz; char* _temp57=( char*) GC_malloc_atomic( sizeof(
char) * _temp56); unsigned int k; struct _tagged_string _temp58={ _temp57,
_temp57, _temp57 + _temp56}; for( k= 0; k < _temp56; k ++){ _temp57[ k]='\000';}
_temp58;}); i= 0; j= 0;} while( i < ans_sz) { char c=({ struct _tagged_string
_temp59= filename; char* _temp61= _temp59.curr +( j ++); if( _temp59.base == 0?
1:( _temp61 < _temp59.base? 1: _temp61 >= _temp59.last_plus_one)){ _throw(
Null_Exception);}* _temp61;});({ struct _tagged_string _temp62= ans; char*
_temp64= _temp62.curr +( i ++); if( _temp62.base == 0? 1:( _temp64 < _temp62.base?
1: _temp64 >= _temp62.last_plus_one)){ _throw( Null_Exception);}* _temp64=( int)
c ==( int)'\\'?'/': c;});} return ans;}