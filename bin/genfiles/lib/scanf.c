...
unsigned char*); extern int* __errno(); extern int isalnum( int __c); extern int
isalpha( int __c); extern int iscntrl( int __c); extern int isdigit( int __c);
extern int isgraph( int __c); extern int islower( int __c); extern int isprint(
int __c); extern int ispunct( int __c); extern int isspace( int __c); extern int
isupper( int __c); extern int isxdigit( int __c); extern int tolower( int __c);
extern int toupper( int __c); extern int isascii( int __c); extern int toascii(
int __c); extern int _tolower( int __c); extern int _toupper( int __c); struct
Cyc_Time_tm{ int tm_sec; int tm_min; int tm_hour; int tm_mday; int tm_mon; int
tm_year; int tm_wday; int tm_yday; int tm_isdst; } ; extern unsigned int time(
unsigned int* t); struct Cyc___sFILE; struct Cyc__reent; struct Cyc__glue{
...
void* _cookie, unsigned char* _buf, int _n); int(* _write)( void* _cookie, const
unsigned char* _buf, int _n); int(* _seek)( void* _cookie, int _offset, int
_whence); int(* _close)( void* _cookie); struct Cyc___sbuf _ub; unsigned char*
...
extern double Cyc_Stdlib_atof( struct _tagged_arr); extern int Cyc_Stdlib_strtol(
...
extern int Cyc_Stdio_fgetc( struct Cyc_Stdio___sFILE*); extern int Cyc_Stdio_getc(
struct Cyc_Stdio___sFILE*); extern int Cyc_Stdio_ungetc( int, struct Cyc_Stdio___sFILE*);
...
void* a){ void* _temp38= a; double* _temp44; _LL40: if(*(( int*) _temp38) == Cyc_Stdio_DoublePtr_sa){
_LL45: _temp44=(( struct Cyc_Stdio_DoublePtr_sa_struct*) _temp38)->f1; goto
_LL41;} else{ goto _LL42;} _LL42: goto _LL43; _LL41: return _temp44; _LL43:( int)
_throw(( void*)({ struct Cyc_Core_InvalidArg_struct* _temp46=( struct Cyc_Core_InvalidArg_struct*)
...
_temp46;})); _LL39:;} static float* Cyc_Stdio_va_arg_float_ptr( void* a){ void*
_temp48= a; float* _temp54; _LL50: if(*(( int*) _temp48) == Cyc_Stdio_FloatPtr_sa){
...
1){ seen_eof ++; break;}} sum ++; width --;} nread += sum;} else{ long long sum=(
long long) 0; struct _tagged_arr _temp96= Cyc_Stdio_va_arg_string_ptr(*(( void**)
...
sum; if( _get_arr_size( _temp96, sizeof( unsigned char)) != 0){*(( unsigned char*)
_check_unknown_subscript( _temp96, sizeof( unsigned char), 0u))='\000';}
...
_IO_getc( fp); if( -- width == 0){ break;} if(( c= _IO_peekc( fp)) == - 1){ if(
n == 0){ goto eof_failure;} seen_eof ++; break;}} if( n == 0){ goto
...
long long)( sizeof( buf) - 1);} flags |=( 64 | 128) | 512; for( p= _tag_arr( buf,
sizeof( unsigned char), 350u); width != 0; width --){ c=( int)(( unsigned char)
_IO_peekc( fp)); switch( c){ case '0': _LL104: if( base == 0){ base= 8; flags |=
256;} if( flags & 512){ flags &= ~(( 64 | 512) | 128);} else{ flags &= ~(( 64 |
256) | 128);} goto ok; case '1': _LL105: goto _LL106; case '2': _LL106: goto
_LL107; case '3': _LL107: goto _LL108; case '4': _LL108: goto _LL109; case '5':
_LL109: goto _LL110; case '6': _LL110: goto _LL111; case '7': _LL111: base=( int)
basefix[ _check_known_subscript_notnull( 17u, base)]; flags &= ~(( 64 | 256) |
128); goto ok; case '8': _LL112: goto _LL113; case '9': _LL113: base=( int)
basefix[ _check_known_subscript_notnull( 17u, base)]; if( base <= 8){ break;}
flags &= ~(( 64 | 256) | 128); goto ok; case 'A': _LL114: goto _LL115; case 'B':
_LL115: goto _LL116; case 'C': _LL116: goto _LL117; case 'D': _LL117: goto
_LL118; case 'E': _LL118: goto _LL119; case 'F': _LL119: goto _LL120; case 'a':
_LL120: goto _LL121; case 'b': _LL121: goto _LL122; case 'c': _LL122: goto
_LL123; case 'd': _LL123: goto _LL124; case 'e': _LL124: goto _LL125; case 'f':
_LL125: if( base <= 10){ break;} flags &= ~(( 64 | 256) | 128); goto ok; case '+':
_LL126: goto _LL127; case '-': _LL127: if( flags & 64){ flags &= ~ 64; goto ok;}
break; case 'x': _LL128: goto _LL129; case 'X': _LL129: if( flags & 256? p.curr
==( _tagged_arr_plus( _tag_arr( buf, sizeof( unsigned char), 350u), sizeof(
unsigned char), 1)).curr: 0){ base= 16; flags &= ~ 256; goto ok;} break;
default: _LL130: break;} break; ok:*(( unsigned char*) _check_unknown_subscript(
...
1){ seen_eof ++; break;}} if( flags & 128){ if( p.curr >( _tag_arr( buf, sizeof(
unsigned char), 350u)).curr){ _tagged_arr_inplace_plus(& p, sizeof(
unsigned char), -1); _IO_ungetc(( int)*(( unsigned char*)
...
unsigned char), -1)); if( c =='x'? 1: c =='X'){ _tagged_arr_inplace_plus(& p,
sizeof( unsigned char), -1); _IO_ungetc( c, fp);} if(( flags & 8) == 0){
unsigned int res;*(( unsigned char*) _check_unknown_subscript( p, sizeof(
unsigned char), 0u))='\000'; if( use_strtoul){ res= Cyc_Stdlib_strtoul( _tag_arr(
buf, sizeof( unsigned char), 350u), 0, base);} else{ res=( unsigned int) Cyc_Stdlib_strtol(
_tag_arr( buf, sizeof( unsigned char), 350u), 0, base);} if( flags & 16){* Cyc_Stdio_va_arg_int_ptr(*((
void**) _check_unknown_subscript( ap, sizeof( void*), 0u)))=( int) res;} else{
if( flags & 4){* Cyc_Stdio_va_arg_short_ptr(*(( void**) _check_unknown_subscript(
ap, sizeof( void*), 0u)))=( short) res;} else{ if( flags & 1){* Cyc_Stdio_va_arg_int_ptr(*((
void**) _check_unknown_subscript( ap, sizeof( void*), 0u)))=( int) res;} else{*
Cyc_Stdio_va_arg_int_ptr(*(( void**) _check_unknown_subscript( ap, sizeof( void*),
...
sizeof( buf) - 1){ width=( long long)( sizeof( buf) - 1);} flags |=(( 64 | 128)
| 256) | 512; for( p= _tag_arr( buf, sizeof( unsigned char), 350u); width != 0;
width --){ c= _IO_peekc( fp); switch( c){ case '0': _LL133: goto _LL134; case '1':
_LL134: goto _LL135; case '2': _LL135: goto _LL136; case '3': _LL136: goto
_LL137; case '4': _LL137: goto _LL138; case '5': _LL138: goto _LL139; case '6':
_LL139: goto _LL140; case '7': _LL140: goto _LL141; case '8': _LL141: goto
_LL142; case '9': _LL142: flags &= ~( 64 | 128); goto fok; case '+': _LL143:
goto _LL144; case '-': _LL144: if( flags & 64){ flags &= ~ 64; goto fok;} break;
case '.': _LL145: if( flags & 256){ flags &= ~( 64 | 256); goto fok;} break;
case 'e': _LL146: goto _LL147; case 'E': _LL147: if(( flags &( 128 | 512)) ==
512){ flags=( flags & ~( 512 | 256) | 64) | 128; goto fok;} break; default:
_LL148: break;} break; fok:*(( unsigned char*) _check_unknown_subscript(
_tagged_arr_inplace_plus_post(& p, sizeof( unsigned char), 1), sizeof(
unsigned char), 0u))=( unsigned char) c; _IO_getc( fp); if( _IO_peekc( fp) == -
...
p, sizeof( unsigned char), 0u))='\000'; res= Cyc_Stdlib_atof( _tag_arr( buf,
sizeof( unsigned char), 350u)); if( flags & 1){* Cyc_Stdio_va_arg_double_ptr(*((
void**) _check_unknown_subscript( ap, sizeof( void*), 0u)))= res;} else{* Cyc_Stdio_va_arg_float_ptr(*((
