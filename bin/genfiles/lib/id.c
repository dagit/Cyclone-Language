...
struct _tagged_string, struct _tagged_string); extern struct _tagged_string Cyc_String_strconcat_l(
struct Cyc_List_List*); extern struct _tagged_string Cyc_String_strdup( struct
_tagged_string src); struct Cyc_Id_id; typedef struct Cyc_Id_id* Cyc_Id_Id;
extern struct Cyc_Id_id* Cyc_Id_id_make( struct _tagged_string, int); extern
struct Cyc_Id_id* Cyc_Id_id_new( struct _tagged_string); extern struct Cyc_Id_id*
Cyc_Id_id_renew( struct Cyc_Id_id*); extern struct Cyc_Id_id* Cyc_Id_id_of_string(
struct _tagged_string); extern struct Cyc_Id_id* Cyc_Id_id_unique( struct
_tagged_string); extern struct _tagged_string Cyc_Id_id_to_string( struct Cyc_Id_id*);
extern void Cyc_Id_id_prn( struct Cyc_Stdio___sFILE*, struct Cyc_Id_id*); extern
struct _tagged_string Cyc_Id_id_to_source( struct Cyc_Id_id*); extern int Cyc_Id_id_compare(
struct Cyc_Id_id*, struct Cyc_Id_id*); struct Cyc_Id_id{ struct _tagged_string s;
int i; } ; struct Cyc_Id_id* Cyc_Id_id_make( struct _tagged_string s, int i){
...
0; struct Cyc_Id_id* Cyc_Id_id_new( struct _tagged_string s){ ++ Cyc_Id_counter;
...
struct Cyc_Id_id* Cyc_Id_id_of_string( struct _tagged_string s){ return Cyc_Id_id_make(
s, - 1);} struct Cyc_Id_id* Cyc_Id_id_unique( struct _tagged_string s){ return
Cyc_Id_id_new( s);} struct _tagged_string Cyc_Id_id_to_string( struct Cyc_Id_id*
x){ if( x->i == - 1){ return Cyc_String_strdup( x->s);} return Cyc_String_strconcat_l(({
struct Cyc_List_List* _temp1=( struct Cyc_List_List*) GC_malloc( sizeof( struct
Cyc_List_List)); _temp1->hd=( void*)({ struct _tagged_string* _temp8=( struct
_tagged_string*) GC_malloc( sizeof( struct _tagged_string) * 1); _temp8[ 0]= x->s;
_temp8;}); _temp1->tl=({ struct Cyc_List_List* _temp2=( struct Cyc_List_List*)
...
_tagged_string)); _temp5[ 0]=({ unsigned char* _temp6="$"; struct _tagged_string
_temp7; _temp7.curr= _temp6; _temp7.base= _temp6; _temp7.last_plus_one= _temp6 +
2; _temp7;}); _temp5;}); _temp2->tl=({ struct Cyc_List_List* _temp3=( struct Cyc_List_List*)
GC_malloc( sizeof( struct Cyc_List_List)); _temp3->hd=( void*)({ struct
_tagged_string* _temp4=( struct _tagged_string*) GC_malloc( sizeof( struct
_tagged_string) * 1); _temp4[ 0]= Cyc_Core_string_of_int( x->i); _temp4;});
_temp3->tl= 0; _temp3;}); _temp2;}); _temp1;}));} void Cyc_Id_id_prn( struct Cyc_Stdio___sFILE*
f, struct Cyc_Id_id* x){({ struct _tagged_string _temp9= Cyc_Id_id_to_string( x);
fprintf( sfile_to_file( f),"%.*s", _temp9.last_plus_one - _temp9.curr, _temp9.curr);});}
struct _tagged_string Cyc_Id_id_to_source( struct Cyc_Id_id* x){ return x->s;}
int Cyc_Id_id_compare( struct Cyc_Id_id* x1, struct Cyc_Id_id* x2){ if( x1->i ==
x2->i){ return Cyc_String_zstrcmp( x1->s, x2->s);} else{ if( x1->i < x2->i){
