...
int tag; float* f1; } ; struct Cyc_Cstdio___sFILE; extern int remove(
...
unsigned char*, int, int); extern int fgetc( struct Cyc_Cstdio___sFILE*); extern
unsigned char* fgets( unsigned char*, int, struct Cyc_Cstdio___sFILE*); extern
int fputc( int, struct Cyc_Cstdio___sFILE*); extern int fputs( unsigned char*,
struct Cyc_Cstdio___sFILE*); extern int getc( struct Cyc_Cstdio___sFILE*);
...
extern int fread( unsigned char*, int _size, int _n, struct Cyc_Cstdio___sFILE*);
extern int fwrite( unsigned char*, int _size, int _n, struct Cyc_Cstdio___sFILE*);
extern int fgetpos( struct Cyc_Cstdio___sFILE*, int*); extern int fseek( struct
Cyc_Cstdio___sFILE*, int, int); extern int fsetpos( struct Cyc_Cstdio___sFILE*,
int*); extern int ftell( struct Cyc_Cstdio___sFILE*); extern void rewind( struct
Cyc_Cstdio___sFILE*); extern void clearerr( struct Cyc_Cstdio___sFILE*); extern
int feof( struct Cyc_Cstdio___sFILE*); extern int ferror( struct Cyc_Cstdio___sFILE*);
extern void perror( unsigned char*); extern struct Cyc_Cstdio___sFILE* fopen(
unsigned char* _name, unsigned char* _type); extern struct Cyc_Cstdio___sFILE*
fdopen( int, unsigned char*); extern int fileno( struct Cyc_Cstdio___sFILE*);
extern int getw( struct Cyc_Cstdio___sFILE*); extern int putw( int, struct Cyc_Cstdio___sFILE*);
...
struct Cyc_Stdio___sFILE)); _temp1->file= cf; _temp1;}): 0;} int Cyc_Stdio_fgetc(
...
_get_arr_size( s, sizeof( unsigned char)); n=( int)( len < n? len:( unsigned int)
n); result= fgets( buffer, n,( struct Cyc_Cstdio___sFILE*) _check_null( f->file));
if( result == 0){ return _tag_arr( 0u, 0u, 0u);} else{ return s;}} int Cyc_Stdio_fputc(
int i, struct Cyc_Stdio___sFILE* f){ return fputc( i,( struct Cyc_Cstdio___sFILE*)
_check_null( f->file));} int Cyc_Stdio_fputs( struct _tagged_arr s, struct Cyc_Stdio___sFILE*
f){ return fputs( string_to_Cstring( s),( struct Cyc_Cstdio___sFILE*)
_check_null( f->file));} int Cyc_Stdio_getc( struct Cyc_Stdio___sFILE* f){
return getc(( struct Cyc_Cstdio___sFILE*) _check_null( f->file));} int Cyc_Stdio_putc(
int i, struct Cyc_Stdio___sFILE* f){ return putc( i,( struct Cyc_Cstdio___sFILE*)
_check_null( f->file));} int Cyc_Stdio_puts( struct _tagged_arr s){ return puts(
string_to_Cstring( s));} int Cyc_Stdio_ungetc( int i, struct Cyc_Stdio___sFILE*
f){ return ungetc( i,( struct Cyc_Cstdio___sFILE*) _check_null( f->file));} int
Cyc_Stdio_fgetpos( struct Cyc_Stdio___sFILE* f, int* x){ return fgetpos(( struct
Cyc_Cstdio___sFILE*) _check_null( f->file), x);} int Cyc_Stdio_fseek( struct Cyc_Stdio___sFILE*
f, int offset, int whence){ return fseek(( struct Cyc_Cstdio___sFILE*)
