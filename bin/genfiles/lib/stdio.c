...
struct Cyc_Stdio___sFILE*); extern int remove( unsigned char*); extern int
rename( unsigned char*, unsigned char*); extern struct Cyc_Stdio___sFILE*
tmpfile(); extern unsigned char* tmpnam( unsigned char*); extern int fclose(
struct Cyc_Stdio___sFILE*); extern int fflush( struct Cyc_Stdio___sFILE*);
extern struct Cyc_Stdio___sFILE* freopen( unsigned char*, unsigned char*, struct
Cyc_Stdio___sFILE*); extern void setbuf( struct Cyc_Stdio___sFILE*,
unsigned char*); extern int setvbuf( struct Cyc_Stdio___sFILE*, unsigned char*,
int, unsigned int); extern int fgetc( struct Cyc_Stdio___sFILE*); extern
unsigned char* fgets( unsigned char*, int, struct Cyc_Stdio___sFILE*); extern
int fputc( int, struct Cyc_Stdio___sFILE*); extern int fputs( unsigned char*,
struct Cyc_Stdio___sFILE*); extern int getc( struct Cyc_Stdio___sFILE*); extern
unsigned char* gets( unsigned char*); extern int putc( int, struct Cyc_Stdio___sFILE*);
extern int puts( unsigned char*); extern int ungetc( int, struct Cyc_Stdio___sFILE*);
extern unsigned int fread( unsigned char*, unsigned int _size, unsigned int _n,
struct Cyc_Stdio___sFILE*); extern unsigned int fwrite( unsigned char*,
unsigned int _size, unsigned int _n, struct Cyc_Stdio___sFILE*); extern int
fgetpos( struct Cyc_Stdio___sFILE*, int*); extern int fseek( struct Cyc_Stdio___sFILE*,
int, int); extern int fsetpos( struct Cyc_Stdio___sFILE*, int*); extern int
ftell( struct Cyc_Stdio___sFILE*); extern void rewind( struct Cyc_Stdio___sFILE*);
extern void clearerr( struct Cyc_Stdio___sFILE*); extern int feof( struct Cyc_Stdio___sFILE*);
extern int ferror( struct Cyc_Stdio___sFILE*); extern void perror( unsigned char*);
extern struct Cyc_Stdio___sFILE* fopen( unsigned char* _name, unsigned char*
_type); extern struct Cyc_Stdio___sFILE* fdopen( int, unsigned char*); extern
int fileno( struct Cyc_Stdio___sFILE*); extern int getw( struct Cyc_Stdio___sFILE*);
extern int putw( int, struct Cyc_Stdio___sFILE*); extern void setbuffer( struct
Cyc_Stdio___sFILE*, unsigned char*, int); extern int setlinebuf( struct Cyc_Stdio___sFILE*);
...
struct Cyc_Stdio___sFILE* f){ return fgetc( f);} struct _tagged_string Cyc_Stdio_fgets(
struct _tagged_string s, int n, struct Cyc_Stdio___sFILE* f){ unsigned char*
result; unsigned char* buffer= underlying_Cstring( s); unsigned int len=({
struct _tagged_string _temp1= s;( unsigned int)( _temp1.last_plus_one - _temp1.curr);});
n=( int)( len < n? len:( unsigned int) n); result= fgets( buffer, n, f); if(
result == 0){ return({ struct _tagged_string _temp0={ 0, 0, 0}; _temp0;});}
else{ return s;}} int Cyc_Stdio_fputc( int i, struct Cyc_Stdio___sFILE* f){
return fputc( i, f);} int Cyc_Stdio_fputs( struct _tagged_string s, struct Cyc_Stdio___sFILE*
f){ return fputs( string_to_Cstring( s), f);} int Cyc_Stdio_getc( struct Cyc_Stdio___sFILE*
f){ return getc( f);} int Cyc_Stdio_putc( int i, struct Cyc_Stdio___sFILE* f){
...
return putw( i, f);} unsigned char Cyc_Stdio_FileCloseError[ 19u]="\000\000\000\000FileCloseError";
