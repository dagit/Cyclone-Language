...
fd, struct _tagged_arr dest, int dest_offset, int max_count); extern
...
static struct _tagged_arr Cyc_Lexing_aux_buffer={( unsigned char*) Cyc_Lexing_aux_buffer_v,(
...
lexbuf->lex_buffer, 0,( struct _tagged_arr) lexbuf->lex_buffer, n,( unsigned int)(
lexbuf->lex_buffer_len - n)); Cyc_String_zstrncpy( lexbuf->lex_buffer, lexbuf->lex_buffer_len
- n,( struct _tagged_arr) Cyc_Lexing_aux_buffer, 0,( unsigned int) n); lexbuf->lex_abs_pos=
...
lbuf->lex_buffer, lbuf->lex_start_pos,( unsigned int) len);*(( unsigned char*)
