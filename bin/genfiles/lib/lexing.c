...
extern int Cyc_Stdio_file_string_read(struct Cyc_Stdio___sFILE *fd,struct _tagged_string dest,
...
extern struct Cyc_Lexing_lexbuf *Cyc_Lexing_from_function(int (*read_fun)(struct _tagged_string,
									  int,
									  void *),
							  void *);
...
char Cyc_Lexing_Error[10];
struct Cyc_Lexing_lexbuf;
struct Cyc_Lexing_function_lexbuf_state;
struct Cyc_Lexing_lex_tables;
static char _temp2[1] = "";
static struct _tagged_string Cyc_Lexing_aux_buffer = (struct _tagged_string){_temp2,
									     _temp2,
									     _temp2 + 1};
void Cyc_Lexing_lex_refill(struct Cyc_Lexing_lexbuf *lexbuf) {
  if (({ struct _tagged_string _temp3 = Cyc_Lexing_aux_buffer;
    (unsigned int)(_temp3.last_plus_one - _temp3.curr); }) == 1) {
    Cyc_Lexing_aux_buffer = Cyc_Core_new_string(4096);
  }
  { int read = (((struct Cyc_Lexing_function_lexbuf_state *)lexbuf->refill_state)->read_fun)(Cyc_Lexing_aux_buffer,
											     (int)({ struct _tagged_string _temp4 = Cyc_Lexing_aux_buffer;
											       (unsigned int)(_temp4.last_plus_one - _temp4.curr); }),
											     (void *)((struct Cyc_Lexing_function_lexbuf_state *)lexbuf->refill_state)->read_fun_state);
    int n = read > 0 ? read : (lexbuf->lex_eof_reached = 1, 0);
    if (lexbuf->lex_start_pos < n) {
      int oldlen = lexbuf->lex_buffer_len;
      int newlen = oldlen * 2;
      struct _tagged_string newbuf = Cyc_Core_new_string(newlen + 1);
      Cyc_String_zstrncpy(newbuf,oldlen,lexbuf->lex_buffer,0,(unsigned int)oldlen);
    ...
    Cyc_String_zstrncpy(lexbuf->lex_buffer,0,lexbuf->lex_buffer,n,(unsigned int)(lexbuf->lex_buffer_len - n));
    Cyc_String_zstrncpy(lexbuf->lex_buffer,lexbuf->lex_buffer_len - n,Cyc_Lexing_aux_buffer,
			0,(unsigned int)n);
...
struct Cyc_Lexing_lexbuf *Cyc_Lexing_from_function(int (*read_fun)(struct _tagged_string,
								   int,void *),
						   void *read_fun_state) {
  return ({ struct Cyc_Lexing_lexbuf *_temp5 = (struct Cyc_Lexing_lexbuf *)GC_malloc(sizeof(struct Cyc_Lexing_lexbuf));
    _temp5->refill_buff = (void (*)(struct Cyc_Lexing_lexbuf *lexbuf))Cyc_Lexing_lex_refill;
    _temp5->refill_state = (void *)({ struct Cyc_Lexing_function_lexbuf_state *_temp6 = (struct Cyc_Lexing_function_lexbuf_state *)GC_malloc(sizeof(struct Cyc_Lexing_function_lexbuf_state));
      _temp6->read_fun = read_fun;
      _temp6->read_fun_state = (void *)read_fun_state;
      _temp6; });
    _temp5->lex_buffer = Cyc_Core_new_string(8192);
    _temp5->lex_buffer_len = 8192;
    _temp5->lex_abs_pos = - 8192;
    _temp5->lex_start_pos = 8192;
    _temp5->lex_curr_pos = 8192;
    _temp5->lex_last_pos = 8192;
    _temp5->lex_last_action = 0;
    _temp5->lex_eof_reached = 0;
    _temp5; });
}
int Cyc_Lexing_read_from_file(struct _tagged_string aux,int n,struct Cyc_Stdio___sFILE *f) {
  return Cyc_Stdio_file_string_read(f,aux,0,n);
}
struct Cyc_Lexing_lexbuf *Cyc_Lexing_from_file(struct Cyc_Stdio___sFILE *f) {
  return ((struct Cyc_Lexing_lexbuf *(*)(int (*read_fun)(struct _tagged_string,
							 int,struct Cyc_Stdio___sFILE *),
					 struct Cyc_Stdio___sFILE *read_fun_state))Cyc_Lexing_from_function)(Cyc_Lexing_read_from_file,
													     f);
}
static void Cyc_Lexing_set_eof(struct Cyc_Lexing_lexbuf *lbuf) {
  lbuf->lex_eof_reached = 1;
}
struct Cyc_Lexing_lexbuf *Cyc_Lexing_from_string(struct _tagged_string s) {
  return ({ struct Cyc_Lexing_lexbuf *_temp7 = (struct Cyc_Lexing_lexbuf *)GC_malloc(sizeof(struct Cyc_Lexing_lexbuf));
    _temp7->refill_buff = Cyc_Lexing_set_eof;
    _temp7->refill_state = (void *)0;
    _temp7->lex_buffer = Cyc_String_strdup(s);
    _temp7->lex_buffer_len = (int)({ struct _tagged_string _temp8 = s;
      (unsigned int)(_temp8.last_plus_one - _temp8.curr); });
    _temp7->lex_abs_pos = 0;
    _temp7->lex_start_pos = 0;
    _temp7->lex_curr_pos = 0;
    _temp7->lex_last_pos = 0;
    _temp7->lex_last_action = 0;
    _temp7->lex_eof_reached = 1;
    _temp7; });
}
struct _tagged_string Cyc_Lexing_lexeme(struct Cyc_Lexing_lexbuf *lbuf) {
  int len = lbuf->lex_curr_pos - lbuf->lex_start_pos;
  struct _tagged_string s = Cyc_Core_new_string(len + 1);
  Cyc_String_zstrncpy(s,0,lbuf->lex_buffer,lbuf->lex_start_pos,(unsigned int)len);
  ({ struct _tagged_string _temp9 = s;
    char *_temp11 = _temp9.curr + len;
...
char Cyc_Lexing_lexeme_char(struct Cyc_Lexing_lexbuf *lbuf,int i) {
  return ({ struct _tagged_string _temp12 = lbuf->lex_buffer;
    char *_temp14 = _temp12.curr + (lbuf->lex_start_pos + i);
    if (_temp12.base == 0 ? 1 : (_temp14 < _temp12.base ? 1 : _temp14 >= _temp12.last_plus_one)) {
      _throw(Null_Exception);
    }
    *_temp14; });
}
...
int Cyc_Lexing_lex_engine(struct Cyc_Lexing_lex_tables *tbl,int start_state,
  ...
  while (1) {
    base = ({ struct _tagged_ptr0 _temp15 = tbl->lex_base;
      int *_temp17 = _temp15.curr + state;
    ...
    backtrk = ({ struct _tagged_ptr0 _temp18 = tbl->lex_backtrk;
      int *_temp20 = _temp18.curr + state;
      if (_temp18.base == 0 ? 1 : (_temp20 < _temp18.base ? 1 : _temp20 >= _temp18.last_plus_one)) {
	_throw(Null_Exception);
      }
      *_temp20; });
    if (backtrk >= 0) {
	...
	c = 256;
      ...
      c = (int)({ struct _tagged_string _temp21 = lbuf->lex_buffer;
	char *_temp23 = _temp21.curr + (lbuf->lex_curr_pos++);
	...
	*_temp23; });
    ...
    if (({ struct _tagged_ptr0 _temp24 = tbl->lex_check;
      int *_temp26 = _temp24.curr + (base + c);
      if (_temp24.base == 0 ? 1 : (_temp26 < _temp24.base ? 1 : _temp26 >= _temp24.last_plus_one)) {
	_throw(Null_Exception);
      }
      *_temp26; }) == state) {
      state = ({ struct _tagged_ptr0 _temp27 = tbl->lex_trans;
	int *_temp29 = _temp27.curr + (base + c);
	...
	*_temp29; });
      ...
      state = ({ struct _tagged_ptr0 _temp30 = tbl->lex_default;
	int *_temp32 = _temp30.curr + state;
	...
	*_temp32; });
	...
	(void)_throw((void *)({ struct Cyc_Lexing_Error_struct *_temp33 = (struct Cyc_Lexing_Error_struct *)GC_malloc(sizeof(struct Cyc_Lexing_Error_struct));
		       _temp33[0] = ({ struct Cyc_Lexing_Error_struct _temp34;
			 _temp34.tag = Cyc_Lexing_Error;
			 _temp34.f1 = (struct _tagged_string)({ char *_temp35 = (char *)"empty token";
			   struct _tagged_string _temp36;
			   _temp36.curr = _temp35;
			   _temp36.base = _temp35;
			   _temp36.last_plus_one = _temp35 + 12;
			   _temp36; });
			 _temp34; });
		       _temp33; }));
      }
