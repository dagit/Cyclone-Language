...
int _toupper( int); struct Cyc_Stdio___sFILE; extern unsigned char Cyc_Stdio_FileCloseError[
...
extern int Cyc_String_strspn( struct _tagged_arr s, struct _tagged_arr accept);
extern struct Cyc_List_List* Cyc_String_explode( struct _tagged_arr s); extern
struct Cyc_List_List* Cyc_String_rexplode( struct _RegionHandle*, struct
...
struct _tagged_arr, struct _tagged_arr, int); extern double atan( double);
extern double cos( double); extern double sin( double); extern double tan(
double); extern double tanh( double); extern double frexp( double, int*); extern
double modf( double, double*); extern double ceil( double); extern double fabs(
double); extern double floor( double); extern double acos( double); extern
...
return( struct _tagged_arr) Cstring_to_string( strerror( errnum));} int Cyc_String_strlen(
...
ofs2))); if( diff !=  0){ return diff;}} return( int) len1 - ( int) len2;} int
Cyc_String_strcmp( struct _tagged_arr s1, struct _tagged_arr s2){ if( s1.curr == 
s2.curr){ return 0;}{ int len1= Cyc_String_int_strleno( s1, 0, _tag_arr("String::strcmp",
sizeof( unsigned char), 15u)); int len2= Cyc_String_int_strleno( s2, 0, _tag_arr("String::strcmp",
...
int min_len= len1 >  len2? len2: len1; int bound= min_len >  n? n: min_len;{ int
i= 0; for( 0; i <  bound; i ++){ int retc; if(( retc= f(*(( const unsigned char*)
...
!=  0){ return retc;}}} return 0;}} int Cyc_String_strncmp( struct _tagged_arr
s1, struct _tagged_arr s2, int n){ int len1= Cyc_String_int_strleno( s1, 0,
_tag_arr("String::strncmp", sizeof( unsigned char), 16u)); int len2= Cyc_String_int_strleno(
s2, 0, _tag_arr("String::strncmp", sizeof( unsigned char), 16u)); return Cyc_String_ncmp(
s1, 0, len1, s2, 0, len2, n, Cyc_String_case_cmp);} int Cyc_String_zstrcmp(
struct _tagged_arr a, struct _tagged_arr b){ if( a.curr ==  b.curr){ return 0;}{
int as=( int) _get_arr_size( a, sizeof( unsigned char)); int bs=( int)
_get_arr_size( b, sizeof( unsigned char)); int min_length= as <  bs? as: bs; int
i= - 1; while( ++ i <  min_length) { int diff=( int)*(( const unsigned char*)
_check_unknown_subscript( a, sizeof( unsigned char), i)) - ( int)*(( const
unsigned char*) _check_unknown_subscript( b, sizeof( unsigned char), i)); if(
diff !=  0){ return diff;}} return( int) as - ( int) bs;}} int Cyc_String_zstrncmp(
struct _tagged_arr s1, struct _tagged_arr s2, int n){ if( n <=  0){ return 0;}{
int s1size=( int) _get_arr_size( s1, sizeof( unsigned char)); int s2size=( int)
...
src, int srcOfs, struct _tagged_arr error){ int i; int dsize; int slen; int dlen;
dsize=( int) _get_arr_size( dest, sizeof( unsigned char)); dlen= Cyc_String_strlen((
...
_check_null( lens))->tl; ans= Cyc_Core_rnew_string( r, total_len +  1);{ int i=
0; while( strs !=  0) { struct _tagged_arr _temp8=*(( struct _tagged_arr*)((
struct Cyc_List_List*) _check_null( strs))->hd); len=( int)(( struct Cyc_List_List*)
...
strs))->tl ==  0){ return Cyc_String_rstrdup( r,*(( struct _tagged_arr*)((
struct Cyc_List_List*) _check_null( strs))->hd));}{ struct Cyc_List_List*
...
1) *  seplen;{ struct _tagged_arr ans= Cyc_Core_rnew_string( r, total_len); int
i= 0; while((( struct Cyc_List_List*) _check_null( strs))->tl !=  0) { struct
...
> ( sz *  5) /  4? maxsizeP *  2:( sz *  5) /  4; str= Cyc_String_rexpand( r,(
struct _tagged_arr) str, maxsizeP);}} return str;} struct _tagged_arr Cyc_String_realloc_str(
...
start + ( int) amt) -  1 > ( int) _get_arr_size( s, sizeof( unsigned char))){(
...
unsigned char), 0u)) == '\000'){ return haystack;}{ int len= Cyc_String_int_strleno(
...
start, needle, len) ==  0){ return start;}}} return( struct _tagged_arr)
_tag_arr( 0u, 0u, 0u);}} int Cyc_String_strspn( struct _tagged_arr s, struct
_tagged_arr accept){ int len= Cyc_String_int_strleno( s, 0, _tag_arr("String::strspn",
sizeof( unsigned char), 15u));{ int i= 0; for( 0; i <  len; i ++){ int j; for( j=
0; j <  _get_arr_size( accept, sizeof( unsigned char)); j ++){ if(*(( const
unsigned char*) _check_unknown_subscript( s, sizeof( unsigned char), i)) == *((
const unsigned char*) _check_unknown_subscript( accept, sizeof( unsigned char),
j))){ break;}} if( j ==  _get_arr_size( accept, sizeof( unsigned char))){ return
i;}}} return len;} struct Cyc_List_List* Cyc_String_rexplode( struct
_RegionHandle* r, struct _tagged_arr s){ struct Cyc_List_List* result= 0;{ int i=
Cyc_String_strlen( s) -  1; for( 0; i >=  0; i --){ result=({ struct Cyc_List_List*
_temp33=( struct Cyc_List_List*) _region_malloc( r, sizeof( struct Cyc_List_List));
...
s, int* offset){ int ans= 0; int sn= 1; int digit_seen= 0; int i= offset ==  0?
0:*(( int*) _check_null( offset)); while( i <  _get_arr_size( s, sizeof(
unsigned char))? isspace(( int)*(( const unsigned char*)
...
==  s2.curr){ return 0;}{ int len1= Cyc_String_int_strleno( s1, 0, _tag_arr("Strings::strcasecmp",
sizeof( unsigned char), 20u)); int len2= Cyc_String_int_strleno( s2, 0, _tag_arr("Strings::strcasecmp",
