...
struct Cyc_Queue_Queue* q){ return q->front == 0;} char Cyc_Queue_Empty[ 10u]="\000\000\000\000Empty";
...
else{ void* x=( void*)({ struct Cyc_List_List* _temp4= q->front; if( _temp4 == 0){
_throw( Null_Exception);} _temp4->hd;}); q->front=({ struct Cyc_List_List*
_temp3= q->front; if( _temp3 == 0){ _throw( Null_Exception);} _temp3->tl;}); if(
q->front == 0){ q->rear= 0;} return x;}} void* Cyc_Queue_peek( struct Cyc_Queue_Queue*
q){ if( q->front == 0){( void) _throw(( void*) Cyc_Queue_Empty);} else{ return(
void*)({ struct Cyc_List_List* _temp5= q->front; if( _temp5 == 0){ _throw(
Null_Exception);} _temp5->hd;});}} void Cyc_Queue_clear( struct Cyc_Queue_Queue*
...
x= q->front; for( 0; x != 0; x=({ struct Cyc_List_List* _temp6= x; if( _temp6 ==
0){ _throw( Null_Exception);} _temp6->tl;})){ f(( void*)({ struct Cyc_List_List*
_temp7= x; if( _temp7 == 0){ _throw( Null_Exception);} _temp7->hd;}));}} void
...
_temp9= x; if( _temp9 == 0){ _throw( Null_Exception);} _temp9->hd;}));}}