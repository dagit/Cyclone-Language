...
tag;}; struct Cyc_Queue_Queue; typedef struct Cyc_Queue_Queue* Cyc_Queue_gqueue_t;
...
char* tag;}; extern struct Cyc_Queue_Queue* Cyc_Queue_create(); extern void Cyc_Queue_add(
...
struct Cyc_Queue_Queue* q){ return q->front == 0;} char Cyc_Queue_Empty_tag[ 6u]="Empty";
...
Cyc_List_List*) GC_malloc( sizeof( struct Cyc_List_List)); _temp1->hd=( void*) x;
_temp1->tl= 0; _temp1;}); if( q->front == 0){ q->front= cell; q->rear= cell;}
else{( q->rear)->tl= cell; q->rear= cell;}} void* Cyc_Queue_take( struct Cyc_Queue_Queue*
q){ if( q->front == 0){( void) _throw(({ struct Cyc_Queue_Empty_struct* _temp2=(
struct Cyc_Queue_Empty_struct*) GC_malloc_atomic( sizeof( struct Cyc_Queue_Empty_struct));*
_temp2=( struct Cyc_Queue_Empty_struct){.tag= Cyc_Queue_Empty_tag};( struct
_xenum_struct*) _temp2;}));} else{ void* x=( void*)( q->front)->hd; q->front=( q->front)->tl;
if( q->front == 0){ q->rear= 0;} return x;}} void* Cyc_Queue_peek( struct Cyc_Queue_Queue*
q){ if( q->front == 0){( void) _throw(({ struct Cyc_Queue_Empty_struct* _temp3=(
struct Cyc_Queue_Empty_struct*) GC_malloc_atomic( sizeof( struct Cyc_Queue_Empty_struct));*
_temp3=( struct Cyc_Queue_Empty_struct){.tag= Cyc_Queue_Empty_tag};( struct
_xenum_struct*) _temp3;}));} else{ return( void*)( q->front)->hd;}} void Cyc_Queue_clear(
