...
GC_malloc_atomic( sizeof( struct Cyc_Queue_Empty_struct));* _temp2=( struct Cyc_Queue_Empty_struct){.tag=
Cyc_Queue_Empty_tag};( struct _xenum_struct*) _temp2;}));} else{ void* x=( void*)(
q->front)->hd; q->front=( q->front)->tl; if( q->front == 0){ q->rear= 0;} return
x;}} void* Cyc_Queue_peek( struct Cyc_Queue_Queue* q){ if( q->front == 0){( void)
_throw(({ struct Cyc_Queue_Empty_struct* _temp3=( struct Cyc_Queue_Empty_struct*)
GC_malloc_atomic( sizeof( struct Cyc_Queue_Empty_struct));* _temp3=( struct Cyc_Queue_Empty_struct){.tag=
Cyc_Queue_Empty_tag};( struct _xenum_struct*) _temp3;}));} else{ return( void*)(
q->front)->hd;}} void Cyc_Queue_clear( struct Cyc_Queue_Queue* q){ q->front= 0;
q->rear= 0;} int Cyc_Queue_length( struct Cyc_Queue_Queue* q){ return(( int(*)(
struct Cyc_List_List* x)) Cyc_List_length)( q->front);} void Cyc_Queue_iter(
void(* f)( void*), struct Cyc_Queue_Queue* q){ struct Cyc_List_List* x= q->front;
for( 0; x != 0; x= x->tl){ f(( void*) x->hd);}} void Cyc_Queue_app( void*(* f)(
