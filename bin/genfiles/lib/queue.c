...
else{(( struct Cyc_List_List*) _check_null( q->rear))->tl= cell; q->rear= cell;}}
void* Cyc_Queue_take( struct Cyc_Queue_Queue* q){ if( q->front == 0){( void)
_throw(( void*) Cyc_Queue_Empty);} else{ void* _temp2=( void*)(( struct Cyc_List_List*)
_check_null( q->front))->hd; goto _LL3; _LL3: q->front=(( struct Cyc_List_List*)
_check_null( q->front))->tl; if( q->front == 0){ q->rear= 0;} return _temp2;}}
void* Cyc_Queue_peek( struct Cyc_Queue_Queue* q){ if( q->front == 0){( void)
_throw(( void*) Cyc_Queue_Empty);} else{ return( void*)(( struct Cyc_List_List*)
_check_null( q->front))->hd;}} void Cyc_Queue_clear( struct Cyc_Queue_Queue* q){
