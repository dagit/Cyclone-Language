...
clock(); extern double difftime( int _time2, int _time1); extern int mktime(
struct Cyc_Time_tm* _timeptr); extern int time( int* _timer); extern struct Cyc_Time_tm*
...
_fmt, const struct Cyc_Time_tm* _t); extern char* asctime_r( const struct Cyc_Time_tm*,
char*); extern char* ctime_r( const int*, char*); extern char* timezone();
struct _tagged_string Cyc_Time_asctime( const struct Cyc_Time_tm* _tblock){
return Cstring_to_string( asctime( _tblock));} struct _tagged_string Cyc_Time_ctime(
...
const struct Cyc_Time_tm* _t){ unsigned int m=({ struct _tagged_string _temp0=
_s;( unsigned int)( _temp0.last_plus_one - _temp0.curr);}) < _maxsize?({ struct
