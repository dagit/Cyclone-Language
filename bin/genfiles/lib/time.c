...
_tzname[ 2u]  __attribute__(( dllimport )) ; extern void tzset()
 __attribute__(( cdecl )) ; extern struct _tagged_string Cyc_Time_asctime( const
struct Cyc_Time_tm* _tblock); extern struct _tagged_string Cyc_Time_ctime( const
int* _time); extern unsigned int Cyc_Time_strftime( struct _tagged_string _s,
unsigned int _maxsize, struct _tagged_string _fmt, const struct Cyc_Time_tm* _t);
