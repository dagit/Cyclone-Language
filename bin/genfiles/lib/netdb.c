...
int listen( int fd, int n); extern int shutdown( int fd, int how); struct Cyc_Inet_in_addr{
...
struct Cyc_NetDB_servent{ struct _tagged_arr s_name; struct _tagged_arr
...
name); extern void Cyc_NetDB_herror( struct _tagged_arr); struct Cyc_Cnetdb_Cservent{
...
unsigned char** h_aliases; short h_addrtype; short h_length; struct Cyc_Inet_in_addr**
h_addr_list; } ; extern struct Cyc_Cnetdb_Cservent* getservbyname( unsigned char*
name, unsigned char* proto); extern struct Cyc_Cnetdb_Chostent* gethostbyname(
unsigned char* name); extern void herror( unsigned char*); extern struct
_tagged_arr pntlp_toCyc( struct Cyc_Inet_in_addr**); struct Cyc_NetDB_servent*
Cyc_NetDB_getservbyname( struct _tagged_arr name, struct _tagged_arr proto){
struct Cyc_Cnetdb_Cservent* src= getservbyname( string_to_Cstring( name),
string_to_Cstring( proto)); return( unsigned int) src?({ struct Cyc_NetDB_servent*
_temp0=( struct Cyc_NetDB_servent*) GC_malloc( sizeof( struct Cyc_NetDB_servent));
_temp0->s_name= Cstring_to_string((( struct Cyc_Cnetdb_Cservent*) _check_null(
src))->s_name); _temp0->s_aliases= ntCsl_to_ntsl((( struct Cyc_Cnetdb_Cservent*)
_check_null( src))->s_aliases); _temp0->s_port=(( struct Cyc_Cnetdb_Cservent*)
_check_null( src))->s_port; _temp0->s_proto= Cstring_to_string((( struct Cyc_Cnetdb_Cservent*)
_check_null( src))->s_proto); _temp0;}): 0;} struct Cyc_NetDB_hostent* Cyc_NetDB_gethostbyname(
