...
int Cyc_Stat_open( struct _tagged_arr pathname, int flags); extern int Cyc_Stat_stat(
...
mask); extern int open( unsigned char* pathname, int flags); extern int stat(
...
return open( string_to_Cstring( pathname), flags);} int Cyc_Stat_stat( struct
