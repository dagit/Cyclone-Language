...
extern unsigned int time( unsigned int* t); extern struct Cyc_Time_tm* gmtime(
const unsigned int* timep); extern struct Cyc_Time_tm* localtime( const
unsigned int* timep); extern int timezone; extern int daylight; struct Cyc_Stat_stat_t{
short st_dev; unsigned short st_ino; unsigned int st_mode; unsigned short
...
filename, struct Cyc_Stat_stat_t* buf); extern unsigned int umask( unsigned int
mask); extern int open( unsigned char* pathname, int flags, unsigned int mode);
extern int stat( unsigned char* filename, struct Cyc_Stat_stat_t* buf); extern
int fstat( int fd, struct Cyc_Stat_stat_t* buf); extern int lstat( unsigned char*
filename, struct Cyc_Stat_stat_t* buf); int Cyc_Stat_open( struct _tagged_arr
