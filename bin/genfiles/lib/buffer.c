...
8u]; extern unsigned int Cyc_String_strlen( struct _tagged_arr s); extern struct
_tagged_arr Cyc_String_strncpy( struct _tagged_arr, int, struct _tagged_arr, int,
unsigned int); extern struct _tagged_arr Cyc_String_zstrncpy( struct _tagged_arr,
int, struct _tagged_arr, int, unsigned int); extern struct _tagged_arr Cyc_String_substring(
...
more){ unsigned int len= b->length; unsigned int new_len= len; struct
_tagged_arr new_buffer; while( b->position + more > new_len) { new_len= 2 *
new_len;} new_buffer= Cyc_Core_new_string(( int) new_len); Cyc_String_strncpy(
new_buffer, 0,( struct _tagged_arr) b->buffer, 0, b->position); b->buffer=
new_buffer; b->length= new_len; return;} void Cyc_Buffer_add_char( struct Cyc_Buffer_t*
b, unsigned char c){ int pos=( int) b->position; if( pos >= b->length){ Cyc_Buffer_resize(
...
offset, int len){ if(( offset < 0? 1: len < 0)? 1: offset + len > _get_arr_size(
s, sizeof( unsigned char))){( int) _throw(( void*)({ struct Cyc_Core_InvalidArg_struct*
...
b, struct _tagged_arr s){ int len=( int) Cyc_String_strlen( s); int new_position=(
int)( b->position + len); if( new_position > b->length){ Cyc_Buffer_resize( b,(
unsigned int) len);} Cyc_String_strncpy( b->buffer,( int) b->position, s, 0,(
