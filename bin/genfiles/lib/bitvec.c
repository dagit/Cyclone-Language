...
struct _tagged_arr bvec); struct Cyc_Stdio___sFILE; extern unsigned char Cyc_Stdio_FileCloseError[
...
_check_unknown_subscript( bvec, sizeof( int), word)) &  ~( 1 <<  offset);} int
Cyc_Bitvec_get_and_set( struct _tagged_arr bvec, int pos){ int word= pos >>  5;
int offset= pos &  31; int slot=*(( int*) _check_unknown_subscript( bvec,
sizeof( int), word)); int ans=( slot >>  offset &  1) ==  1; if( ! ans){*(( int*)
_check_unknown_subscript( bvec, sizeof( int), word))= slot |  1 <<  offset;}
return ans;} void Cyc_Bitvec_union_two( struct _tagged_arr dest, struct
_tagged_arr src1, struct _tagged_arr src2){ int len=( int) _get_arr_size( dest,
sizeof( int)); int i= 0; for( 0; i <  len; ++ i){*(( int*)
...
0; i <  32 *  _get_arr_size( bvec, sizeof( int)); ++ i){({ struct Cyc_Stdio_Int_pa_struct
_temp15; _temp15.tag= Cyc_Stdio_Int_pa; _temp15.f1=( int)(( unsigned int)( Cyc_Bitvec_get(
bvec, i)? 1: 0));{ void* _temp14[ 1u]={& _temp15}; Cyc_Stdio_printf( _tag_arr("%d",
sizeof( unsigned char), 3u), _tag_arr( _temp14, sizeof( void*), 1u));}});}}({
void* _temp16[ 0u]={}; Cyc_Stdio_printf( _tag_arr("\n", sizeof( unsigned char),
2u), _tag_arr( _temp16, sizeof( void*), 0u));});}
