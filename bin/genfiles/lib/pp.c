...
Cyc_List_Nth[ 8u]; extern unsigned int Cyc_String_strlen( struct _tagged_arr s);
extern int Cyc_String_zstrptrcmp( struct _tagged_arr*, struct _tagged_arr*);
...
Cyc_String_strlen( s); return({ struct Cyc_PP_Doc* _temp70=( struct Cyc_PP_Doc*)
GC_malloc( sizeof( struct Cyc_PP_Doc)); _temp70->mwo= slen; _temp70->mw= Cyc_PP_infinity;
_temp70->f=( void*)(( void*(*)( struct Cyc_PP_Out*(* f)( struct _tuple1*, struct
Cyc_PP_Ppstate*), struct _tuple1* x)) Cyc_Fn_make_fn)( Cyc_PP_text_doc_f,({
struct _tuple1* _temp71=( struct _tuple1*) GC_malloc( sizeof( struct _tuple1));
_temp71->f1= slen; _temp71->f2= s; _temp71;})); _temp70;});} struct Cyc_PP_Doc*
...
struct _tagged_arr shrt, struct _tagged_arr full){ int slen=( int) Cyc_String_strlen(
