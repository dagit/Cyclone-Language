...
dllimport )) ; extern void tzset()  __attribute__(( cdecl )) ; struct Cyc___sFILE;
...
_read)( void* _cookie, char* _buf, int _n)  __attribute__(( cdecl )) ; int(*
_write)( void* _cookie, const char* _buf, int _n)  __attribute__(( cdecl )) ;
int(* _seek)( void* _cookie, int _offset, int _whence)  __attribute__(( cdecl
)) ; int(* _close)( void* _cookie)  __attribute__(( cdecl )) ; struct Cyc___sbuf
_ub; char* _up; int _ur; char _ubuf[ 3u]; char _nbuf[ 1u]; struct Cyc___sbuf _lb;
int _blksize; int _offset; struct Cyc__reent* _data; } ; struct Cyc__reent_u1{
unsigned int _unused_rand; int _strtok_last; char _asctime_buf[ 26u]; struct Cyc_Time_tm
...
_stdout; struct Cyc___sFILE* _stderr; int _inc; char _emergency[ 25u]; int
_current_category; const char* _current_locale; int __sdidinit; void(* __cleanup)(
struct Cyc__reent*)  __attribute__(( cdecl )) ; struct Cyc__Bigint* _result; int
_result_k; struct Cyc__Bigint* _p5s; struct Cyc__Bigint** _freelist; int _cvtlen;
char* _cvtbuf; union Cyc__reent_union _new; struct Cyc__atexit* _atexit; struct
Cyc__atexit _atexit0; void(** _sig_func)( int); struct Cyc__glue __sglue; struct
Cyc___sFILE __sf[ 3u]; } ; extern struct Cyc__reent* _impure_ptr; extern void
_reclaim_reent( struct Cyc__reent*); struct Cyc_Stdlib__Div{ int quot; int rem;
} ; typedef struct Cyc_Stdlib__Div Cyc_Stdlib_div_t; struct Cyc_Stdlib__Ldiv{
...
_tagged_string); extern double atof( char* _nptr)  __attribute__(( cdecl )) ;
extern float atoff( char* _nptr)  __attribute__(( cdecl )) ; extern int atoi(
char* _nptr)  __attribute__(( cdecl )) ; extern int atol( char* _nptr)
 __attribute__(( cdecl )) ; extern char* getenv( char* __string)
 __attribute__(( cdecl )) ; extern char* _getenv_r( struct Cyc__reent*, char*
__string)  __attribute__(( cdecl )) ; extern char* _findenv( char*, int*)
 __attribute__(( cdecl )) ; extern char* _findenv_r( struct Cyc__reent*, char*,
int*)  __attribute__(( cdecl )) ; extern int putenv( char* __string)
 __attribute__(( cdecl )) ; extern int _putenv_r( struct Cyc__reent*, char*
__string)  __attribute__(( cdecl )) ; extern int setenv( char* __string, char*
__value, int __overwrite)  __attribute__(( cdecl )) ; extern int _setenv_r(
struct Cyc__reent*, char* __string, char* __value, int __overwrite)
 __attribute__(( cdecl )) ; extern void unsetenv( char* __string)
 __attribute__(( cdecl )) ; extern void _unsetenv_r( struct Cyc__reent*, char*
__string)  __attribute__(( cdecl )) ; double Cyc_Stdlib_atof( struct
_tagged_string _nptr){ return atof( string_to_Cstring( _nptr));} float Cyc_Stdlib_atoff(
struct _tagged_string _nptr){ return atoff( string_to_Cstring( _nptr));} int Cyc_Stdlib_atoi(
