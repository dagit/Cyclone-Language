...
return c - '0';} else{ if('a' <=  c? c <= 'f': 0){ return( 10 +  c) - 'a';}
else{ if('A' <=  c? c <= 'F': 0){ return( 10 +  c) - 'A';} else{( int) _throw((
void*)({ struct Cyc_Core_InvalidArg_struct* _temp17=( struct Cyc_Core_InvalidArg_struct*)
...
_tag_arr("string to integer conversion", sizeof( unsigned char), 29u); _temp18;});
_temp17;}));}}}} int Cyc_Core_int_of_string( struct _tagged_arr s){ int n; int i;
int base; int sign= 1; for( i= 0; i <  _get_arr_size( s, sizeof( unsigned char))?
Cyc_Core_is_space(*(( const unsigned char*) _check_unknown_subscript( s, sizeof(
unsigned char), i))): 0; ++ i){;} while( i <  _get_arr_size( s, sizeof(
unsigned char))?*(( const unsigned char*) _check_unknown_subscript( s, sizeof(
unsigned char), i)) == '-'? 1:*(( const unsigned char*) _check_unknown_subscript(
s, sizeof( unsigned char), i)) == '+': 0) { if(*(( const unsigned char*)
_check_unknown_subscript( s, sizeof( unsigned char), i)) == '-'){ sign= - sign;}
i ++;} if( i ==  _get_arr_size( s, sizeof( unsigned char))){( int) _throw(( void*)({
...
_tag_arr("string to integer conversion", sizeof( unsigned char), 29u); _temp20;});
_temp19;}));} if( i +  1 ==  _get_arr_size( s, sizeof( unsigned char))? 1:*((
const unsigned char*) _check_unknown_subscript( s, sizeof( unsigned char), i))
!= '0'){ base= 10;} else{ switch(*(( const unsigned char*)
...
_check_unknown_subscript( s, sizeof( unsigned char), i)) != '\000': 0; ++ i){
int digit= Cyc_Core_int_of_char(*(( const unsigned char*)
_check_unknown_subscript( s, sizeof( unsigned char), i))); if( digit >=  base){(
int) _throw(( void*)({ struct Cyc_Core_InvalidArg_struct* _temp26=( struct Cyc_Core_InvalidArg_struct*)
...
for( i= len -  1; n >  0; -- i){*(( unsigned char*) _check_unknown_subscript(
ans, sizeof( unsigned char), i))=( unsigned char)('0' +  n %  10); n /= 10;}
for( 0; i >=  0; -- i){*(( unsigned char*) _check_unknown_subscript( ans,
sizeof( unsigned char), i))='0';} if( negative){*(( unsigned char*)
...
unsigned int m= n; do { ++ len;} while (( m /= 10) >  0);{ struct _tagged_arr
ans= Cyc_Core_new_string( len +  1);{ int i= len -  1; for( 0; i >=  0; -- i){*((
