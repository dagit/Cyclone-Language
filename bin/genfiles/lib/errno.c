...
extern struct _tagged_string Cyc_Errno_sys_err(int);
extern int *__errno() __attribute__((dllimport));
extern int _sys_nerr __attribute__((dllimport));
extern char *_sys_errlist[135] __attribute__((dllimport));
struct _tagged_string Cyc_Errno_sys_err(int i) {
  if (i < 0 ? 1 : i > _sys_nerr) {
    (void)_throw((void *)({ struct Cyc_Core_InvalidArg_struct *_temp0 = (struct Cyc_Core_InvalidArg_struct *)GC_malloc(sizeof(struct Cyc_Core_InvalidArg_struct));
		   _temp0[0] = ({ struct Cyc_Core_InvalidArg_struct _temp1;
		     ...
		     _temp1.f1 = (struct _tagged_string)({ char *_temp2 = (char *)"sys_err: integer argument out of range";
		       struct _tagged_string _temp3;
		       _temp3.curr = _temp2;
		       _temp3.base = _temp2;
		       _temp3.last_plus_one = _temp2 + 39;
		       _temp3; });
		     _temp1; });
		   _temp0; }));
  }
  return Cstring_to_string(({ char **_temp4 = (char **)_sys_errlist;
			     unsigned int _temp5 = i;
			     if (_temp5 >= 135) {
