...
_get_zero_arr_size_char(_tmp2,13));});_tmp1;});_tmp0;}));limit_ofs=base_ofs + len;
...
_get_zero_arr_size_char(_tmp5,13));});_tmp4;});_tmp3;}));{struct _dynforward_ptr
from=({unsigned int _tmp6=(unsigned int)len;void**_tmp7=(void**)_cycalloc(
_check_times(sizeof(void*),_tmp6));struct _dynforward_ptr _tmp9=_tag_dynforward(
_tmp7,sizeof(void*),_tmp6);{unsigned int _tmp8=_tmp6;unsigned int i;for(i=0;i < 
_tmp8;i ++){_tmp7[i]=*((void**)_check_dynforward_subscript(arr,sizeof(void*),0));}}
_tmp9;});struct _dynforward_ptr to=arr;struct _dynforward_ptr swap;int swapped=0;int
stepsize;int start;int lstart;int lend;int rstart;int rend;int dest;for(stepsize=1;
...
_tmpC=_tmpA;unsigned int i;for(i=0;i < _tmpC;i ++){_tmpB[i]=*((void**)
_check_dynforward_subscript(x,sizeof(void*),(int)i));}}_tmpD;});}struct
_dynforward_ptr Cyc_Array_map(void*(*f)(void*),struct _dynforward_ptr x){int sx=(int)
_get_dynforward_size(x,sizeof(void*));return({unsigned int _tmpE=(unsigned int)sx;
void**_tmpF=(void**)_cycalloc(_check_times(sizeof(void*),_tmpE));struct
_dynforward_ptr _tmp11=_tag_dynforward(_tmpF,sizeof(void*),_tmpE);{unsigned int
_tmp10=_tmpE;unsigned int i;for(i=0;i < _tmp10;i ++){_tmpF[i]=f(*((void**)
_check_dynforward_subscript(x,sizeof(void*),(int)i)));}}_tmp11;});}struct
...
sizeof(void*),_tmp12));struct _dynforward_ptr _tmp15=_tag_dynforward(_tmp13,
sizeof(void*),_tmp12);{unsigned int _tmp14=_tmp12;unsigned int i;for(i=0;i < _tmp14;
i ++){_tmp13[i]=f(env,*((void**)_check_dynforward_subscript(x,sizeof(void*),(int)
i)));}}_tmp15;});}void Cyc_Array_imp_map(void*(*f)(void*),struct _dynforward_ptr x){
int sx=(int)_get_dynforward_size(x,sizeof(void*));int i=0;for(0;i < sx;++ i){*((void**)
_check_dynforward_subscript(x,sizeof(void*),i))=f(*((void**)
...
_tmp18;i ++){_tmp17[i]=f(*((void**)_check_dynforward_subscript(x,sizeof(void*),(
int)i)),*((void**)_check_dynforward_subscript(y,sizeof(void*),(int)i)));}}_tmp19;});}
void Cyc_Array_app(void*(*f)(void*),struct _dynforward_ptr x){int sx=(int)
_get_dynforward_size(x,sizeof(void*));int i=0;for(0;i < sx;++ i){f(*((void**)
_check_dynforward_subscript(x,sizeof(void*),i)));}}void Cyc_Array_app_c(void*(*f)(
void*,void*),void*env,struct _dynforward_ptr x){int sx=(int)_get_dynforward_size(x,
sizeof(void*));int i=0;for(0;i < sx;++ i){f(env,*((void**)
_check_dynforward_subscript(x,sizeof(void*),i)));}}void Cyc_Array_iter(void(*f)(
void*),struct _dynforward_ptr x){int sx=(int)_get_dynforward_size(x,sizeof(void*));
int i=0;for(0;i < sx;++ i){f(*((void**)_check_dynforward_subscript(x,sizeof(void*),
i)));}}void Cyc_Array_iter_c(void(*f)(void*,void*),void*env,struct _dynforward_ptr
x){int sx=(int)_get_dynforward_size(x,sizeof(void*));int i=0;for(0;i < sx;++ i){f(
env,*((void**)_check_dynforward_subscript(x,sizeof(void*),i)));}}void Cyc_Array_app2(
...
void**)_check_dynforward_subscript(x,sizeof(void*),(int)i));_tmp21.f2=*((void**)
_check_dynforward_subscript(y,sizeof(void*),(int)i));_tmp21;});}}_tmp22;});}
struct _tuple1 Cyc_Array_split(struct _dynforward_ptr x){int sx=(int)
_get_dynforward_size(x,sizeof(struct _tuple0));return({struct _tuple1 _tmp23;_tmp23.f1=({
unsigned int _tmp28=(unsigned int)sx;void**_tmp29=(void**)_cycalloc(_check_times(
sizeof(void*),_tmp28));struct _dynforward_ptr _tmp2B=_tag_dynforward(_tmp29,
sizeof(void*),_tmp28);{unsigned int _tmp2A=_tmp28;unsigned int i;for(i=0;i < _tmp2A;
i ++){_tmp29[i]=(*((struct _tuple0*)_check_dynforward_subscript(x,sizeof(struct
_tuple0),(int)i))).f1;}}_tmp2B;});_tmp23.f2=({unsigned int _tmp24=(unsigned int)
sx;void**_tmp25=(void**)_cycalloc(_check_times(sizeof(void*),_tmp24));struct
...
_check_dynforward_subscript(x,sizeof(struct _tuple0),(int)i))).f2;}}_tmp27;});
_tmp23;});}int Cyc_Array_memq(struct _dynforward_ptr l,void*x){int s=(int)
_get_dynforward_size(l,sizeof(void*));{int i=0;for(0;i < s;++ i){if(*((void**)
_check_dynforward_subscript(l,sizeof(void*),i))== x)return 1;}}return 0;}int Cyc_Array_mem(
int(*compare)(void*,void*),struct _dynforward_ptr l,void*x){int s=(int)
...
_get_zero_arr_size_char(_tmp2E,15));});_tmp2D;});_tmp2C;}));return({unsigned int
