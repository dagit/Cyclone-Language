...
for(i=0;i < sz;++ i){if(*((const char*)_check_dyneither_subscript(s,sizeof(char),(
int)i))== '\000')return i;}return i;}int Cyc_strcmp(struct _dyneither_ptr s1,struct
_dyneither_ptr s2){if(s1.curr == s2.curr)return 0;{int i=0;unsigned int sz1=
_get_dyneither_size(s1,sizeof(char));unsigned int sz2=_get_dyneither_size(s2,
sizeof(char));unsigned int minsz=sz1 < sz2?sz1: sz2;minsz <= _get_dyneither_size(s1,
...
const char*_tmp0="minsz <= numelts(s1) && minsz <= numelts(s2)";_tag_dyneither(
_tmp0,sizeof(char),45);}),({const char*_tmp1="string.cyc";_tag_dyneither(_tmp1,
sizeof(char),11);}),60);while(i < minsz){char c1=*((const char*)
