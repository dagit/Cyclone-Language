...
unsigned int);struct Cyc_in_addr{unsigned int s_addr;};unsigned long Cyc_htonl(
unsigned long x);unsigned short Cyc_htons(unsigned short x);unsigned long Cyc_ntohl(
unsigned long x);unsigned short Cyc_ntohs(unsigned short x);unsigned long
__stub_htonl(unsigned long x);unsigned short __stub_htons(unsigned short x);
unsigned long __stub_ntohl(unsigned long x);unsigned short __stub_ntohs(
unsigned short x);unsigned long Cyc_htonl(unsigned long x){return __stub_htonl(x);}
unsigned short Cyc_htons(unsigned short x){return __stub_htons(x);}unsigned long Cyc_ntohl(
unsigned long x){return __stub_ntohl(x);}unsigned short Cyc_ntohs(unsigned short x){
return __stub_ntohs(x);}struct Cyc_dirent{long d_version;long __d_reserved[2];long
d_fd;unsigned long d_ino;char d_name[256];};struct Cyc___cycDIR;int Cyc_closedir(
