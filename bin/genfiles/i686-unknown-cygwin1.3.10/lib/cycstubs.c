...
char**_tmp87=newargs;_tag_dyneither(_tmp87,sizeof(const char*),
_get_zero_arr_size_voidstar(_tmp87,1));}),sizeof(const char*),0);const char*_tmp89=*((
const char**)_check_dyneither_subscript(_tmp88,sizeof(const char*),0));const char*
_tmp8A=(const char*)arg0;if(_get_dyneither_size(_tmp88,sizeof(const char*))== 1
 && (_tmp89 == 0  && _tmp8A != 0))_throw_arraybounds();*((const char**)_tmp88.curr)=
_tmp8A;});{int i=0;for(0;i < _get_dynforward_size(argv,sizeof(const char*))- 1;i ++){({
struct _dyneither_ptr _tmp8C=_dyneither_ptr_plus(({const char**_tmp8B=newargs;
_tag_dyneither(_tmp8B,sizeof(const char*),_get_zero_arr_size_voidstar(_tmp8B,1));}),
