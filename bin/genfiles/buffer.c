...
# 83 "buffer.h"
void Cyc_Buffer_add_substring(struct Cyc_Buffer_t*,struct _dyneither_ptr,int offset,int len);struct Cyc_List_List{void*hd;struct Cyc_List_List*tl;};extern char Cyc_List_List_mismatch[14U];struct Cyc_List_List_mismatch_exn_struct{char*tag;};extern char Cyc_List_Nth[4U];struct Cyc_List_Nth_exn_struct{char*tag;};
# 38 "string.h"
...
# 108 "string.h"
struct _dyneither_ptr Cyc_substring(struct _dyneither_ptr,int ofs,unsigned long n);struct Cyc_Buffer_t{struct _dyneither_ptr buffer;unsigned position;unsigned length;struct _dyneither_ptr initial_buffer;};
# 48 "buffer.cyc"
struct Cyc_Buffer_t*Cyc_Buffer_create(unsigned n){
if(n > (unsigned)0){
struct _dyneither_ptr s=({unsigned _tmp1=n + (unsigned)1;_tag_dyneither(_region_calloc(Cyc_Core_unique_region,sizeof(char),_tmp1),sizeof(char),_tmp1);});
...
b->length=_get_dyneither_size(_tmp17,sizeof(char))- (unsigned)1;
...
# 116
new_buffer=({unsigned _tmp1C=new_len + (unsigned long)1;_tag_dyneither(_region_calloc(Cyc_Core_unique_region,sizeof(char),_tmp1C),sizeof(char),_tmp1C);});
...
if((offset < 0 || len < 0)||(unsigned)(offset + len)> _get_dyneither_size(s,sizeof(char)))
