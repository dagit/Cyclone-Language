...
struct Cyc_Queue_Queue)); _temp0->front= 0; _temp0->rear= 0; _temp0;});} void
Cyc_Queue_radd( struct _RegionHandle* r, struct Cyc_Queue_Queue* q, void* x){
struct Cyc_List_List* cell=({ struct Cyc_List_List* _temp1=( struct Cyc_List_List*)
_region_malloc( r, sizeof( struct Cyc_List_List)); _temp1->hd=( void*) x; _temp1->tl=
0; _temp1;}); if( q->front ==  0){ q->front= cell; q->rear= cell;} else{((
struct Cyc_List_List*) _check_null( q->rear))->tl= cell; q->rear= cell;}} void
Cyc_Queue_add( struct Cyc_Queue_Queue* q, void* x){ Cyc_Queue_radd( Cyc_Core_heap_region,
q, x);} void* Cyc_Queue_take( struct Cyc_Queue_Queue* q){ if( q->front ==  0){(
int) _throw(( void*) Cyc_Queue_Empty);} else{ void* _temp2=( void*)(( struct Cyc_List_List*)
