...
struct _tagged_arr gr_name; struct _tagged_arr gr_passwd; unsigned short gr_gid;
...
unsigned short uid); extern int Cyc_std_initgroups( struct _tagged_arr user,
unsigned short group); struct Cyc_Cgrp_Cgroup{ unsigned char* gr_name;
unsigned char* gr_passwd; unsigned short gr_gid; unsigned char** gr_mem; } ;
extern struct Cyc_Cgrp_Cgroup* getgrnam( unsigned char* name); extern struct Cyc_Cgrp_Cgroup*
getgrgid( unsigned short gid); extern int initgroups( unsigned char* user,
unsigned short group); struct Cyc_std_group* Cyc_std_getgrnam( struct
_tagged_arr name){ struct Cyc_Cgrp_Cgroup* src= getgrnam( string_to_Cstring(
name)); return( unsigned int) src?({ struct Cyc_std_group* _temp0=( struct Cyc_std_group*)
GC_malloc( sizeof( struct Cyc_std_group)); _temp0->gr_name=( struct _tagged_arr)
...
GC_malloc( sizeof( struct Cyc_std_group)); _temp1->gr_name=( struct _tagged_arr)
Cstring_to_string((( struct Cyc_Cgrp_Cgroup*) _check_null( src))->gr_name);
...
_tagged_arr user, unsigned short group){ return initgroups( string_to_Cstring(
user), group);}
