...
unsigned short st_uid; unsigned short st_gid; short st_rdev; int st_size;
unsigned int st_blksize; unsigned int st_blocks; int st_atime; int st_mtime; int
st_ctime; } ; extern int Cyc_Std_open( struct _tagged_arr pathname, int flags,
struct _tagged_arr mode); extern int Cyc_Std_stat( struct _tagged_arr filename,
struct Cyc_Std_stat_t* buf); extern int fstat( int fd, struct Cyc_Std_stat_t*
buf); extern int Cyc_Std_lstat( struct _tagged_arr filename, struct Cyc_Std_stat_t*
buf); extern int umask( int mask); extern int Cyc_Std_mkdir( struct _tagged_arr
pathname, int mode); extern int open( unsigned char* pathname, int flags, int
mode); extern int stat( unsigned char* filename, struct Cyc_Std_stat_t* buf);
extern int fstat( int fd, struct Cyc_Std_stat_t* buf); extern int lstat(
unsigned char* filename, struct Cyc_Std_stat_t* buf); extern int mkdir(
unsigned char* pathname, int mode); int Cyc_Std_open( struct _tagged_arr
pathname, int flags, struct _tagged_arr mode){ int my_mode; if( _get_arr_size(
mode, sizeof( int)) ==  0){ my_mode= - 1;} else{ if( _get_arr_size( mode,
sizeof( int)) ==  1){ my_mode=*(( int*) _check_unknown_subscript( mode, sizeof(
int), 0));} else{( int) _throw(( void*)({ struct Cyc_Core_Failure_struct* _temp0=(
