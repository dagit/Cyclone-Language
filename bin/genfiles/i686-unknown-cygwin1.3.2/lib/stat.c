...
buf); extern int umask( int mask); extern int open( unsigned char* pathname, int
flags, int mode); extern int stat( unsigned char* filename, struct Cyc_Std_stat_t*
buf); extern int fstat( int fd, struct Cyc_Std_stat_t* buf); extern int lstat(
unsigned char* filename, struct Cyc_Std_stat_t* buf); int Cyc_Std_open( struct
_tagged_arr pathname, int flags, struct _tagged_arr mode){ int my_mode; if(
_get_arr_size( mode, sizeof( int)) ==  0){ my_mode= - 1;} else{ if(
