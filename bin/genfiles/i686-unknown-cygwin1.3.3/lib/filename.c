...
ans_sz){unsigned char c=*((const unsigned char*)_check_unknown_subscript(filename,
sizeof(unsigned char),j ++));*((unsigned char*)_check_unknown_subscript(ans,
sizeof(unsigned char),i ++))=c == '\\'?'/': c;}return ans;}
