...
cmp)(void*,void*));struct Cyc_Dict_Dict*Cyc_Dict_rempty(struct _RegionHandle*,int(*
cmp)(void*,void*));int Cyc_Dict_is_empty(struct Cyc_Dict_Dict*d);int Cyc_Dict_member(
...
struct Cyc_Std___cycFILE*fp);unsigned char Cyc_Dict_Absent[11]="\000\000\000\000Absent";
...
_tmp11B.f1=_tag_arr("Dict::marshal: Write failure",sizeof(unsigned char),29);
_tmp11B;});_tmp11A;}));while(dict_list != 0){env=((void*(*)(void*,struct Cyc_Std___cycFILE*,
...
sizeof(unsigned char),35);_tmp11D;});_tmp11C;}));{int i=0;for(0;i < len;i ++){void*
key=read_key(env,fp);void*val=read_val(env,fp);dict=Cyc_Dict_insert(dict,key,val);}}
return dict;}
