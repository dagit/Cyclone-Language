...
int i=0;for(0;i < sz;++ i){ans=ans ^ ((const char*)s.curr)[i]<< shift;shift +=8;if(
shift == 32)shift=0;}}return ans;}int Cyc_Hashtable_hash_stringptr(struct
_tagged_arr*s){return Cyc_Hashtable_hash_string(*s);}void Cyc_Hashtable_insert_bucket(
...
sizeof(char),5),_tag_arr(_tmpD,sizeof(void*),1));}});{struct Cyc_List_List*iter=((
struct Cyc_List_List**)odata.curr)[i];for(0;iter != 0;iter=iter->tl){({void*_tmpF[
0]={};Cyc_Std_printf(_tag_arr("(",sizeof(char),2),_tag_arr(_tmpF,sizeof(void*),0));});
prn_key((((struct _tuple0*)iter->hd)[_check_known_subscript_notnull(1,0)]).f1);({
void*_tmp10[0]={};Cyc_Std_printf(_tag_arr(",",sizeof(char),2),_tag_arr(_tmp10,
sizeof(void*),0));});prn_val((((struct _tuple0*)iter->hd)[
_check_known_subscript_notnull(1,0)]).f2);({void*_tmp11[0]={};Cyc_Std_printf(
_tag_arr(") ",sizeof(char),3),_tag_arr(_tmp11,sizeof(void*),0));});}}({void*
_tmp12[0]={};Cyc_Std_printf(_tag_arr("\n",sizeof(char),2),_tag_arr(_tmp12,
sizeof(void*),0));});}}
