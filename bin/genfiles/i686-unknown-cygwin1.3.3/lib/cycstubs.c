...
unsigned char*tag;struct _tagged_arr f1;};unsigned char*string_to_Cstring(struct
_tagged_arr);unsigned char*underlying_Cstring(struct _tagged_arr);struct
_tagged_arr Cstring_to_string(unsigned char*);struct _tagged_arr
wrap_Cstring_as_string(unsigned char*,unsigned int);struct _tagged_arr
...
struct _tagged_arr name);void*opendir(unsigned char*name);void*Cyc_Std_opendir(
...
int tag;struct Cyc_Std_flock*f1;};struct Cyc_Std_Long_struct{int tag;int f1;};int Cyc_Std_fcntl(
...
lock);int creat(unsigned char*,int);int open_without_mode(unsigned char*,int);int
...
p_proto;};struct Cyc_Cnetdb_Cservent*getservbyname(unsigned char*name,
unsigned char*proto);struct Cyc_Cnetdb_Chostent*gethostbyname(unsigned char*name);
struct Cyc_Cnetdb_Cprotoent*getprotobyname(unsigned char*name);void herror(
unsigned char*);struct _tagged_arr pntlp_toCyc(struct Cyc_Std_in_addr**);struct Cyc_Std_servent*
Cyc_Std_getservbyname(struct _tagged_arr name,struct _tagged_arr proto){struct Cyc_Cnetdb_Cservent*
...
_tmp6->h_aliases=ntCsl_to_ntsl(src->h_aliases);_tmp6->h_addrtype=(int)src->h_addrtype;
_tmp6->h_length=(int)src->h_length;_tmp6->h_addr_list=pntlp_toCyc(src->h_addr_list);
...
struct _tagged_arr Cyc_Std_ctime_r(const int*,struct _tagged_arr);struct Cyc_Std_stat_t{
...
st_spare4[2];};int Cyc_Std_stat(struct _tagged_arr filename,struct Cyc_Std_stat_t*
buf);int Cyc_Std_lstat(struct _tagged_arr filename,struct Cyc_Std_stat_t*buf);int Cyc_Std_mkdir(
struct _tagged_arr pathname,int mode);int Cyc_Std_chmod(struct _tagged_arr path,int
mode);int stat(unsigned char*filename,struct Cyc_Std_stat_t*buf);int lstat(
unsigned char*filename,struct Cyc_Std_stat_t*buf);int mkdir(unsigned char*pathname,
int mode);int chmod(unsigned char*pathname,int mode);int Cyc_Std_stat(struct
_tagged_arr filename,struct Cyc_Std_stat_t*buf){return stat(string_to_Cstring(
...
chown(unsigned char*,unsigned short,unsigned short);unsigned char*getcwd(
