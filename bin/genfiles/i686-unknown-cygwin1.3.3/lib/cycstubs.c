...
unsigned short d_reclen; unsigned char d_type; unsigned char d_name[ 256u]; } ;
void* Cyc_Std_opendir( struct _tagged_arr name); extern struct Cyc_Std_dirent*
readdir( void* dir); extern int closedir( void* dir); extern void* opendir(
unsigned char* name); void* Cyc_Std_opendir( struct _tagged_arr name){ return
opendir( string_to_Cstring( name));} struct Cyc_Std_flock{ short l_type; int
l_start; short l_whence; int l_len; int l_pid; } ; static const int Cyc_Std_Flock=
0; struct Cyc_Std_Flock_struct{ int tag; struct Cyc_Std_flock* f1; } ; static
const int Cyc_Std_Long= 1; struct Cyc_Std_Long_struct{ int tag; int f1; } ;
extern int Cyc_Std_fcntl( int fd, int cmd, struct _tagged_arr); extern int Cyc_Std_open(
struct _tagged_arr, int, struct _tagged_arr); extern int Cyc_Std_creat( struct
_tagged_arr, int); extern int fcntl( int fd, int cmd); extern int fcntl_with_arg(
int fd, int cmd, int arg); extern int fcntl_with_lock( int fd, int cmd, struct
Cyc_Std_flock* lock); extern int creat( unsigned char*, int); extern int
open_without_mode( unsigned char*, int); extern int open_with_mode(
unsigned char*, int, int); int Cyc_Std_fcntl( int fd, int cmd, struct
...
_temp8); _LL7: return fcntl_with_lock( fd, cmd,( struct Cyc_Std_flock*) _temp10);
_LL3:;}}} int Cyc_Std_creat( struct _tagged_arr s, int m){ return creat(
string_to_Cstring( s), m);} int Cyc_Std_open( struct _tagged_arr s, int i,
struct _tagged_arr ms){ if( _get_arr_size( ms, sizeof( int)) >=  1){ return
open_with_mode( string_to_Cstring( s), i,*(( int*) _check_unknown_subscript( ms,
sizeof( int), 0)));} else{ return open_without_mode( string_to_Cstring( s), i);}}
...
h_length; struct Cyc_Std_in_addr** h_addr_list; } ; struct Cyc_Cnetdb_Cprotoent{
...
ntCsl_to_ntsl( src->h_aliases); _temp13->h_addrtype=( int) src->h_addrtype;
_temp13->h_length=( int) src->h_length; _temp13->h_addr_list= pntlp_toCyc( src->h_addr_list);
_temp13;}): 0;} struct Cyc_Std_protoent* Cyc_Std_getprotobyname( struct
...
_temp14=( struct Cyc_Std_protoent*) _cycalloc( sizeof( struct Cyc_Std_protoent));
_temp14->p_name= Cstring_to_string( src->p_name); _temp14->p_aliases=
ntCsl_to_ntsl( src->p_aliases); _temp14->p_proto= src->p_proto; _temp14;}): 0;}
...
; extern int Cyc_Std_inet_aton( struct _tagged_arr cp, struct Cyc_Std_in_addr*
...
struct _tagged_arr); extern int _timezone; extern int daylight; struct Cyc_Std_stat_t{
short st_dev; unsigned int st_ino; int st_mode; unsigned short st_nlink;
unsigned short st_uid; unsigned short st_gid; short st_rdev; int st_size; int
st_atime; int st_spare1; int st_mtime; int st_spare2; int st_ctime; int
st_spare3; int st_blksize; int st_blocks; int st_spare4[ 2u]; } ; extern int Cyc_Std_stat(
...
pathname, int mode){ return chmod( string_to_Cstring( pathname), mode);} struct
Cyc_Cstdio___sFILE; struct Cyc_Std___sFILE; extern struct Cyc_Std___sFILE* Cyc_Std_fromCfile(
struct Cyc_Cstdio___sFILE* cf); extern int Cyc_Std_remove( struct _tagged_arr);
extern int Cyc_Std_rename( struct _tagged_arr, struct _tagged_arr); extern int
Cyc_Std_fclose( struct Cyc_Std___sFILE*); extern int Cyc_Std_fflush( struct Cyc_Std___sFILE*);
...
extern int setsid(); extern int getsid( int pid); extern unsigned short getuid();
