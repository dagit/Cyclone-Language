...
int Cyc_Std_fflush(struct Cyc_Std___cycFILE*);extern unsigned char Cyc_Std_FileCloseError[
...
struct Cyc_List_List*places);extern unsigned char Cyc_Position_Exit[9];void Cyc_Position_reset_position(
...
_tmp4C;});int i=- 1;while(++ i < (*x).f2){*((unsigned char*)_check_unknown_subscript(
marker_str,sizeof(unsigned char),i))=' ';}while(++ i < (*x).f3){*((unsigned char*)
_check_unknown_subscript(marker_str,sizeof(unsigned char),i))='^';}({struct Cyc_Std_String_pa_struct
