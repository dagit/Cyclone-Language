...
unresolvedmem_inits=({struct Cyc_List_List*_tmpBB=_cycalloc(sizeof(*_tmpBB));({struct Cyc_Absyn_Exp*_tmp15C=({struct Cyc_Absyn_Exp*_tmp15B=({struct Cyc_Absyn_Exp*_tmp15A=var;Cyc_Absyn_subscript_exp(_tmp15A,Cyc_Absyn_uint_exp(i,0U),0U);});Cyc_Absyn_assign_exp(_tmp15B,(*((struct _tuple11*)dles->hd)).f2,0U);});_tmpBB->hd=_tmp15C;}),_tmpBB->tl=unresolvedmem_inits;_tmpBB;});}}
# 486
vd->initializer=0;
...
# 659
Cyc_Absyn_assign_exp(_tmp168,(*((struct _tuple11*)dles->hd)).f2,0U);}),0U);Cyc_Absyn_seq_stmt(_tmp169,s3,0U);});}}
# 664
s2->r=s3->r;
