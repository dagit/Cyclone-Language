...
(_tmp7D->i).tvars=0,(_tmp7D->i).effect=0,({struct Cyc_Absyn_Tqual _tmp151=Cyc_Absyn_empty_tqual(0U);(_tmp7D->i).ret_tqual=_tmp151;}),(_tmp7D->i).ret_type=rettype,(_tmp7D->i).args=params,(_tmp7D->i).c_varargs=0,(_tmp7D->i).cyc_varargs=0,(_tmp7D->i).rgn_po=0,(_tmp7D->i).attributes=0,(_tmp7D->i).requires_clause=0,(_tmp7D->i).requires_relns=0,(_tmp7D->i).ensures_clause=0,(_tmp7D->i).ensures_relns=0,(_tmp7D->i).return_value=0,_tmp7D->cached_type=0,_tmp7D->param_vardecls=0,_tmp7D->fn_vardecl=0;_tmp7D;});
# 376
...
void*_tmp174=Cyc_Tcutil_fndecl2type(fd);_tmp12A->type=_tmp174;}),_tmp12A->initializer=0,_tmp12A->rgn=0,_tmp12A->attributes=(fd->i).attributes,_tmp12A->escapes=0;_tmp12A;});
# 722
