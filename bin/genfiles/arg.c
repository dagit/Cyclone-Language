...
_tmp3 <= _get_dyneither_size(s,sizeof(char))?0:({struct _dyneither_ptr _tmp68=({const char*_tmp4="len <= numelts(s)";_tag_dyneither(_tmp4,sizeof(char),18U);});((int(*)(struct _dyneither_ptr assertion,struct _dyneither_ptr file,unsigned line))Cyc___assert_fail)(_tmp68,({const char*_tmp5="arg.cyc";_tag_dyneither(_tmp5,sizeof(char),8U);}),93U);});
...
# 135
struct _dyneither_ptr indentstr=({unsigned _tmp11=(unsigned)(indent + 2)+ 1U;char*_tmp10=_cycalloc_atomic(_check_times(_tmp11,sizeof(char)));({{unsigned _tmp59=(unsigned)(indent + 2);unsigned i;for(i=0;i < _tmp59;++ i){_tmp10[i]='\000';}_tmp10[_tmp59]=0;}0;});_tag_dyneither(_tmp10,sizeof(char),_tmp11);});
{unsigned i=0U;for(0;i < (unsigned)(indent + 1);++ i){
({struct _dyneither_ptr _tmp6=_dyneither_ptr_plus(indentstr,sizeof(char),(int)i);char _tmp7=*((char*)_check_dyneither_subscript(_tmp6,sizeof(char),0U));char _tmp8=i == (unsigned)0?'\n':' ';if(_get_dyneither_size(_tmp6,sizeof(char))== 1U &&(_tmp7 == 0 && _tmp8 != 0))_throw_arraybounds();*((char*)_tmp6.curr)=_tmp8;});}}{
...
# 144
struct _dyneither_ptr temp=({unsigned _tmpE=(((unsigned long)indent - _tmp9)+ (unsigned long)1)+ 1U;char*_tmpD=_cycalloc_atomic(_check_times(_tmpE,sizeof(char)));({{unsigned _tmp58=((unsigned long)indent - _tmp9)+ (unsigned long)1;unsigned i;for(i=0;i < _tmp58;++ i){_tmpD[i]='\000';}_tmpD[_tmp58]=0;}0;});_tag_dyneither(_tmpD,sizeof(char),_tmpE);});
{unsigned i=0U;for(0;i < (unsigned long)indent - _tmp9;++ i){({struct _dyneither_ptr _tmpA=_dyneither_ptr_plus(temp,sizeof(char),(int)i);char _tmpB=*((char*)_check_dyneither_subscript(_tmpA,sizeof(char),0U));char _tmpC=' ';if(_get_dyneither_size(_tmpA,sizeof(char))== 1U &&(_tmpB == 0 && _tmpC != 0))_throw_arraybounds();*((char*)_tmpA.curr)=_tmpC;});}}
itemsep=temp;}}else{
# 149
return;}
# 151
...
# 183
struct _dyneither_ptr progname=(unsigned)prog_pos < _get_dyneither_size(Cyc_Arg_args,sizeof(struct _dyneither_ptr))?*((struct _dyneither_ptr*)_check_dyneither_subscript(Cyc_Arg_args,sizeof(struct _dyneither_ptr),prog_pos)):({const char*_tmp33="(?)";_tag_dyneither(_tmp33,sizeof(char),4U);});
...
unsigned l=_get_dyneither_size(Cyc_Arg_args,sizeof(struct _dyneither_ptr));
# 214
if(({char*_tmp7E=(char*)((struct _dyneither_ptr*)_check_dyneither_subscript(Cyc_Arg_args,sizeof(struct _dyneither_ptr),(int)(l - (unsigned)1)))->curr;_tmp7E == (char*)(_tag_dyneither(0,0,0)).curr;}))l=l - (unsigned)1;
++ Cyc_Arg_current;
...
if((({char*_tmp7F=(char*)s.curr;_tmp7F != (char*)(_tag_dyneither(0,0,0)).curr;})&& _get_dyneither_size(s,sizeof(char))>= (unsigned)1)&&(int)((const char*)s.curr)[0]== (int)'-'){
void*action;
