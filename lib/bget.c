/* Nov. 2004, Mar. 2005
   Modified by Nikhil Swamy, to support regions
*/
...
struct bget_region {
  ...
  struct bget_region *next;
  struct bget_region *prev;
#ifdef BufStats
...
typedef struct bget_region *bget_region_t;

#if (defined(BufStats) && defined(BECtl))

#define UNPACK_REGION(regkey) \
 bget_region_t reg = regkey->reg; \
 struct bfhead *freelist = &reg->freelist; \
 bufsize *totalloc = &reg->totalloc; \
 long *numget = &reg->numget; \
 long *numrel = &reg->numrel; \
 long *numpblk = &reg->numpblk; \
long *numpget = &reg->numpget; \
long *numprel = &reg->numprel; \
...
void *bget(bget_rgn_key_t regkey, bufsize requested_size)
{
  ...
  while (1) {
    ...
    if ((compfcn == NULL) || (!(*compfcn)(regkey, size, ++compactseq))) {
      break;
    }
  }
...
void *bgetz(bget_rgn_key_t regkey, bufsize size)
{
  char *buf = (char *) bget(regkey, size);
...
void *bgetr(bget_rgn_key_t regkey, void *buf, bufsize size)
{
  void *nbuf;
  bufsize osize;		      /* Old size of buffer */
  struct bhead *b;

  if ((nbuf = bget(regkey,size)) == NULL) { /* Acquire new buffer */
    return NULL;
  }
  if (buf == NULL) {
    return nbuf;
...
#endif /* FreeWipe */
    if(relfcn != NULL) //directly acquired buffers are freed lazily with the rest of the reap
      (*relfcn)(regkey, (void *) bdh);      /* Release it directly. */
    return;
  }
#endif /* BECtl */
...
void bectl(bget_rgn_key_t regkey,
	   ...
	   bufsize pool_incr)
{
  regkey->reg->compfcn = compact;
  regkey->reg->acqfcn = acquire;
  regkey->reg->relfcn = release;
  regkey->reg->exp_incr = pool_incr;
}
#endif

/*  BPOOL  --  Add a region of memory to the buffer pool.  */

void bpool(bget_rgn_key_t regkey,
	   void *buf,
	   bufsize len)
{
...
void bstats(bget_rgn_key_t regkey, bufsize *curalloc, 
	    bufsize *totfree, bufsize *maxfree,
	    long *nget, long *nrel)
{
  bget_region_t reg = regkey->reg;
  struct bfhead *b = reg->freelist.ql.flink;

  *nget = reg->numget;
  *nrel = reg->numrel;
  *curalloc = reg->totalloc;
  *totfree = 0;
  *maxfree = -1;
  while (b != &reg->freelist) {
    assert(b->bh.bsize > 0);
    *totfree += b->bh.bsize;
...
void bstatse(bget_rgn_key_t regkey,
	     bufsize *pool_incr,
	     long *npool, long *npget, long *nprel, 
	     long *ndget, long *ndrel)
{
  bget_region_t reg = regkey->reg;
  *pool_incr = (reg->pool_len < 0) ? -reg->exp_incr : reg->exp_incr;
  *npool = reg->numpblk;
  *npget = reg->numpget;
  *nprel = reg->numprel;
  *ndget = reg->numdget;
...
void bufdump(bget_rgn_key_t regkey,
	     void *buf)
{
  struct bfhead *b;
  unsigned char *bdump;
...
void bpoold(bget_rgn_key_t regkey,
	    void *buf,
	    int dumpalloc, 
	    int dumpfree)
{
  ...
  while (b->bh.bsize != ESent) {
    ...
    if (bs < 0) {
      ...
      if (dumpalloc) {
	bufdump(regkey, (void *) (((char *) b) + sizeof(struct bhead)));
      }
    } else {
      char *lerr = "";
...
int bpoolv(bget_rgn_key_t regkey,
	   void *buf)
{
  struct bfhead *b = BFH(buf);
