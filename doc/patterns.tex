...
\item \textbf{datatype patterns}
  ...
  \item Syntax: \texttt{X} if X is a variant that carries no values.
    ...
    \texttt{`tm}.
  \item Types for match: \texttt{datatype Foo.X} (or instantiations of
    it). The number of existential type variables in the pattern must
    be the number of existential type variables for 
    \texttt{datatype Foo.X}.
