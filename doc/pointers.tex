...
\item \hyperlink{pointer_null}{Whether a value of the type may be
    \texttt{NULL}}
\item \hyperlink{pointer_bound}{The (minimum) number of objects pointed to}
\item \hyperlink{pointer_region}{The region into which the pointer
    points}
\item Whether the pointed-to memory is zero-terminated (to be documented).
\end{itemize}
...
evaluated once.  To guarantee there is no check at
run-time, use \texttt{@} instead of \texttt{*}.  For example, the
...
However, a dataflow analysis eliminates many redundant null-checks.
C's under-specified order-of-evaluation can reduce the effectiveness
of the analysis, as in the example on the right above.  But in the
current implementation, the following example has only one check:
\begin{verbatim}
int sum3(int *{3} x) {
  int ans = x[0];
  ans += x[1];
...
\begin{itemize}
\item We already use a dataflow analysis to avoid inserting provably
  unnecessary null-checks.  However, this analysis does not suppress
  spurious warnings about implicit casts from \texttt{t*} even when it
  knows the cast cannot fail.  Instead, it just produces faster code.
