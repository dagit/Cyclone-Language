...
\begin{defun2}{bool}{exists_c}{(bool \vvar{pred}(`a,`b),`a \vvar{env},`b {?}`r \vvar{x});}
  A version of \code{exists} where the predicate takes an environment.
\end{defun2}
...
NUL (0) characters; in any case, the entire substring is appended to
\var{b}, not just the substring up to the first NUL character.
\end{defun}
...
Most functions use the typedef \code{list_t}, which is a possibly-null
pointer to a cons cell; NULL serves as the empty list.  The typedef
\code{List_t} refers to non-null pointers to cons cells; so a
\code{List_t} is a list that has at least one element.
...
\begin{defun2}{int}{length}{(list_t \vvar{x});}
  Returns the length of the list \vvar{x}.
\end{defun2}

\begin{defun2}{`a}{hd}{(list_t<`a> \vvar{x});}
  Returns the first element of list \vvar{x}, or throws
  \code{List_empty} if \vvar{x} is empty.
\end{defun2}

\begin{defun2}{list_t<`a,`r>}{tl}{(list_t<`a,`r> \vvar{x});}
  Returns the tail of \vvar{x}, or throws \code{List_empty} if
  \vvar{x} is empty.  Note, \code{hd(\vvar{x})} is not quite
  equivalent to \code{\vvar{x}->hd}, because the latter will throw
  \code{Null_Exception} if \vvar{x} is empty.
\end{defun2}

\begin{defun2}{list_t<`a>}{copy}{(list_t<`a> \vvar{src});}
  Returns a fresh copy of the list \vvar{src} (all of the cons cells
  are duplicated).
\end{defun2}

\begin{defun2}{list_t<`a,`r>}{rcopy}{(region_t<`r>, list_t<`a> \vvar{src});}
  Returns a fresh copy of the list \vvar{src}, allocated in region
  \code{`r}.
\end{defun2}

\begin{defun2}{`a}{nth}{(list_t<`a> \vvar{x}, int \vvar{i});}
  Returns the \vvar{i}th element of list \vvar{x}.  The elements of
  \vvar{x} are numbered starting with 0.  If \vvar{i} does not
  correspond to an element of \vvar{x}, the exception \code{Nth} is
  thrown.
\end{defun2}

\begin{defun2}{list_t<`a,`r>}{nth_tail}{(list_t<`a,`r> \vvar{x},int \vvar{i});}
  Returns the tail of the list \vvar{x} starting at the \vvar{i}th
  element.  The elements of \vvar{x} are numbered starting with 0.  If
  \vvar{i} does not correspond to an element of \vvar{x}, the
  exception \code{Nth} is thrown.
\end{defun2}

\subsubsection*{Iteration and mapping}

\begin{defun2}{list_t<`b>}{map}{(`b \vvar{f}(`a),list_t<`a> \vvar{x});}
  Applies the function \vvar{f} to each element of list \vvar{x}, and
  returns a new, heap-allocated list of the results.
\end{defun2}

\begin{defun2}{list_t<`b,`r>}{rmap}{(region_t<`r>,`b \vvar{f}(`a),list_t<`a> \vvar{x});}
  A version of map where the returned list is allocated in region
  \vvar{`r}.
\end{defun2}

\begin{defun2}{list_t<`b>}{map_c}{(`b \vvar{f}(`c,`a),`c \vvar{env},list_t<`a> \vvar{x});}
  A version of map where \vvar{f} takes an environment.
\end{defun2}

\begin{defun2}{list_t<`b,`r>}{rmap_c}{(region_t<`r>, `b \vvar{f}(`c,`a), `c, list_t<`a>);}
  A version of \code{map_c} where the returned list is allocated in
  region \vvar{`r}.
\end{defun2}

\begin{defun2}{list_t<`c>}{map2}{(`c \vvar{f}(`a,`b),list_t<`a> \vvar{x},list_t<`b> \vvar{y});}
  Applies \vvar{f} to the pairs consisting of an element from list
  \vvar{x} and its corresponding element from list \vvar{y}.  The
  results are returned in a new list.  If \vvar{x} and \vvar{y} don't
  have the same length, it throws \code{List_mismatch}.  Note,
  \vvar{f} may be applied some number of times before
  \code{List_mismatch} is thrown.
\end{defun2}

\begin{defun2}{list_t<`c,`r>}{rmap2}{(region_t<`r>,`c \vvar{f}(`a,`b), list_t<`a>, list_t<`b>);}
  A version of \code{map2} where the returned list is allocated in
  region \vvar{`r}.
\end{defun2}

\begin{defun2}{void}{app}{(`b \vvar{f}(`a), list_t<`a> \vvar{x});}
  Applies \vvar{f} to each element of list \vvar{x}.  The results of the
  function application are not saved.
\end{defun2}

\begin{defun2}{void}{app_c}{(`c \vvar{f}(`a,`b), `a, list_t<`b> \vvar{x});}
  A version of app, where \vvar{f} takes an environment.
\end{defun2}

\begin{defun2}{void}{app2}{(`c \vvar{f}(`a,`b),list_t<`a> \vvar{x},list_t<`b> \vvar{y});}
  Applies \vvar{f} to one element of \vvar{x} and one element of
  \vvar{y} in turn, from left to right in the lists \vvar{x} and
  \vvar{y}.  If the lists \vvar{x} and \vvar{y} are not the same
  length, \code{List_mismatch} is thrown.
\end{defun2}

\begin{defun2}{void}{app2_c}{(`d \vvar{f}(`a,`b,`c), `a \vvar{env}, list_t<`b> \vvar{x},list_t<`c> \vvar{y});}
  A version of \code{app2} where \vvar{f} takes an environment
  \vvar{env}.
\end{defun2}

\begin{defun2}{void}{iter}{(void \vvar{f}(`a),list_t<`a> \vvar{x});}
  Like \code{app(\vvar{f},\vvar{x})} except that \vvar{f} returns void.
\end{defun2}

\begin{defun2}{void}{iter_c}{(void \vvar{f}(`b,`a),`b \vvar{env},list_t<`a> \vvar{x});}
  Like \code{app_c(\vvar{f},\vvar{env},\vvar{x})} except that \vvar{f}
  returns void.
\end{defun2}

\begin{defun2}{void}{iter2}{(void f(`a,`b),list_t<`a> x,list_t<`b> y);}
  Like \code{app2(\var{f},\var{x},\var{y})} except that \var{f}
  returns void.
\end{defun2}

\begin{defun2}{void}{iter2_c}{(void \vvar{f}(`a,`b,`c),`a \vvar{env},list_t<`b> \vvar{x},list_t<`c> \vvar{y});}
  Like \code{app2_c(\vvar{f},\vvar{env},\vvar{x},\vvar{y})} except that
  \vvar{f} returns void.
\end{defun2}
