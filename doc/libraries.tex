...
\begin{defun2}{int}{sprintf}{(char {?}`r1 \var{s}, const char {?}`r2 \var{fmt}, ...`r3 inject parg_t<`r4>);}
  ...
  occurred.
\end{defun2}
\begin{defun2}{char ?}{xprintf(const char {?}`r2 \var{fmt}, ...`r3 inject parg_t<`r4>);}
...
\begin{defun2}{char {?}`r1}{vrxprintf}{(region_t<`r1> r1, const char {?}`r2 fmt, parg_t<`r4> ? `r3 ap);}
  A version of \code{rxprintf} suitable for calling from a user's
  vararg function.
\end{defun2}
...
\code{sscanf}, using its type safe vararg feature; the library
