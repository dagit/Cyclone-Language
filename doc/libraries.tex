...
\begin{defun}{msort}{(comp,x,ofs,len)}
  Does an in-place sort of a segment of array \var{x}, starting at the
  element with index \var{ofs}, and continuing for \var{len} elements.
  The function \var{comp} should return an integer less than, equal
  to, or greater than 0 if its first argument is less than, equal to,
  or greater than its second argument, respectively.  \code{msort}
  throws \code{Core::InvalidArg("Array::msort")} if \var{len} is
  negative or \var{ofs} and \var{len} specify a segment outside the
  bounds of \var{x}.
\end{defun}
...
\begin{defun}{singleton}{(comp,key,data)}
...
\begin{defun}{create}{(sz,comp,hash)}
Builds a new, empty hash table with initial size \var{sz}.  \var{comp}
...
be a hash function on the keys of the hash table.  \code{comp} and
\code{hash} should satisfy the following property: if
\code{comp(\var{x},\var{y})} is 0, then \code{hash(\var{x}) ==
hash(\var{y})}.
\end{defun}
