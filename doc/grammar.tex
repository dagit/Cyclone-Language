...
\begin{rules}{type-specifier}
  ...
  \tk{valueof_t} \tk{(} \nt{expression} \tk{)} 
\end{rules}
\begin{rulesoneof}{kind}
  \tk{::A} ~ \tk{::B} ~ \tk{::M} ~ \tk{::R} ~ \tk{::I} ~ \tk{::TR} ~ \tk{::UR}
\end{rulesoneof}
...
\begin{rules}{datatype-specifier}
  ...
  \nt{xdatatype}  \nt{identifier} \tk{.} \nt{identifier} \nt{type-params}\opt
\end{rules}
\begin{rules}{xdatatype}
  \tk{datatype} \\
  \tk{@extensible} \tk{datatype}
\end{rules}
...
\end{rules}
\begin{rules}{pointer}
  \tk{*}\ \nt{pointer-qualifier-list} \nt{range}\opt\ \nt{region}\opt\ \nt{type-qualifier-list}\opt\ \nt{pointer}\opt\\
  \tk{@}\ \nt{pointer-qualifier-list} \nt{range}\opt\ \nt{region}\opt\ \nt{type-qualifier-list}\opt\ \nt{pointer}\opt\\
  \tk{?}\ \nt{pointer-qualifier-list} \nt{region}\opt\ \nt{type-qualifier-list}\opt\ \nt{pointer}\opt
\end{rules}
\begin{rulesoneof}{array-qualifier}
  \tk{@zeroterm} ~
  \tk{@nozeroterm}
\end{rulesoneof}
\begin{rules}{pointer-qualifier-list}
  (empty) \\
  \nt{pointer-qualifier} \nt{pointer-qualifier-list}
\end{rules}
\begin{rules}{pointer-qualifier}
  \nt{array-qualifier} \\
...
\begin{rules}{pattern}
  ...
  \nt{identifier} \nt{type-params}\opt\ \tk{(} \nt{tuple-pattern-list} \tk{, ...}\tk{)}\\
  \tk{\$\tk{(}} \nt{tuple-pattern-list} \tk{)}\\
  \tk{\$\tk{(}} \nt{tuple-pattern-list} \tk{, ...} \tk{)}\\
  \nt{identifier} \nt{type-params}\opt\ {\lb} {\rb}\\
  \nt{identifier} \nt{type-params}\opt\ {\lb} \nt{field-pattern-list} {\rb}\\
  \nt{identifier} \nt{type-params}\opt\ {\lb} \nt{field-pattern-list} \tk{, ...} {\rb}\\
  \tk{\&} \nt{pattern}\\
  \tk{*} \nt{identifier}
\end{rules}
...
\begin{rules}{unary-expression}
  ...
  \tk{malloc} \tk{(} \nt{expression} \tk{)} \\
  \tk{rmalloc} \tk{(} \nt{assignment-expression} \tk{,} \nt{expression} \tk{)} \\
  \tk{calloc} \tk{(} \nt{assignment-expression} \tk{,} \tk{sizeof(} \nt{type-name} \tk{)} \tk{)} \\
  \tk{rcalloc} \tk{(} \nt{assignment-expression} \tk{,} \nt{assignment-expression} \tk{,} \tk{sizeof(} \nt{type-name} \tk{)} \tk{)} \\
  \tk{numelts} \tk{(} \nt{expression} \tk{)} \\
