...
Cyclone provides \emph{datatypes} so that you can safely write
functions that accept an argument with more than one possible type.
Like a C \texttt{union}, a Cyclone \texttt{datatype} is a type that has
...
\begin{verbatim}
    ...
    datatype t @x = new Integer(3);
    datatype t @y = new String("hello, world");
\end{verbatim}
This declares a new type, \texttt{datatype t}, that can hold either an
...
\texttt{datatype t}.  For example, \texttt{x} is initialized with
\texttt{new Integer(3)}, which allocates storage on the heap,
initializes it with the tag \texttt{Integer} and the integer 3, and
returns a pointer to the storage.  Therefore, \texttt{x} has type
``non-NULL pointer to a value of type \texttt{datatype t}'', or
\texttt{datatype t @} for short.  The next declaration is
similar, except that \texttt{y} is initialized with the
\texttt{String} case.

\emph{Pattern matching} is used to determine the tag of a value of
type \texttt{datatype t}, and to extract the underlying value.  For
example, here is a function that will print either an integer or a
string:
\begin{verbatim}
    void print(datatype t a) {
      switch (a) {
...
\end{verbatim}
The argument \texttt{a} has type \texttt{datatype t}, so it is either
...
The cases of a \texttt{datatype} can carry any number of values,
...
\begin{verbatim}
    datatype tree {
      Empty;
      Leaf(int);
      Node(datatype tree @, datatype tree @);
    };
\end{verbatim}
A tree can be empty, or it can be a single (leaf) node holding an
integer, or it can be an internal node that points to a left and a
right subtree.  In other words, \texttt{datatype tree} is the type of
possibly empty binary trees with integer leaves.

Here's a function, \texttt{sum}, that calculates the sum of the leaves
of a tree:
\begin{verbatim}
    int sum(datatype tree @x) {
      switch (x) {
      case &Empty: return 0;
...
tag in the type \texttt{datatype tree}.  One thing to notice is that
the argument \texttt{x} is passed as a \emph{pointer} to a
\texttt{datatype t}, instead of just a \texttt{datatype t}.  This is
because each case of a Cyclone datatype can occupy
a different size in memory (in contrast to a C union).  For example,
the \texttt{Empty} case only needs to hold a tag (\texttt{Empty}),
while a value \texttt{Leaf(i)} must hold both a tag and an integer,
and a value \texttt{Node(y,z)} must hold a tag and two pointers.
So, \texttt{x} is passed as a function.

Similarly, the subtrees \texttt{x} and \texttt{y} of
\texttt{Node(x,y)} are pointers as well.  If we tried to make them
just \texttt{datatype tree}, then the compiler would not know the size
of a \texttt{Node} value.

You can find out more about patterns in
\hyperref[{sec:patterns}]{Section~\ref{sec:patterns}}; for more about
\texttt{datatype} and memory management, see
\hyperref[{sec:regions}]{Section~\ref{sec:regions}}.
...
\texttt{datatype}: with \texttt{datatype}, all of the cases have to be
...
\texttt{datatype} types can be used to code around many of the uses
...
\item Unions in Cyclone can only hold ``bits.''  In particular, they
  ...
  multiple types, then use tagged unions (\texttt{datatype}s).
...
\item Cyclone currently does not support nested type declarations
  ...
  \texttt{datatype}, \texttt{xdatatype}, and \texttt{typedef} definitions must
  be at the top-level.

\item Cyclone does not allow you to use a \texttt{struct}, \texttt{datatype},
  \texttt{union}, \texttt{xdatatype}, or \texttt{enum} type without first
