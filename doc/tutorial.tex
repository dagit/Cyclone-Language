...
\hyperref[{sec:install}]{Appendix~\ref{sec:install}} or the Cyclone
web site if you need to install the compiler).
...
\texttt{int}, Cyclone requires that it explicitly return an integer.
...
receive some random value instead.  If the returned value is supposed
to be a pointer, the caller might try to dereference it, and
dereferencing a random address can cause the program to crash.  So,
...
\item You can't cast an integer to a pointer.  Cyclone prevents this
  ...
  \texttt{NULL}) as a pointer in Cyclone.  Instead, Cyclone provides
  \texttt{NULL} as a keyword.
\item You can't do pointer arithmetic on a \texttt{*} pointer.
  ...
  causes a crash.  (Note, however, that pointer arithmetic is possible
  using \texttt{?}\ pointers instead.)
...
We try to explain pointers in much more detail in
...
region inference}.  For instance, we can re-write the function
...
8 }
\end{verbatim}
and Cyclone can still figure out that \texttt{y} is a pointer into
...
There is a special region for the heap, written \texttt{`H}
that holds all of the storage for top-level variables, and for data
allocated via \texttt{new} or \texttt{malloc}.  For instance, if
we write the following declarations at the top-level:
\begin{verbatim}
struct Point p = {0,1};
struct Point *ptr = &p;
\end{verbatim}
...
Note that unlike block
regions, the number of objects \texttt{`r} that you allocate into
...
as a handle on the heap.  Indeed, \texttt{new} and \texttt{malloc(e)}
are just abbreviations for \texttt{rnew(heap_region)} and
\texttt{rmalloc(heap_region,e)} respectively.  
...
since \texttt{x} was initialized by \texttt{new Integer(3)}, and
...
\item Struct expressions:  If you've declared
  ...
  \texttt{point\{.x=expression, .y=expression\}} to allocate and
...
In addition, there are a number of short-comings of the current
...
\item Cyclone does not allow 0 (zero) to be treated as the NULL pointer.
\end{itemize}
