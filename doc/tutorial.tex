...
regions is discussed in Section~\ref{sec:dynamic-regions}.}
...
\begin{verbatim}
foo.cyc:2: type mismatch: struct Point *`GR0 != struct Point *`GR1
  `GR1 and `GR0 are not compatible. (variable types are not the same)
\end{verbatim}
...
But you can require that they be instantiated with the same region by
explicitly specifying the same explicit region variable
...
\emph{reference-counted} regions.  For more information on these,
